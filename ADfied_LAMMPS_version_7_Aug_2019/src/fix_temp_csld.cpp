...
void FixTempCSLD::end_of_step()
{
  ...
  myScalar t_current = temperature->compute_scalar();
  myScalar ekin_old = t_current * 0.5 * temperature->dof * force->boltz;
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      const myScalar factor = 1.0/sqrt(m);
      const myScalar vx = random->gaussian() * factor;
      vhold[i][0] = v[i][0];
      v[i][0] = vx;
      const myScalar vy = random->gaussian() * factor;
      vhold[i][1] = v[i][1];
      v[i][1] = vy;
      const myScalar vz = random->gaussian() * factor;
      vhold[i][2] = v[i][2];
      v[i][2] = vz;
    }
  }
  ...
  const myScalar c2 = sqrt((1.0-c1*c1)*t_target/temperature->compute_scalar());
...
void FixTempCSLD::reset_target(myScalar t_new)
{
...
myScalar FixTempCSLD::compute_scalar()
{
  return energy;
}
