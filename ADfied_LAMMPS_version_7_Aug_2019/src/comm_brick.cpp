...
void CommBrick::setup()
{
  ...
  if (triclinic == 0) {
    ...
    if (mode == Comm::MULTI) {
      myScalar *cuttype = neighbor->cuttype;
      for (i = 1; i <= ntypes; i++) {
        cut = 0.0;
    ...
    myScalar length0,length1,length2;
  ...
  if (layout == Comm::LAYOUT_UNIFORM) {
    ...
    int all[6];
    MY_MPI_Allreduce(&recvneed[0][0],all,6,MPI_INT,MPI_MAX,world);
...
int CommBrick::updown(int dim, int dir, int loc,
                 myScalar prd, int periodicity, myScalar *split)
{
  int index,count;
  myScalar frac,delta;
...
void CommBrick::forward_comm_array(int nsize, myScalar **array)
{
...
int CommBrick::exchange_variable(int n, myScalar *inbuf, myScalar *&outbuf)
{
  ...
  nrecv = n;
  if (nrecv > maxrecv) grow_recv(nrecv);
  memcpy(buf_recv,inbuf,nrecv*sizeof(myScalar));
