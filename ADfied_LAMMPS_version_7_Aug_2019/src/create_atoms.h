...
class CreateAtoms : protected Pointers {
 ...
 private:
  ...
  int triclinic;
  myScalar sublo[3],subhi[3];   // epsilon-extended proc sub-box for adding atoms

  void add_single();
  void add_random();
  void add_lattice();
  void add_molecule(myScalar *, myScalar * = NULL);
  int vartest(myScalar *);        // evaluate a variable with new atom position
};
