...
void MinLineSearch::setup_style()
{
  ...
  if (nextra_global) {
    gextra = new myScalar[nextra_global];
    hextra = new myScalar[nextra_global];
  }

  // memory for x0,g,h for extra per-atom dof

  if (nextra_atom) {
    x0extra_atom = new myScalar*[nextra_atom];
    gextra_atom = new myScalar*[nextra_atom];
    hextra_atom = new myScalar*[nextra_atom];
...
int MinLineSearch::linemin_backtrack(myScalar eoriginal, myScalar &alpha)
{
  int i,m,n;
  myScalar fdothall,fdothme,hme,hmax,hmaxall;
  myScalar de_ideal,de;
  myScalar *xatom,*x0atom,*fatom,*hatom;
  ...
  MY_MPI_Allreduce(&fdothme,&fdothall,1,MPI_DOUBLE,MPI_SUM,world);
  if (nextra_global)
  ...
  hme = 0.0;
  for (i = 0; i < nvec; i++) hme = MAX(ad::passive_value(hme),fabs(ad::passive_value(h[i])));
  MY_MPI_Allreduce(&hme,&hmaxall,1,MPI_DOUBLE,MPI_MAX,world);
  alpha = MIN(ALPHA_MAX,ad::passive_value(dmax/hmaxall));
  if (nextra_atom)
    for (m = 0; m < nextra_atom; m++) {
      hatom = hextra_atom[m];
      n = extra_nlen[m];
      hme = 0.0;
      for (i = 0; i < n; i++) hme = MAX(ad::passive_value(hme),fabs(ad::passive_value(hatom[i])));
      MY_MPI_Allreduce(&hme,&hmax,1,MPI_DOUBLE,MPI_MAX,world);
      alpha = MIN(alpha,extra_max[m]/ad::passive_value(hmax));
      hmaxall = MAX(ad::passive_value(hmaxall),ad::passive_value(hmax));
    }
  if (nextra_global) {
    myScalar alpha_extra = modify->max_alpha(hextra);
    alpha = MIN(alpha,alpha_extra);
    for (i = 0; i < nextra_global; i++)
      hmaxall = MAX(ad::passive_value(hmaxall),fabs(ad::passive_value(hextra[i])));
  }
  ...
  // myScalar etmp;
  // myScalar alphatmp = alpha*1.0e-4;
...
int MinLineSearch::linemin_quadratic(myScalar eoriginal, myScalar &alpha)
{
  int i,m,n;
  myScalar fdothall,fdothme,hme,hmax,hmaxall;
  myScalar de_ideal,de;
  myScalar delfh,engprev,relerr,alphaprev,fhprev,ff,fh,alpha0;
  myScalar dot[2],dotall[2];
  myScalar *xatom,*x0atom,*fatom,*hatom;
  myScalar alphamax;
  ...
  for (i = 0; i < nvec; i++) hme = MAX(ad::passive_value(hme),fabs(ad::passive_value(h[i])));
  MY_MPI_Allreduce(&hme,&hmaxall,1,MPI_DOUBLE,MPI_MAX,world);
  alphamax = MIN(ALPHA_MAX,ad::passive_value(dmax/hmaxall));
  if (nextra_atom)
    for (m = 0; m < nextra_atom; m++) {
      hatom = hextra_atom[m];
      n = extra_nlen[m];
      ...
      for (i = 0; i < n; i++) hme = MAX(ad::passive_value(hme),fabs(ad::passive_value(hatom[i])));
      MY_MPI_Allreduce(&hme,&hmax,1,MPI_DOUBLE,MPI_MAX,world);
      alphamax = MIN(ad::passive_value(alphamax),ad::passive_value(extra_max[m]/hmax));
      hmaxall = MAX(ad::passive_value(hmaxall),hmax);
    }
  if (nextra_global) {
    myScalar alpha_extra = modify->max_alpha(hextra);
  ...
  // // important diagnostic: test the gradient against energy
  // myScalar etmp;
  // myScalar alphatmp = alphamax*1.0e-4;
  // etmp = alpha_step(alphatmp,1);
  // printf("alpha = %g dele = %g dele_force = %g err = %g\n",
  //        alphatmp,etmp-eoriginal,-alphatmp*fdothall,
  //        etmp-eoriginal+alphatmp*fdothall);
  ...
  while (1) {
    ...
    MY_MPI_Allreduce(dot,dotall,2,MPI_DOUBLE,MPI_SUM,world);
    if (nextra_global) {
      for (i = 0; i < nextra_global; i++) {
...
int MinLineSearch::linemin_forcezero(myScalar eoriginal, myScalar &alpha)
{
  int i,m,n;
  myScalar fdothall,fdothme,hme,hmax,hmaxall;
  myScalar de;
  ...
  hme = 0.0;
  for (i = 0; i < nvec; i++)
    hme = MAX(ad::passive_value(hme),fabs(ad::passive_value(h[i])));
  ...
  if (nextra_atom)
    for (m = 0; m < nextra_atom; m++) {
      ...
      for (i = 0; i < n; i++) hme = MAX(ad::passive_value(hme),fabs(ad::passive_value(hatom[i])));
      MY_MPI_Allreduce(&hme,&hmax,1,MPI_DOUBLE,MPI_MAX,world);
      alpha_max = MIN(alpha_max,extra_max[m]/ad::passive_value(hmax));
      hmaxall = MAX(ad::passive_value(hmaxall),ad::passive_value(hmax));
    }
  ...
  while (1) {
    ...
    // new alpha_del should be much smaller
      ...
      myScalar boostFactor = LIMIT_BOOST;
...
myScalar MinLineSearch::alpha_step(myScalar alpha, int resetflag)
{
  int i,n,m;
  myScalar *xatom,*x0atom,*hatom;
...
myScalar MinLineSearch::compute_dir_deriv(myScalar &ff)
{
   int i,m,n;
   myScalar *hatom, *fatom;
   myScalar dot[2],dotall[2];
   myScalar fh;
