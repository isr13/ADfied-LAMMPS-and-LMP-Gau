...
void Set::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    MY_MPI_Allreduce(&count,&allcount,1,MPI_INT,MPI_SUM,world);
...
void Set::set(int keyword)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    if (keyword == TYPE) {
      ...
      if (dvalue >= 0.0) atom->dpdTheta[i] = dvalue;
        ...
        myScalar vx = atom->v[i][0];
        myScalar vy = atom->v[i][1];
        myScalar vz = atom->v[i][2];
        myScalar tfactor = force->mvv2e / (domain->dimension * force->boltz);
    ...
    // else set rmass to density directly
      ...
      if (atom->radius_flag && atom->radius[i] > 0.0)
        ...
        myScalar *shape = avec_ellipsoid->bonus[atom->ellipsoid[i]].shape;
        ...
        myScalar length = avec_line->bonus[atom->line[i]].length;
        atom->rmass[i] = length * dvalue;
      } else if (atom->tri_flag && atom->tri[i] >= 0) {
        myScalar *c1 = avec_tri->bonus[atom->tri[i]].c1;
        myScalar *c2 = avec_tri->bonus[atom->tri[i]].c2;
        myScalar *c3 = avec_tri->bonus[atom->tri[i]].c3;
    ...
    // set magnetic moments
      ...
      myScalar **sp = atom->sp;
      myScalar inorm = 1.0/sqrt(xvalue*xvalue+yvalue*yvalue+zvalue*zvalue);
      sp[i][0] = inorm*xvalue;
      sp[i][1] = inorm*yvalue;
      sp[i][2] = inorm*zvalue;
      sp[i][3] = dvalue;
    ...
    // enforce quat rotation vector in z dir for 2d systems
      ...
      myScalar theta2 = MY_PI2 * wvalue/180.0;
      myScalar sintheta2 = sin(theta2);
      quat[0] = cos(theta2);
    ...
    // set value for custom integer or myScalar vector
  ...
  if (keyword == SHAPE) {
    ...
    MY_MPI_Allreduce(&nlocal_bonus,&atom->nellipsoids,1,
...
void Set::setrandom(int keyword)
{
  ...
  // set quaternions to random orientations in 3d and 2d
    ...
    if (domain->dimension == 3) {
      myScalar s,t1,t2,theta1,theta2;
      for (i = 0; i < nlocal; i++)
        if (select[i]) {
          if (avec_ellipsoid && atom->ellipsoid[i] >= 0)
            quat = avec_ellipsoid->bonus[atom->ellipsoid[i]].quat;
