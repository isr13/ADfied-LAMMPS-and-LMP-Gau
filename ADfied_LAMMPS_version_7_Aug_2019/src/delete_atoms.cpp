...
void DeleteAtoms::command(int narg, char **arg)
{
  ...
  if (atom->nellipsoids > 0) {
    ...
    MY_MPI_Allreduce(&nlocal_bonus,&atom->nellipsoids,1,MPI_LMP_BIGINT,MPI_SUM,world);
  }
  if (atom->nlines > 0) {
    nlocal_bonus = avec_line->nlocal_bonus;
    MY_MPI_Allreduce(&nlocal_bonus,&atom->nlines,1,MPI_LMP_BIGINT,MPI_SUM,world);
  }
  if (atom->ntris > 0) {
    nlocal_bonus = avec_tri->nlocal_bonus;
    MY_MPI_Allreduce(&nlocal_bonus,&atom->ntris,1,MPI_LMP_BIGINT,MPI_SUM,world);
  }
  if (atom->nbodies > 0) {
    nlocal_bonus = avec_body->nlocal_bonus;
    MY_MPI_Allreduce(&nlocal_bonus,&atom->nbodies,1,MPI_LMP_BIGINT,MPI_SUM,world);
  }
...
void DeleteAtoms::delete_overlap(int narg, char **arg)
{
  ...
  myScalar cut = force->numeric(FLERR,arg[1]);
  myScalar cutsq = cut*cut;
  ...
  // myScalar loop over owned atoms and their full neighbor list
  ...
  myScalar factor_lj,factor_coul;
...
void DeleteAtoms::delete_porosity(int narg, char **arg)
{
  ...
  myScalar porosity_fraction = force->numeric(FLERR,arg[2]);
...
void DeleteAtoms::recount_topology()
{
  ...
  if (atom->avec->bonds_allow) {
    MY_MPI_Allreduce(&nbonds,&atom->nbonds,1,MPI_LMP_BIGINT,MPI_SUM,world);
    if (!force->newton_bond) atom->nbonds /= 2;
  }
  if (atom->avec->angles_allow) {
    MY_MPI_Allreduce(&nangles,&atom->nangles,1,MPI_LMP_BIGINT,MPI_SUM,world);
    if (!force->newton_bond) atom->nangles /= 3;
  }
  if (atom->avec->dihedrals_allow) {
    MY_MPI_Allreduce(&ndihedrals,&atom->ndihedrals,1,MPI_LMP_BIGINT,MPI_SUM,world);
    if (!force->newton_bond) atom->ndihedrals /= 4;
  }
  if (atom->avec->impropers_allow) {
    MY_MPI_Allreduce(&nimpropers,&atom->nimpropers,1,MPI_LMP_BIGINT,MPI_SUM,world);
