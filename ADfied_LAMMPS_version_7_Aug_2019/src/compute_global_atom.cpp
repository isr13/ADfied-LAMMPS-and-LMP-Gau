...
void ComputeGlobalAtom::compute_peratom()
{
  ...
  // integer indices are rounded down from myScalar values
  ...
  for (int m = 0; m < nvalues; m++) {
    ...
    // output = array
      ...
      if (which[m] == COMPUTE) {
        ...
        myScalar **compute_array = compute->array;
...
myScalar ComputeGlobalAtom::memory_usage()
{
  myScalar bytes = nmax*nvalues * sizeof(myScalar);
  bytes += nmax * sizeof(int);                    // indices
  if (varatom) bytes += nmax * sizeof(myScalar);    // varatom
  bytes += maxvector * sizeof(myScalar);            // vecglobal
  return bytes;
}
