...
void FixTempBerendsen::end_of_step()
{
  ...
  myScalar lamda = sqrt(1.0 + update->dt/t_period*(t_target/t_current - 1.0));
  myScalar efactor = 0.5 * force->boltz * tdof;
...
void FixTempBerendsen::reset_target(myScalar t_new)
{
...
myScalar FixTempBerendsen::compute_scalar()
{
  return energy;
}
