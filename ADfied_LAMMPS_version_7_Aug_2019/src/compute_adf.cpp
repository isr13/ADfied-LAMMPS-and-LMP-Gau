  ...
  neighjkatom(NULL), bothjkatom(NULL), delrjkatom(NULL)
{
  ...
  rcutinnerj = new myScalar[ntriples];
  rcutouterj = new myScalar[ntriples];
  rcutinnerk = new myScalar[ntriples];
  rcutouterk = new myScalar[ntriples];
  ...
  delrjkatom = new myScalar**[ntriples];
  for (int m = 0; m < ntriples; m++) {
    maxjkatom[m] = 10;
...
{
  myScalar mycutneigh = 0.0;
  myScalar maxouter = 0.0;
...
void ComputeADF::compute_array()
{
  ...
  myScalar delr1[3],delr2[3],rinv1,rinv2,rinv12,cs,theta;
  ...
  MY_MPI_Allreduce(hist[0],histall[0],ntriples*nbin,MPI_DOUBLE,MPI_SUM,world);
  MY_MPI_Allreduce(iatomcount,iatomcountall,ntriples,MPI_INT,MPI_SUM,world);
  ...
  for (m = 0; m < ntriples; m++) {
    ...
    myScalar normfac1, pdftheta, normfac2, adftheta;
