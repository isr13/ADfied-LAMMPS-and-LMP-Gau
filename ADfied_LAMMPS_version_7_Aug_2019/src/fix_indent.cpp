...
void FixIndent::post_force(int /*vflag*/)
{
  ...
  if (istyle == SPHERE) {
    ...
    myScalar delx,dely,delz,r,dr,fmag,fx,fy,fz;
...
myScalar FixIndent::compute_scalar()
{
  // only sum across procs one time

  if (indenter_flag == 0) {
    MY_MPI_Allreduce(indenter,indenter_all,4,MPI_DOUBLE,MPI_SUM,world);
    indenter_flag = 1;
  }
...
myScalar FixIndent::compute_vector(int n)
{
  // only sum across procs one time
