...
void WriteRestart::write(char *file)
{
  ...
  //   but nlocal * myScalars-peratom could oveflow

  int max_size;
  int send_size = atom->avec->size_restart();
  MY_MPI_Allreduce(&send_size,&max_size,1,MPI_INT,MPI_MAX,world);
  ...
  // else wait for ping from fileproc, send my data to fileproc
    ...
    if (filewriter) {
      ...
      for (int iproc = 0; iproc < nclusterprocs; iproc++) {
        ...
        write_myScalar_vec(PERPROC,recv_size,buf);
      }
      fclose(fp);
      fp = NULL;
...
void WriteRestart::header()
{
  ...
  write_myScalar_vec(BOUNDMIN,6,minbound);
  ...
  write_myScalar_vec(BOXLO,3,domain->boxlo);
  write_myScalar_vec(BOXHI,3,domain->boxhi);
  write_myScalar(XY,domain->xy);
  write_myScalar(XZ,domain->xz);
  write_myScalar(YZ,domain->yz);

  write_myScalar_vec(SPECIAL_LJ,3,&force->special_lj[1]);
  write_myScalar_vec(SPECIAL_COUL,3,&force->special_coul[1]);

  write_myScalar(TIMESTEP,update->dt);
  ...
  write_myScalar(ATOM_SORTBIN,atom->userbinsize);

  write_int(COMM_MODE,comm->mode);
  write_myScalar(COMM_CUTOFF,comm->cutghostuser);
...
{
  if (atom->mass) write_myScalar_vec(MASS,atom->ntypes,&atom->mass[1]);
...
/* ----------------------------------------------------------------------
   write a flag and a myScalar into restart file
------------------------------------------------------------------------- */

void WriteRestart::write_myScalar(int flag, myScalar value)
{
  fwrite(&flag,sizeof(int),1,fp);
  fwrite(&value,sizeof(myScalar),1,fp);
}
...
/* ----------------------------------------------------------------------
   write a flag and vector of N myScalars into restart file
------------------------------------------------------------------------- */

void WriteRestart::write_myScalar_vec(int flag, int n, myScalar *vec)
{
  fwrite(&flag,sizeof(int),1,fp);
  fwrite(&n,sizeof(int),1,fp);
  fwrite(vec,sizeof(myScalar),n,fp);
}
