...
myScalar FixSpringSelf::compute_scalar()
{
  myScalar all;
  MY_MPI_Allreduce(&espring,&all,1,MPI_DOUBLE,MPI_SUM,world);
  return all;
}
...
myScalar FixSpringSelf::memory_usage()
{
...
int FixSpringSelf::pack_exchange(int i, myScalar *buf)
{
  buf[0] = xoriginal[i][0];
  buf[1] = xoriginal[i][1];
  buf[2] = xoriginal[i][2];
...
int FixSpringSelf::unpack_exchange(int nlocal, myScalar *buf)
{
...
int FixSpringSelf::pack_restart(int i, myScalar *buf)
{
  buf[0] = 4;
  buf[1] = xoriginal[i][0];
  buf[2] = xoriginal[i][1];
