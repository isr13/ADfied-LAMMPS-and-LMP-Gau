...
{
  int i1,i2,n,type;
  myScalar delx,dely,delz,ebond,fbond;
  myScalar rsq,r0sq,rlogarg,sr2,sr6;
...
void BondFENEExpand::coeff(int narg, char **arg)
{
  ...
  myScalar r0_one = force->numeric(FLERR,arg[2]);
  myScalar epsilon_one = force->numeric(FLERR,arg[3]);
  myScalar sigma_one = force->numeric(FLERR,arg[4]);
  myScalar shift_one = force->numeric(FLERR,arg[5]);
...
myScalar BondFENEExpand::equilibrium_distance(int i)
{
...
{
  fwrite(&k[1],sizeof(myScalar),atom->nbondtypes,fp);
  fwrite(&r0[1],sizeof(myScalar),atom->nbondtypes,fp);
  fwrite(&epsilon[1],sizeof(myScalar),atom->nbondtypes,fp);
  fwrite(&sigma[1],sizeof(myScalar),atom->nbondtypes,fp);
  fwrite(&shift[1],sizeof(myScalar),atom->nbondtypes,fp);
}
...
void BondFENEExpand::read_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    fread(&k[1],sizeof(myScalar),atom->nbondtypes,fp);
    fread(&r0[1],sizeof(myScalar),atom->nbondtypes,fp);
    fread(&epsilon[1],sizeof(myScalar),atom->nbondtypes,fp);
    fread(&sigma[1],sizeof(myScalar),atom->nbondtypes,fp);
    fread(&shift[1],sizeof(myScalar),atom->nbondtypes,fp);
  }
...
myScalar BondFENEExpand::single(int type, myScalar rsq, int /*i*/, int /*j*/,
                        myScalar &fforce)
{
  myScalar r = sqrt(rsq);
  myScalar rshift = r - shift[type];
  myScalar rshiftsq = rshift*rshift;
  myScalar r0sq = r0[type] * r0[type];
  myScalar rlogarg = 1.0 - rshiftsq/r0sq;
  ...
  myScalar eng = -0.5 * k[type]*r0sq*log(rlogarg);
