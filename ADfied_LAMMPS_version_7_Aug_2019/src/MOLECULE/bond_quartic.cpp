...
void BondQuartic::compute(int eflag, int vflag)
{
  ...
  myScalar delx,dely,delz,ebond,fbond,evdwl,fpair;
  myScalar r,rsq,dr,r2,ra,rb,sr2,sr6;
  ...
  myScalar **cutsq = force->pair->cutsq;
  myScalar **x = atom->x;
  myScalar **f = atom->f;
...
void BondQuartic::coeff(int narg, char **arg)
{
  ...
  myScalar b1_one = force->numeric(FLERR,arg[2]);
  myScalar b2_one = force->numeric(FLERR,arg[3]);
  myScalar rc_one = force->numeric(FLERR,arg[4]);
  myScalar u0_one = force->numeric(FLERR,arg[5]);
...
myScalar BondQuartic::equilibrium_distance(int /*i*/)
{
  return 0.97;
}
...
void BondQuartic::write_restart(FILE *fp)
{
  ...
  fwrite(&b1[1],sizeof(myScalar),atom->nbondtypes,fp);
  fwrite(&b2[1],sizeof(myScalar),atom->nbondtypes,fp);
  fwrite(&rc[1],sizeof(myScalar),atom->nbondtypes,fp);
  fwrite(&u0[1],sizeof(myScalar),atom->nbondtypes,fp);
}
...
void BondQuartic::read_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    ...
    fread(&b1[1],sizeof(myScalar),atom->nbondtypes,fp);
    fread(&b2[1],sizeof(myScalar),atom->nbondtypes,fp);
    fread(&rc[1],sizeof(myScalar),atom->nbondtypes,fp);
    fread(&u0[1],sizeof(myScalar),atom->nbondtypes,fp);
  }
...
myScalar BondQuartic::single(int type, myScalar rsq, int i, int j,
                           myScalar &fforce)
{
  myScalar r,dr,r2,ra,rb,sr2,sr6;
