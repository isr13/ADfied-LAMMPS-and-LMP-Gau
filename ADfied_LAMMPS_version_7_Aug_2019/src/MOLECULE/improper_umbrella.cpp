...
void ImproperUmbrella::compute(int eflag, int vflag)
{
  ...
  myScalar eimproper,f1[3],f2[3],f3[3],f4[3];
  myScalar vb1x,vb1y,vb1z,vb2x,vb2y,vb2z,vb3x,vb3y,vb3z;
  myScalar domega,c,a,s,projhfg,dhax,dhay,dhaz,dahx,dahy,dahz,cotphi;
  myScalar ax,ay,az,ra2,rh2,ra,rh,rar,rhr,arx,ary,arz,hrx,hry,hrz;
...
void ImproperUmbrella::coeff(int narg, char **arg)
{
  ...
  myScalar w_one = force->numeric(FLERR,arg[2]);
...
{
  fwrite(&kw[1],sizeof(myScalar),atom->nimpropertypes,fp);
  fwrite(&w0[1],sizeof(myScalar),atom->nimpropertypes,fp);
  fwrite(&C[1],sizeof(myScalar),atom->nimpropertypes,fp);
}
...
void ImproperUmbrella::read_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    fread(&kw[1],sizeof(myScalar),atom->nimpropertypes,fp);
    fread(&w0[1],sizeof(myScalar),atom->nimpropertypes,fp);
    fread(&C[1],sizeof(myScalar),atom->nimpropertypes,fp);
  }
