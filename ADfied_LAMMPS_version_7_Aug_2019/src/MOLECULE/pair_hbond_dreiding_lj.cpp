...
void PairHbondDreidingLJ::compute(int eflag, int vflag)
{
  ...
  tagint tagprev;
  myScalar delx,dely,delz,rsq,rsq1,rsq2,r1,r2;
  myScalar factor_hb,force_angle,force_kernel,evdwl,eng_lj,ehbond,force_switch;
  myScalar c,s,a,b,ac,a11,a12,a22,vx1,vx2,vy1,vy2,vz1,vz2,d;
  myScalar fi[3],fj[3],delr1[3],delr2[3];
  myScalar r2inv,r10inv;
...
void PairHbondDreidingLJ::coeff(int narg, char **arg)
{
  ...
  myScalar epsilon_one = force->numeric(FLERR,arg[4]);
  myScalar sigma_one = force->numeric(FLERR,arg[5]);

  int ap_one = ap_global;
  if (narg > 6) ap_one = force->inumeric(FLERR,arg[6]);
  myScalar cut_inner_one = cut_inner_global;
  myScalar cut_outer_one = cut_outer_global;
  if (narg > 8) {
  ...
  myScalar cut_angle_one = cut_angle_global;
...
void PairHbondDreidingLJ::init_style()
{
  ...
  for (int m = 0; m < nparams; m++) {
    ...
    /*
    if (offset_flag) {
      myScalar ratio = params[m].sigma / params[m].cut_outer;
...
myScalar PairHbondDreidingLJ::init_one(int i, int j)
{
  int m;
...
myScalar PairHbondDreidingLJ::single(int i, int j, int itype, int jtype,
                                   myScalar rsq,
                                   ...
                                   myScalar &fforce)
{
  ...
  tagint tagprev;
  myScalar eng,eng_lj,force_kernel,force_angle;
  myScalar rsq1,rsq2,r1,r2,c,s,ac,r2inv,r10inv,factor_hb;
  myScalar switch1,switch2;
  myScalar delr1[3],delr2[3];
  tagint *klist;
