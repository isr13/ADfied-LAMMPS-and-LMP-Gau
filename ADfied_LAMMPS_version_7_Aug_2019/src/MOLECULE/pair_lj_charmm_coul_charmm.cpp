...
void PairLJCharmmCoulCharmm::compute(int eflag, int vflag)
{
  ...
  myScalar philj,switch1,switch2;
...
void PairLJCharmmCoulCharmm::coeff(int narg, char **arg)
{
  ...
  myScalar eps14_one = epsilon_one;
  myScalar sigma14_one = sigma_one;
  if (narg == 6) {
...
myScalar PairLJCharmmCoulCharmm::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        ...
        fwrite(&eps14[i][j],sizeof(myScalar),1,fp);
        fwrite(&sigma14[i][j],sizeof(myScalar),1,fp);
      }
    }
}
    ...
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          ...
          fread(&eps14[i][j],sizeof(myScalar),1,fp);
          fread(&sigma14[i][j],sizeof(myScalar),1,fp);
        }
...
myScalar PairLJCharmmCoulCharmm::single(int i, int j, int itype, int jtype,
                                      myScalar rsq,
                                      myScalar factor_coul, myScalar factor_lj,
                                      myScalar &fforce)
{
  myScalar r2inv,r6inv,forcecoul,forcelj,phicoul,philj;
  myScalar switch1,switch2;
