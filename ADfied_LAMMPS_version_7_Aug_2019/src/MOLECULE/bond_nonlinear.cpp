...
void BondNonlinear::compute(int eflag, int vflag)
{
  ...
  myScalar rsq,r,dr,drsq,lamdasq,denom,denomsq;
...
void BondNonlinear::coeff(int narg, char **arg)
{
  ...
  myScalar epsilon_one = force->numeric(FLERR,arg[1]);
  myScalar r0_one = force->numeric(FLERR,arg[2]);
  myScalar lamda_one = force->numeric(FLERR,arg[3]);
...
myScalar BondNonlinear::equilibrium_distance(int i)
{
  return r0[i];
}
...
void BondNonlinear::write_restart(FILE *fp)
{
  ...
  fwrite(&lamda[1],sizeof(myScalar),atom->nbondtypes,fp);
}
...
void BondNonlinear::read_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    ...
    fread(&lamda[1],sizeof(myScalar),atom->nbondtypes,fp);
  }
...
myScalar BondNonlinear::single(int type, myScalar rsq, int /*i*/, int /*j*/,
                             myScalar &fforce)
{
  myScalar r = sqrt(rsq);
  myScalar dr = r - r0[type];
  myScalar drsq = dr*dr;
  myScalar lamdasq = lamda[type]*lamda[type];
  myScalar denom = lamdasq - drsq;
  myScalar denomsq = denom*denom;
