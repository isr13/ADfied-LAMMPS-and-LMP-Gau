...
myScalar FixSetForce::compute_vector(int n)
{
  // only sum across procs one time

  if (force_flag == 0) {
    MY_MPI_Allreduce(foriginal,foriginal_all,3,MPI_DOUBLE,MPI_SUM,world);
    force_flag = 1;
  }
...
myScalar FixSetForce::memory_usage()
{
  myScalar bytes = 0.0;
  if (varflag == ATOM) bytes = maxatom*3 * sizeof(myScalar);
  return bytes;
}
