...
class Domain : protected Pointers {
 public:
  ...
  int tiltsmall;                         // 1 if limit tilt, else 0
                                         ...
                                         // orthogonal box
  myScalar xprd,yprd,zprd;                 // global box dimensions
  myScalar xprd_half,yprd_half,zprd_half;  // half dimensions
  myScalar prd[3];                         // array form of dimensions
  myScalar prd_half[3];                    // array form of half dimensions

                                         // triclinic box
                                         // xprd,xprd_half,prd,prd_half =
                                         // same as if untilted
  myScalar prd_lamda[3];                   // lamda box = (1,1,1)
  myScalar prd_half_lamda[3];              // lamda half box = (0.5,0.5,0.5)

  myScalar boxlo[3],boxhi[3];              // orthogonal box global bounds

                                         // triclinic box
                                         // boxlo/hi = same as if untilted
  myScalar boxlo_lamda[3],boxhi_lamda[3];  // lamda box = (0,1)
  myScalar boxlo_bound[3],boxhi_bound[3];  // bounding box of tilted domain
  myScalar corners[8][3];                  // 8 corner points

                                         // orthogonal box & triclinic box
  myScalar minxlo,minxhi;                  // minimum size of global box
  myScalar minylo,minyhi;                  // when shrink-wrapping
  myScalar minzlo,minzhi;                  // tri only possible for non-skew dims

                                         // orthogonal box
  myScalar sublo[3],subhi[3];              // sub-box bounds on this proc

                                         // triclinic box
                                         // sublo/hi = undefined
  myScalar sublo_lamda[3],subhi_lamda[3];  // bounds of subbox in lamda

                                         // triclinic box
  myScalar xy,xz,yz;                       // 3 tilt factors
  myScalar h[6],h_inv[6];                  // shape matrix in Voigt notation
  myScalar h_rate[6],h_ratelo[3];          // rate of box size/shape change
  ...
  void subbox_too_small_check(myScalar);
  void minimum_image(myScalar &, myScalar &, myScalar &);
  void minimum_image(myScalar *);
  void minimum_image_once(myScalar *);
  int closest_image(int, int);
  int closest_image(const myScalar * const, int);
  void closest_image(const myScalar * const, const myScalar * const, myScalar * const);
  void remap(myScalar *, imageint &);
  void remap(myScalar *);
  void remap_near(myScalar *, myScalar *);
  void unmap(myScalar *, imageint);
  void unmap(const myScalar *, imageint, myScalar *);
  void image_flip(int, int, int);
  int ownatom(int, myScalar *, imageint *, int);
  ...
  virtual void lamda2x(myScalar *, myScalar *);
  virtual void x2lamda(myScalar *, myScalar *);
  int inside(myScalar *);
  int inside_nonperiodic(myScalar *);
  void x2lamda(myScalar *, myScalar *, myScalar *, myScalar *);
  void bbox(myScalar *, myScalar *, myScalar *, myScalar *);
  void box_corners();
  void subbox_corners();
  void lamda_box_corners(myScalar *, myScalar *);
  ...
  inline int minimum_image_check(myScalar dx, myScalar dy, myScalar dz) {
 ...
 protected:
  myScalar small[3];                  // fractions of box lengths
