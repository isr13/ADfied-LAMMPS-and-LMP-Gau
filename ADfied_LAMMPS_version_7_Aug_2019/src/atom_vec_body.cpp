...
AtomVecBody::AtomVecBody(LAMMPS *lmp) : AtomVec(lmp)
{
  ...
  if (sizeof(myScalar) == sizeof(int)) intmyScalarratio = 1;
  else if (sizeof(myScalar) == 2*sizeof(int)) intmyScalarratio = 2;
...
int AtomVecBody::pack_comm(int n, int *list, myScalar *buf,
...
int AtomVecBody::pack_comm_vel(int n, int *list, myScalar *buf,
                              int pbc_flag, int *pbc)
{
  int i,j,m;
  myScalar dx,dy,dz,dvx,dvy,dvz;
  myScalar *quat;
...
int AtomVecBody::pack_comm_hybrid(int n, int *list, myScalar *buf)
{
  int i,j,m;
  myScalar *quat;
...
void AtomVecBody::unpack_comm(int n, int first, myScalar *buf)
{
  int i,m,last;
  myScalar *quat;
...
void AtomVecBody::unpack_comm_vel(int n, int first, myScalar *buf)
{
  int i,m,last;
  myScalar *quat;
...
int AtomVecBody::unpack_comm_hybrid(int n, int first, myScalar *buf)
{
  int i,m,last;
  myScalar *quat;
...
int AtomVecBody::pack_reverse(int n, int first, myScalar *buf)
{
  int i,m,last;
...
int AtomVecBody::pack_reverse_hybrid(int n, int first, myScalar *buf)
{
  int i,m,last;
...
void AtomVecBody::unpack_reverse(int n, int *list, myScalar *buf)
{
  int i,j,m;
...
int AtomVecBody::unpack_reverse_hybrid(int n, int *list, myScalar *buf)
{
  int i,j,m;
...
int AtomVecBody::pack_border(int n, int *list, myScalar *buf,
                            int pbc_flag, int *pbc)
{
  int i,j,m;
  myScalar dx,dy,dz;
  myScalar *quat,*inertia;
  ...
  if (pbc_flag == 0) {
    for (i = 0; i < n; i++) {
      ...
      if (body[j] < 0) buf[m++] = ubuf(0).d;
        ...
        buf[m++] = ubuf(bonus[body[j]].nmyScalar).d;
...
int AtomVecBody::pack_border_vel(int n, int *list, myScalar *buf,
                                int pbc_flag, int *pbc)
{
  int i,j,m;
  myScalar dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecBody::pack_border_hybrid(int n, int *list, myScalar *buf)
{
  int i,j,m;
  myScalar *quat,*inertia;
...
void AtomVecBody::unpack_border(int n, int first, myScalar *buf)
{
  int i,j,m,last;
  myScalar *quat,*inertia;
...
void AtomVecBody::unpack_border_vel(int n, int first, myScalar *buf)
{
  int i,j,m,last;
  myScalar *quat,*inertia;
...
int AtomVecBody::unpack_border_hybrid(int n, int first, myScalar *buf)
{
  int i,j,m,last;
  myScalar *quat,*inertia;
...
int AtomVecBody::pack_exchange(int i, myScalar *buf)
{
  int m = 1;
  buf[m++] = x[i][0];
  buf[m++] = x[i][1];
  ...
  if (body[i] < 0) buf[m++] = ubuf(0).d;
    ...
    buf[m++] = ubuf(1).d;
    int j = body[i];
    myScalar *quat = bonus[j].quat;
    myScalar *inertia = bonus[j].inertia;
    buf[m++] = quat[0];
    buf[m++] = quat[1];
    buf[m++] = quat[2];
    buf[m++] = quat[3];
    ...
    buf[m++] = ubuf(bonus[j].nmyScalar).d;
    memcpy(&buf[m],bonus[j].ivalue,bonus[j].ninteger*sizeof(int));
    if (intmyScalarratio == 1) m += bonus[j].ninteger;
    else m += (bonus[j].ninteger+1)/2;
    memcpy(&buf[m],bonus[j].dvalue,bonus[j].nmyScalar*sizeof(myScalar));
    m += bonus[j].nmyScalar;
  }
...
int AtomVecBody::unpack_exchange(myScalar *buf)
{
  ...
  if (body[nlocal] == 0) body[nlocal] = -1;
    ...
    myScalar *quat = bonus[nlocal_bonus].quat;
    myScalar *inertia = bonus[nlocal_bonus].inertia;
    quat[0] = buf[m++];
    quat[1] = buf[m++];
    quat[2] = buf[m++];
    quat[3] = buf[m++];
    ...
    bonus[nlocal_bonus].nmyScalar = (int) ubuf(buf[m++]).i;
    ...
    bonus[nlocal_bonus].dvalue = dcp->get(bonus[nlocal_bonus].nmyScalar,
                                          bonus[nlocal_bonus].dindex);
    memcpy(bonus[nlocal_bonus].ivalue,&buf[m],
           bonus[nlocal_bonus].ninteger*sizeof(int));
    if (intmyScalarratio == 1) m += bonus[nlocal_bonus].ninteger;
    else m += (bonus[nlocal_bonus].ninteger+1)/2;
    memcpy(bonus[nlocal_bonus].dvalue,&buf[m],
           bonus[nlocal_bonus].nmyScalar*sizeof(myScalar));
    m += bonus[nlocal_bonus].nmyScalar;
...
int AtomVecBody::size_restart()
{
  ...
  for (i = 0; i < nlocal; i++)
    if (body[i] >= 0) {
      n += 26;
      if (intmyScalarratio == 1) n += bonus[body[i]].ninteger;
      else n += (bonus[body[i]].ninteger+1)/2;
      n += bonus[body[i]].nmyScalar;
    } else n += 17;
...
int AtomVecBody::pack_restart(int i, myScalar *buf)
{
  int m = 1;
  buf[m++] = x[i][0];
  buf[m++] = x[i][1];
...
int AtomVecBody::unpack_restart(myScalar *buf)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) {
    grow(0);
...
void AtomVecBody::create_atom(int itype, myScalar *coord)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecBody::data_atom(myScalar *coord, imageint imagetmp, char **values)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecBody::data_body(int m, int ninteger, int nmyScalar,
                            int *ivalues, myScalar *dvalues)
{
  if (body[m]) error->one(FLERR,"Assigning body parameters to non-body atom");
  if (nlocal_bonus == nmax_bonus) grow_bonus();
  bonus[nlocal_bonus].ilocal = m;
  bptr->data_body(nlocal_bonus,ninteger,nmyScalar,ivalues,dvalues);
  body[m] = nlocal_bonus++;
}
...
void AtomVecBody::pack_data(myScalar **buf)
{
  int nlocal = atom->nlocal;
...
int AtomVecBody::pack_data_hybrid(int i, myScalar *buf)
{
...
void AtomVecBody::write_data(FILE *fp, int n, myScalar **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecBody::write_data_hybrid(FILE *fp, myScalar *buf)
{
...
void AtomVecBody::pack_vel(myScalar **buf)
{
  int nlocal = atom->nlocal;
  for (int i = 0; i < nlocal; i++) {
    buf[i][0] = ubuf(tag[i]).d;
...
int AtomVecBody::pack_vel_hybrid(int i, myScalar *buf)
{
  buf[0] = angmom[i][0];
  buf[1] = angmom[i][1];
  buf[2] = angmom[i][2];
...
void AtomVecBody::write_vel(FILE *fp, int n, myScalar **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecBody::write_vel_hybrid(FILE *fp, myScalar *buf)
{
...
myScalar AtomVecBody::radius_body(int ninteger, int nmyScalar,
                                int *ivalues, myScalar *dvalues)
{
  return bptr->radius_body(ninteger,nmyScalar,ivalues,dvalues);
}
...
void AtomVecBody::set_quat(int m, myScalar *quat_external)
{
  if (body[m] < 0) error->one(FLERR,"Assigning quat to non-body atom");
  myScalar *quat = bonus[body[m]].quat;
...
bigint AtomVecBody::memory_usage()
{
  ...
  for (int i = 0; i < nall; i++) {
    ...
    bytes += bonus[i].nmyScalar * sizeof(myScalar);
  }
