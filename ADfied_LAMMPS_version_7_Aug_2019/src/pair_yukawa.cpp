...
void PairYukawa::compute(int eflag, int vflag)
{
  ...
  myScalar rsq,r2inv,r,rinv,screening,forceyukawa,factor;
...
myScalar PairYukawa::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
  ...
  if (offset_flag && (cut[i][j] > 0.0)) {
    myScalar screening = exp(-kappa * cut[i][j]);
...
{
  fwrite(&kappa,sizeof(myScalar),1,fp);
...
{
  if (comm->me == 0) {
    fread(&kappa,sizeof(myScalar),1,fp);
...
myScalar PairYukawa::single(int /*i*/, int /*j*/, int itype, int jtype, myScalar rsq,
                          myScalar /*factor_coul*/, myScalar factor_lj,
                          myScalar &fforce)
{
  myScalar r2inv,r,rinv,screening,forceyukawa,phi;
