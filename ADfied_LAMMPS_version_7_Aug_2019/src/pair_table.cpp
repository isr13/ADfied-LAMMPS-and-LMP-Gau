...
void PairTable::compute(int eflag, int vflag)
{
  ...
  myScalar rsq,factor_lj,fraction,value,a,b;
  char estr[128];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (tabstyle == LOOKUP) {
          ...
          rsq_lookup.f = static_cast<long>(rsq);
...
void PairTable::allocate()
{
  ...
  memset(&cutsq[0][0],0,nt*nt*sizeof(myScalar));
...
myScalar PairTable::init_one(int i, int j)
{
...
void PairTable::read_table(Table *tb, char *file, char *keyword)
{
  ...
  myScalar r,e,f,rprev,rnext,eprev,enext,fleft,fright;
...
void PairTable::compute_table(Table *tb)
{
  ...
  if (tabstyle == SPLINE) {
    ...
    myScalar ep0 = - tb->f[0] / (2.0 * sqrt(tb->innersq));
    myScalar epn = - tb->f[tlm1] / (2.0 * tb->cut);
    ...
    myScalar fp0,fpn;
    myScalar secant_factor = 0.1;
    if (tb->fpflag) fp0 = (tb->fplo/sqrt(tb->innersq) - tb->f[0]/tb->innersq) /
      (2.0 * sqrt(tb->innersq));
    else {
      myScalar rsq1 = tb->innersq;
      myScalar rsq2 = rsq1 + secant_factor*tb->delta;
    ...
    if (tb->fpflag && tb->cut == tb->rfile[tb->ninput-1]) fpn =
      ...
      myScalar rsq2 = tb->cut * tb->cut;
      myScalar rsq1 = rsq2 - secant_factor*tb->delta;
    ...
    if (rsq_lookup.f < tb->cut*tb->cut) {
      if (tb->match) {
        ...
        tb->drsq[itablemax] = tb->drsq[itablemaxm1];
            ...
            rsq_lookup.f = static_cast<long>(tb->cut*tb->cut);
        r = sqrtf(rsq_lookup.f);
...
void PairTable::spline(myScalar *x, myScalar *y, int n,
...
myScalar PairTable::splint(myScalar *xa, myScalar *ya, myScalar *y2a, int n, myScalar x)
{
  int klo,khi,k;
  myScalar h,b,a,y;
...
myScalar PairTable::single(int /*i*/, int /*j*/, int itype, int jtype, myScalar rsq,
                         myScalar /*factor_coul*/, myScalar factor_lj,
                         myScalar &fforce)
{
  int itable;
  myScalar fraction,value,a,b,phi;
  int tlm1 = tablength - 1;
...
void *PairTable::extract(const char *str, int &dim)
{
  ...
  myScalar cut_coul = tables[0].cut;
