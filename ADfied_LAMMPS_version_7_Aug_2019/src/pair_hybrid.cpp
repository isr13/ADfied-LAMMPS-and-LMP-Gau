...
void PairHybrid::compute(int eflag, int vflag)
{
  ...
  myScalar *saved_special = save_special();
  ...
  for (m = 0; m < nstyles; m++) {
    ...
    if (eflag_atom) {
      ...
      myScalar *eatom_substyle = styles[m]->eatom;
      for (i = 0; i < n; i++) eatom[i] += eatom_substyle[i];
    }
    if (vflag_atom) {
      n = atom->nlocal;
      if (force->newton_pair) n += atom->nghost;
      myScalar **vatom_substyle = styles[m]->vatom;
      for (i = 0; i < n; i++)
        for (j = 0; j < 6; j++)
...
void PairHybrid::settings(int narg, char **arg)
{
  ...
  special_lj = new myScalar*[narg];
  special_coul = new myScalar*[narg];
...
void PairHybrid::flags()
{
  ...
  if (single_extra) {
    delete [] svector;
    svector = new myScalar[single_extra];
  }
}
...
myScalar PairHybrid::init_one(int i, int j)
{
  ...
  for (int k = 0; k < nmap[i][j]; k++) {
    map[j][i][k] = map[i][j][k];
    myScalar cut = styles[map[i][j][k]]->init_one(i,j);
...
void PairHybrid::write_restart(FILE *fp)
{
  ...
  for (int m = 0; m < nstyles; m++) {
    ...
    fwrite(&n,sizeof(int),1,fp);
    if (n) fwrite(special_lj[m],sizeof(myScalar),4,fp);
    n = (special_coul[m] == NULL) ? 0 : 1;
    fwrite(&n,sizeof(int),1,fp);
    if (n) fwrite(special_coul[m],sizeof(myScalar),4,fp);
  }
}
...
void PairHybrid::read_restart(FILE *fp)
{
  ...
  special_lj = new myScalar*[nstyles];
  special_coul = new myScalar*[nstyles];
  ...
  for (int m = 0; m < nstyles; m++) {
    ...
    MPI_Bcast(&n,1,MPI_INT,0,world);
    if (n > 0 ) {
      special_lj[m] = new myScalar[4];
      if (me == 0) fread(special_lj[m],sizeof(myScalar),4,fp);
      MPI_Bcast(special_lj[m],4,MPI_DOUBLE,0,world);
    }
    if (me == 0) fread(&n,sizeof(int),1,fp);
    MPI_Bcast(&n,1,MPI_INT,0,world);
    if (n > 0 ) {
      special_coul[m] = new myScalar[4];
      if (me == 0) fread(special_coul[m],sizeof(myScalar),4,fp);
...
myScalar PairHybrid::single(int i, int j, int itype, int jtype,
...
void PairHybrid::modify_special(int m, int /*narg*/, char **arg)
{
  ...
  if (strcmp(arg[0],"lj/coul") == 0) {
    if (!special_lj[m]) special_lj[m] = new myScalar[4];
    if (!special_coul[m]) special_coul[m] = new myScalar[4];
    for (i = 0; i < 4; ++i)
...
myScalar * PairHybrid::save_special()
{
  myScalar *saved = new myScalar[8];
...
void PairHybrid::restore_special(myScalar *saved)
{
  for (int i = 0; i < 4; ++i) {
...
void *PairHybrid::extract(const char *str, int &dim)
{
  ...
  for (int m = 0; m < nstyles; m++) {
    ...
    if (ptr && strcmp(str,"cut_coul") == 0) {
      ...
      myScalar *p_newvalue = (myScalar *) ptr;
      myScalar newvalue = *p_newvalue;
...
myScalar PairHybrid::memory_usage()
{
