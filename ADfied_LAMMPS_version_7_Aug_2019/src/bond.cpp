...
void Bond::ev_tally(int i, int j, int nlocal, int newton_bond,
                    myScalar ebond, myScalar fbond,
                    myScalar delx, myScalar dely, myScalar delz)
{
  myScalar ebondhalf,v[6];
...
void Bond::write_file(int narg, char **arg)
{
  ...
  myScalar inner = force->numeric(FLERR,arg[2]);
  myScalar outer = force->numeric(FLERR,arg[3]);
  if (inner <= 0.0 || inner >= outer)
    error->all(FLERR,"Invalid rlo/rhi values in bond_write command");


  myScalar r0 = equilibrium_distance(btype);
  ...
  if (me == 0) {
    ...
    const myScalar dr = (outer-inner) / static_cast<myScalar>(n-1);
    for (int i = 0; i < n; i++) {
      r = inner + dr * static_cast<myScalar>(i);
