  ...
  coord(NULL), vector(NULL)
{
  ...
  bin = new myScalar[nbins];
  bin_total = new myScalar[nbins];
  bin_all = new myScalar[nbins];
  coord = new myScalar[nbins];
...
void FixAveHisto::end_of_step()
{
  ...
  if (kind == PERATOM || kind == LOCAL) {
    MY_MPI_Allreduce(stats,stats_all,2,MPI_DOUBLE,MPI_SUM,world);
    MY_MPI_Allreduce(&stats[2],&stats_all[2],1,MPI_DOUBLE,MPI_MIN,world);
    MY_MPI_Allreduce(&stats[3],&stats_all[3],1,MPI_DOUBLE,MPI_MAX,world);
    MY_MPI_Allreduce(bin,bin_all,nbins,MPI_DOUBLE,MPI_SUM,world);
...
myScalar FixAveHisto::compute_vector(int i)
{
...
myScalar FixAveHisto::compute_array(int i, int j)
{
...
void FixAveHisto::bin_one(myScalar value)
{
...
void FixAveHisto::bin_vector(int n, myScalar *values, int stride)
{
  int m = 0;
  for (int i = 0; i < n; i++) {
    bin_one(values[m]);
...
void FixAveHisto::bin_atoms(myScalar *values, int stride)
{
  int *mask = atom->mask;
