  ...
  flangevin(NULL), tforce(NULL), franprev(NULL), id_temp(NULL), random(NULL)
{
  ...
  gfactor1 = new myScalar[atom->ntypes+1];
  gfactor2 = new myScalar[atom->ntypes+1];
  ratio = new myScalar[atom->ntypes+1];
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"angmom") == 0) {
      ...
      myScalar scale = force->numeric(FLERR,arg[iarg+2]);
...
void FixLangevin::init()
{
  ...
  if (ad::passive_value(ascale) && !atom->ellipsoid_flag)
...
void FixLangevin::post_force_templated()
{
  ...
  myScalar fdrag[3],fran[3],fsum[3],fsumall[3];
  bigint count;
  myScalar fswap;

  myScalar boltz = force->boltz;
  myScalar dt = update->dt;
  myScalar mvv2e = force->mvv2e;
  myScalar ftm2v = force->ftm2v;
  ...
  if (Tp_ZERO) {
    MY_MPI_Allreduce(fsum,fsumall,3,MPI_DOUBLE,MPI_SUM,world);
    fsumall[0] /= count;
    fsumall[1] /= count;
    fsumall[2] /= count;
    for (int i = 0; i < nlocal; i++) {
  ...
  if (ad::passive_value(ascale)) angmom_thermostat();
}
...
void FixLangevin::angmom_thermostat()
{
  ...
  myScalar inertia[3],omega[3],tran[3];
  myScalar *shape,*quat;
...
void FixLangevin::reset_target(myScalar t_new)
{
...
myScalar FixLangevin::compute_scalar()
{
  ...
  myScalar energy_me = energy - 0.5*energy_onestep*update->dt;

  myScalar energy_all;
  MY_MPI_Allreduce(&energy_me,&energy_all,1,MPI_DOUBLE,MPI_SUM,world);
  return -energy_all;
}
...
myScalar FixLangevin::memory_usage()
{
  myScalar bytes = 0.0;
  if (gjfflag) bytes += atom->nmax*3 * sizeof(myScalar);
  if (tallyflag) bytes += atom->nmax*3 * sizeof(myScalar);
  if (tforce) bytes += atom->nmax * sizeof(myScalar);
  return bytes;
}
...
int FixLangevin::pack_exchange(int i, myScalar *buf)
{
...
int FixLangevin::unpack_exchange(int nlocal, myScalar *buf)
{
