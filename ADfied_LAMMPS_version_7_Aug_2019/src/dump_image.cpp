  ...
  chooseghost(NULL), bufcopy(NULL)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"atom") == 0) {
      ...
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) {
        ...
        myScalar theta = force->numeric(FLERR,arg[iarg+1]);
      ...
      if (strstr(arg[iarg+2],"v_") == arg[iarg+2]) {
        ...
        myScalar phi = force->numeric(FLERR,arg[iarg+2]);
        phi *= MY_PI/180.0;
        image->phi = phi;
      }
      iarg += 3;
      ...
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) {
        ...
        myScalar zoom = force->numeric(FLERR,arg[iarg+1]);
      ...
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) {
        ...
        myScalar persp = force->numeric(FLERR,arg[iarg+1]);
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal dump image command");
      myScalar shiny = force->numeric(FLERR,arg[iarg+1]);
      ...
      image->seed = seed;
      myScalar ssaoint = force->numeric(FLERR,arg[iarg+3]);
  ...
  diamtype = new myScalar[ntypes+1];
  diamelement = new myScalar[ntypes+1];
  colortype = new myScalar*[ntypes+1];
  colorelement = new myScalar*[ntypes+1];
  ...
  if (bondflag) {
    bdiamtype = new myScalar[atom->nbondtypes+1];
    bcolortype = new myScalar*[atom->nbondtypes+1];
...
void DumpImage::write()
{
  ...
  if (acolor == ATTRIBUTE && image->map_dynamic(0)) {
    ...
    two[0] = -lo;
    two[1] = hi;
    MY_MPI_Allreduce(two,twoall,2,MPI_DOUBLE,MPI_MAX,world);
...
void DumpImage::view_params()
{
  ...
  if (thetastr) {
    myScalar theta = input->variable->compute_equal(thetavar);
    if (theta < 0.0 || theta > 180.0)
      error->all(FLERR,"Invalid dump image theta value");
    theta *= MY_PI/180.0;
    image->theta = theta;
  ...
  if (phistr) {
    myScalar phi = input->variable->compute_equal(phivar);
    phi *= MY_PI/180.0;
    image->phi = phi;
  }
...
void DumpImage::create_image()
{
  ...
  tagint tagprev;
  myScalar diameter,delx,dely,delz;
  int *bodyvec,*fixvec;
  myScalar **bodyarray,**fixarray;
  myScalar *color,*color1,*color2;
  myScalar *p1,*p2,*p3;
  myScalar xmid[3],pt1[3],pt2[3],pt3[3];
  myScalar mat[3][3];
  ...
  if (subboxflag) {
    myScalar diameter = MIN(boxxhi-boxxlo,boxyhi-boxylo);
    if (domain->dimension == 3) diameter = MIN(diameter,boxzhi-boxzlo);
    diameter *= subboxdiam;

    myScalar *sublo = domain->sublo;
    myScalar *subhi = domain->subhi;

    myScalar (*boxcorners)[3];
    myScalar box[8][3];
    if (domain->triclinic == 0) {
  ...
  if (axesflag) {
    ...
    myScalar offset = MAX(boxxhi-boxxlo,boxyhi-boxylo);
...
int DumpImage::pack_forward_comm(int n, int *list, myScalar *buf,
...
void DumpImage::unpack_forward_comm(int n, int first, myScalar *buf)
{
  int i,m,last;
...
int DumpImage::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"adiam") == 0) {
    ...
    int nlo,nhi;
    force->bounds(FLERR,arg[1],atom->ntypes,nlo,nhi);
    myScalar diam = force->numeric(FLERR,arg[2]);
  ...
  if (strcmp(arg[0],"backcolor") == 0) {
    ...
    myScalar *color = image->color2rgb(arg[1]);
