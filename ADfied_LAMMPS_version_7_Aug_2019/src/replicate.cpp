...
void Replicate::command(int narg, char **arg)
{
  ...
  if (atom->molecule_flag) {
    ...
    tagint maxmol_all;
    MY_MPI_Allreduce(&maxmol,&maxmol_all,1,MPI_LMP_TAGINT,MPI_MAX,world);
    maxmol = maxmol_all;
  }
  ...
  if (bbox_flag) {
    ...
    MY_MPI_Allreduce(MPI_IN_PLACE, &(_imagelo[0]), 3, MPI_INT, MPI_MIN, world);
    MY_MPI_Allreduce(MPI_IN_PLACE, &(_imagehi[0]), 3, MPI_INT, MPI_MAX, world);
  }
  ...
  myScalar old_xprd = domain->xprd;
  myScalar old_yprd = domain->yprd;
  myScalar old_zprd = domain->zprd;
  myScalar old_xy = domain->xy;
  myScalar old_xz = domain->xz;
  myScalar old_yz = domain->yz;
  ...
  if (bbox_flag) {
    ...
    int size_buf_all = 0;
    MY_MPI_Allreduce(&n, &size_buf_all, 1, MPI_INT, MPI_SUM, world);
    ...
    if (me == 0 && screen) {
      ...
      fprintf(screen,"  bounding box extra memory = %.2f MB\n",
              (myScalar)size_buf_all*sizeof(myScalar)/1024/1024);
    }
    ...
    myScalar _orig_lo[3], _orig_hi[3];
    if (triclinic) {
    ...
    myScalar avg = (myScalar) sum / nprocs;
    if (me == 0 && screen)
