...
void Comm::copy_arrays(Comm *oldcomm)
{
  if (oldcomm->grid2proc) {
    ...
    memcpy(xsplit,oldcomm->xsplit,(procgrid[0]+1)*sizeof(myScalar));
    memcpy(ysplit,oldcomm->ysplit,(procgrid[1]+1)*sizeof(myScalar));
    memcpy(zsplit,oldcomm->zsplit,(procgrid[2]+1)*sizeof(myScalar));
  }
...
myScalar Comm::get_comm_cutoff()
{
  myScalar maxcommcutoff, maxbondcutoff = 0.0;
...
int Comm::coord2proc(myScalar *x, int &igx, int &igy, int &igz)
{
...
int Comm::binary(myScalar value, int n, myScalar *vec)
{
  int lo = 0;
  int hi = n-1;
                ...
                void *outbuf, void *ptr, int self)
{
  ...
  int nbytes = n*nper;
  int maxbytes;
  MY_MPI_Allreduce(&nbytes,&maxbytes,1,MPI_INT,MPI_MAX,world);
                     ...
                     int outsize, void *ptr, int statflag)
{
  ...
  bigint irregular1_bytes =ad::passive_value ( irregular->memory_usage() );
                   ...
                   int statflag)
{
  ...
  int overflowall;
  MY_MPI_Allreduce(&overflow,&overflowall,1,MPI_INT,MPI_MAX,world);
                            ...
                            int insize, int outsize, bigint commsize)
{
  ...
  MY_MPI_Allreduce(&size,&size_in_all,1,MPI_LMP_BIGINT,MPI_SUM,world);
  MY_MPI_Allreduce(&size,&size_in_max,1,MPI_LMP_BIGINT,MPI_MAX,world);
  MY_MPI_Allreduce(&size,&size_in_min,1,MPI_LMP_BIGINT,MPI_MIN,world);

  size = (bigint) nout*outsize;
  MY_MPI_Allreduce(&size,&size_out_all,1,MPI_LMP_BIGINT,MPI_SUM,world);
  MY_MPI_Allreduce(&size,&size_out_max,1,MPI_LMP_BIGINT,MPI_MAX,world);
  MY_MPI_Allreduce(&size,&size_out_min,1,MPI_LMP_BIGINT,MPI_MIN,world);

  size = (bigint) nrvous*insize;
  MY_MPI_Allreduce(&size,&size_inrvous_all,1,MPI_LMP_BIGINT,MPI_SUM,world);
  MY_MPI_Allreduce(&size,&size_inrvous_max,1,MPI_LMP_BIGINT,MPI_MAX,world);
  MY_MPI_Allreduce(&size,&size_inrvous_min,1,MPI_LMP_BIGINT,MPI_MIN,world);

  size = (bigint) nrvous_out*insize;
  MY_MPI_Allreduce(&size,&size_outrvous_all,1,MPI_LMP_BIGINT,MPI_SUM,world);
  MY_MPI_Allreduce(&size,&size_outrvous_max,1,MPI_LMP_BIGINT,MPI_MAX,world);
  MY_MPI_Allreduce(&size,&size_outrvous_min,1,MPI_LMP_BIGINT,MPI_MIN,world);

  size = commsize;
  MY_MPI_Allreduce(&size,&size_comm_all,1,MPI_LMP_BIGINT,MPI_SUM,world);
  MY_MPI_Allreduce(&size,&size_comm_max,1,MPI_LMP_BIGINT,MPI_MAX,world);
  MY_MPI_Allreduce(&size,&size_comm_min,1,MPI_LMP_BIGINT,MPI_MIN,world);
