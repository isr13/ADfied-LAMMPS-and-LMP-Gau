...
void FixAdapt::init()
{
  ...
  for (int m = 0; m < nadapt; m++) {
    ...
    if (ad->which == PAIR) {
      ...
      if (ad->pdim == 2) ad->array = (myScalar **) ptr;
      if (ad->pdim == 0) ad->scalar = (myScalar *) ptr;
      ...
      if (ad->bdim == 1) ad->vector = (myScalar *) ptr;
      ...
      kspace_scale = (myScalar *) force->kspace->extract("scale");
...
void FixAdapt::change_settings()
{
  ...
  for (int m = 0; m < nadapt; m++) {
    Adapt *ad = &adapt[m];
    myScalar value = input->variable->compute_equal(ad->ivar);
