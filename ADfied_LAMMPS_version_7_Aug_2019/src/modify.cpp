...
void Modify::init()
{
  ...
  int checkall;
  MY_MPI_Allreduce(&check,&checkall,1,MPI_INT,MPI_SUM,world);
...
void Modify::thermo_energy_atom(int nlocal, myScalar *energy)
{
  int i,j;
  myScalar *eatom;
...
myScalar Modify::min_energy(myScalar *fextra)
{
  int ifix,index;
...
void Modify::min_step(myScalar alpha, myScalar *hextra)
{
  int ifix,index;
...
myScalar Modify::max_alpha(myScalar *hextra)
{
  int ifix,index;
  ...
  for (int i = 0; i < n_min_energy; i++) {
    ifix = list_min_energy[i];
    myScalar alpha_one = fix[ifix]->max_alpha(&hextra[index]);
...
int Modify::check_rigid_group_overlap(int groupbit)
{
  ...
  int n_all = 0;
  MY_MPI_Allreduce(&n,&n_all,1,MPI_INT,MPI_SUM,world);
