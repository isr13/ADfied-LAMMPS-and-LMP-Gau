...
void Velocity::command(int narg, char **arg)
{
  ...
  if (style == CREATE) {
    myScalar t_desired = force->numeric(FLERR,arg[2]);
...
void Velocity::create(myScalar t_desired, int seed)
{
  int i;
  myScalar **vhold;
...
{
  myScalar t_desired = force->numeric(FLERR,arg[0]);
  ...
  if (bias_flag == 0) {
    myScalar t = temperature->compute_scalar();
    rescale(t,t_desired);
  } else {
    myScalar t = temperature->compute_scalar();
    temperature->remove_bias_all();
...
void Velocity::rescale(myScalar t_old, myScalar t_new)
{
  if (t_old == 0.0) error->all(FLERR,"Attempting to rescale a 0.0 temperature");

  myScalar factor = sqrt(t_new/t_old);
