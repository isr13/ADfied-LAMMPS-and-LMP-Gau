...
class Lattice : protected Pointers {
 public:
  ...
  myScalar xlattice,ylattice,zlattice;   // lattice scale factors in 3 dims
  myScalar a1[3],a2[3],a3[3];            // edge vectors of unit cell
  int nbasis;                          // # of basis atoms in unit cell
  myScalar **basis;                      // fractional coords of each basis atom
                                       // within unit cell (0 <= coord < 1)

  Lattice(class LAMMPS *, int, char **);
  ~Lattice();
  void lattice2box(myScalar &, myScalar &, myScalar &);
  void box2lattice(myScalar &, myScalar &, myScalar &);
  void bbox(int, myScalar, myScalar, myScalar,
            myScalar &, myScalar &, myScalar &, myScalar &, myScalar &, myScalar &);

private:
  myScalar scale;
  myScalar origin[3];                    // lattice origin
  int orientx[3];                      // lattice orientation vecs
  int orienty[3];                      // orientx = what lattice dir lies
  int orientz[3];                      //           along x dim in box

  myScalar primitive[3][3];              // lattice <-> box transform matrices
  myScalar priminv[3][3];
  myScalar rotaterow[3][3];
  myScalar rotatecol[3][3];

  int orthogonal();
  int right_handed();
  int collinear();
  void setup_transform();
  void add_basis(myScalar, myScalar, myScalar);
  myScalar dot(myScalar *, myScalar *);
  void cross(myScalar *, myScalar *, myScalar *);
};
