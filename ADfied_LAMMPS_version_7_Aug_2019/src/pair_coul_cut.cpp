...
void PairCoulCut::compute(int eflag, int vflag)
{
  ...
  myScalar rsq,r2inv,rinv,forcecoul,factor_coul;
...
myScalar PairCoulCut::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
void PairCoulCut::write_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) fwrite(&cut[i][j],sizeof(myScalar),1,fp);
    }
}
...
void PairCoulCut::read_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) fread(&cut[i][j],sizeof(myScalar),1,fp);
...
myScalar PairCoulCut::single(int i, int j, int /*itype*/, int /*jtype*/,
                           myScalar rsq, myScalar factor_coul, myScalar /*factor_lj*/,
                           myScalar &fforce)
{
  myScalar r2inv,rinv,forcecoul,phicoul;
