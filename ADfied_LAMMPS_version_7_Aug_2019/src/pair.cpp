...
void Pair::init_tables(myScalar cut_coul, myScalar *cut_respa)
{
  int masklo,maskhi;
  myScalar r,grij,expm2,derfc,egamma,fgamma,rsw;
  myScalar qqrd2e = force->qqrd2e;

  if (force->kspace == NULL)
    error->all(FLERR,"Pair style requires a KSpace style");
  myScalar g_ewald = force->kspace->g_ewald;

  myScalar cut_coulsq = cut_coul * cut_coul;
  ...
  myScalar f_tmp,c_tmp,e_tmp,p_tmp,v_tmp;
  p_tmp = 0.0;
  v_tmp = 0.0;
  ...
  if (rsq_lookup.f < cut_coulsq) {
    rsq_lookup.f = static_cast<long>(cut_coulsq);
    r = sqrtf(rsq_lookup.f);
    if (msmflag) {
      egamma = 1.0 - (r/cut_coul)*force->kspace->gamma(r/cut_coul);
      fgamma = 1.0 + (rsq_lookup.f/cut_coulsq)*
...
void Pair::init_tables_disp(myScalar cut_lj_global)
{
  int masklo,maskhi;
  myScalar rsq;
  myScalar g_ewald_6 = force->kspace->g_ewald_6;
  myScalar g2 = g_ewald_6*g_ewald_6, g6 = g2*g2*g2, g8 = g6*g2;
  ...
  myScalar f_tmp,e_tmp;
  myScalar cut_lj_globalsq;
  ...
  if (rsq_lookup.f < (cut_lj_globalsq = cut_lj_global * cut_lj_global)) {
    rsq_lookup.f = static_cast<long>(cut_lj_globalsq);
...
myScalar Pair::mix_energy(myScalar eps1, myScalar eps2, myScalar sig1, myScalar sig2)
{
...
myScalar Pair::mix_distance(myScalar sig1, myScalar sig2)
{
  if (mix_flag == GEOMETRIC)
...
void Pair::ev_tally(int i, int j, int nlocal, int newton_pair,
                    myScalar evdwl, myScalar ecoul, myScalar fpair,
                    myScalar delx, myScalar dely, myScalar delz)
{
  myScalar evdwlhalf,ecoulhalf,epairhalf,v[6];
...
void Pair::ev_tally_full(int i, myScalar evdwl, myScalar ecoul, myScalar fpair,
                         myScalar delx, myScalar dely, myScalar delz)
{
  myScalar v[6];
...
void Pair::ev_tally_xyz(int i, int j, int nlocal, int newton_pair,
                        myScalar evdwl, myScalar ecoul,
                        myScalar fx, myScalar fy, myScalar fz,
                        myScalar delx, myScalar dely, myScalar delz)
{
  myScalar evdwlhalf,ecoulhalf,epairhalf,v[6];
...
void Pair::ev_tally_xyz_full(int i, myScalar evdwl, myScalar ecoul,
                             myScalar fx, myScalar fy, myScalar fz,
                             myScalar delx, myScalar dely, myScalar delz)
{
  myScalar evdwlhalf,ecoulhalf,epairhalf,v[6];
...
void Pair::ev_tally3(int i, int j, int k, myScalar evdwl, myScalar ecoul,
                     myScalar *fj, myScalar *fk, myScalar *drji, myScalar *drki)
{
  myScalar epairthird,v[6];
...
void Pair::ev_tally4(int i, int j, int k, int m, myScalar evdwl,
                     myScalar *fi, myScalar *fj, myScalar *fk,
                     myScalar *drim, myScalar *drjm, myScalar *drkm)
{
  myScalar epairfourth,v[6];
...
void Pair::ev_tally_tip4p(int key, int *list, myScalar *v,
                          myScalar ecoul, myScalar alpha)
{
  int i;
...
void Pair::v_tally(int i, myScalar *fi, myScalar *deli)
{
  myScalar v[6];
...
void Pair::v_tally2(int i, int j, myScalar fpair, myScalar *drij)
{
  myScalar v[6];
...
void Pair::v_tally3(int i, int j, int k,
                    myScalar *fi, myScalar *fj, myScalar *drik, myScalar *drjk)
{
  myScalar v[6];
...
void Pair::v_tally_tensor(int i, int j, int nlocal, int newton_pair,
                          myScalar vxx, myScalar vyy, myScalar vzz,
                          myScalar vxy, myScalar vxz, myScalar vyz)
{
  myScalar v[6];
...
void Pair::write_file(int narg, char **arg)
{
  ...
  myScalar inner = force->numeric(FLERR,arg[4]);
  myScalar outer = force->numeric(FLERR,arg[5]);
...
void Pair::init_bitmap(myScalar inner, myScalar outer, int ntablebits,
             int &masklo, int &maskhi, int &nmask, int &nshiftbits)
{
  ...
  int nlowermin = 1;
  while (!((pow(myScalar(2),(myScalar)nlowermin) <= inner*inner) &&
           (pow(myScalar(2),(myScalar)nlowermin+1.0) > inner*inner))) {
    if (pow(myScalar(2),(myScalar)nlowermin) <= inner*inner) nlowermin++;
    else nlowermin--;
  }

  int nexpbits = 0;
  myScalar required_range = outer*outer / pow(myScalar(2),(myScalar)nlowermin);
  myScalar available_range = 2.0;

  while (available_range < required_range) {
    nexpbits++;
    available_range = pow(myScalar(2),pow(myScalar(2),(myScalar)nexpbits));
  }
  ...
  union_int_float_t rsq_lookup;
  rsq_lookup.f = static_cast<long>(outer*outer);
  maskhi = rsq_lookup.i & ~(nmask);
  rsq_lookup.f = static_cast<long>(inner*inner);
  masklo = rsq_lookup.i & ~(nmask);
}

/* ---------------------------------------------------------------------- */

myScalar Pair::memory_usage()
{
  myScalar bytes = comm->nthreads*maxeatom * sizeof(myScalar);
  bytes += comm->nthreads*maxvatom*6 * sizeof(myScalar);
  return bytes;
}
