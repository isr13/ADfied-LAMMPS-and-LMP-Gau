...
void DumpDCD::write_header(bigint n)
{
  ...
  if (domain->triclinic) {
    myScalar *h = domain->h;
    myScalar alen = h[0];
    myScalar blen = sqrt(h[5]*h[5] + h[1]*h[1]);
    myScalar clen = sqrt(h[4]*h[4] + h[3]*h[3] + h[2]*h[2]);
    dim[0] = alen;
    dim[2] = blen;
    dim[5] = clen;
...
void DumpDCD::pack(tagint *ids)
{
  ...
  if (unwrap_flag) {
    ...
    myScalar xy = domain->xy;
    myScalar xz = domain->xz;
    myScalar yz = domain->yz;
...
void DumpDCD::write_data(int n, myScalar *mybuf)
{
  // copy buf atom coords into 3 global arrays

  int m = 0;
  for (int i = 0; i < n; i++) {
    xf[ntotal] = ad::passive_value(mybuf[m++]);
    yf[ntotal] = ad::passive_value(mybuf[m++]);
    zf[ntotal] = ad::passive_value(mybuf[m++]);
    ntotal++;
  }
...
void DumpDCD::write_dcd_header(const char *remarks)
{
  ...
  fwrite_int32(fp,0);
  fwrite_int32(fp,0);
  out_float = ad::passive_value(update->dt);
