...
void Image::view_params(myScalar boxxlo, myScalar boxxhi, myScalar boxylo,
                        myScalar boxyhi, myScalar boxzlo, myScalar boxzhi)
{
  ...
  myScalar dot = MathExtra::dot3(up,camDir);
  ...
  myScalar delx = 2.0*(boxxhi-boxxlo);
  myScalar dely = 2.0*(boxyhi-boxylo);
  myScalar delz = 2.0*(boxzhi-boxzlo);
  myScalar maxdel = MAX(delx,dely);
  ...
  tanPerPixel = -(maxdel / (myScalar) height);
}
...
void Image::draw_box(myScalar (*corners)[3], myScalar diameter)
{
...
void Image::draw_axes(myScalar (*axes)[3], myScalar diameter)
{
...
void Image::draw_sphere(myScalar *x, myScalar *surfaceColor, myScalar diameter)
{
  int ix,iy;
  myScalar projRad;
  myScalar xlocal[3],surface[3];
  myScalar depth;
  ...
  myScalar xmap = MathExtra::dot3(camRight,xlocal);
  myScalar ymap = MathExtra::dot3(camUp,xlocal);
  myScalar dist = MathExtra::dot3(camPos,camDir) - MathExtra::dot3(xlocal,camDir);

  myScalar radius = 0.5*diameter;
  myScalar radsq = radius*radius;
  myScalar pixelWdith_1 = tanPerPixel * dist;
  myScalar pixelWdith_2 = -tanPerPixel / zoom;
  myScalar pixelWidth = (tanPerPixel > 0) ?  pixelWdith_1: pixelWdith_2;
  myScalar pixelRadiusFull = radius / pixelWidth;
  int pixelRadius = static_cast<int> (pixelRadiusFull + 0.5) + 1;

  myScalar xf = xmap / pixelWidth;
  myScalar yf = ymap / pixelWidth;
  int xc = static_cast<int> (xf);
  int yc = static_cast<int> (yf);
  myScalar width_error = xf - xc;
  myScalar height_error = yf - yc;
...
void Image::draw_cube(myScalar *x, myScalar *surfaceColor, myScalar diameter)
{
  myScalar xlocal[3],surface[3],normal[3];
  myScalar t,tdir[3];
  myScalar depth;
  ...
  myScalar halfWidth = diameter;
  myScalar pixelHalfWidthFull = halfWidth / pixelWidth;
  ...
  for (int iy = yc - pixelHalfWidth; iy <= yc + pixelHalfWidth; iy ++) {
    for (int ix = xc - pixelHalfWidth; ix <= xc + pixelHalfWidth; ix ++) {
      ...
      myScalar sy = ((iy - yc) - height_error) * pixelWidth;
      myScalar sx = ((ix - xc) - width_error) * pixelWidth;
...
void Image::draw_cylinder(myScalar *x, myScalar *y,
                          myScalar *surfaceColor, myScalar diameter, int sflag)
{
  myScalar surface[3], normal[3];
  myScalar mid[3],xaxis[3],yaxis[3],zaxis[3];
  myScalar camLDir[3], camLRight[3], camLUp[3];
  myScalar zmin, zmax;
  ...
  myScalar rasterWidth = fabs(MathExtra::dot3(zaxis, camRight)) + diameter;
  myScalar rasterHeight = fabs(MathExtra::dot3(zaxis, camUp)) + diameter;

  mid[0] = (y[0] + x[0]) * 0.5 - xctr;
  mid[1] = (y[1] + x[1]) * 0.5 - yctr;
  mid[2] = (y[2] + x[2]) * 0.5 - zctr;

  myScalar len = MathExtra::len3(zaxis);
  MathExtra::scale3(1.0/len,zaxis);
  len *= 0.5;
  zmax = len;
  zmin = -len;

  myScalar xmap = MathExtra::dot3(camRight,mid);
  myScalar ymap = MathExtra::dot3(camUp,mid);
  myScalar dist = MathExtra::dot3(camPos,camDir) - MathExtra::dot3(mid,camDir);
  ...
  myScalar pixelHalfWidthFull = (rasterWidth * 0.5) / pixelWidth;
  myScalar pixelHalfHeightFull = (rasterHeight * 0.5) / pixelWidth;
  int pixelHalfWidth = static_cast<int> (pixelHalfWidthFull + 0.5);
  ...
  myScalar a = camLDir[0] * camLDir[0];
  ...
  for (int iy = yc - pixelHalfHeight; iy <= yc + pixelHalfHeight; iy ++) {
    for (int ix = xc - pixelHalfWidth; ix <= xc + pixelHalfWidth; ix ++) {
      ...
      myScalar b = 2 * camLDir[0] * surface[0];
      myScalar c = surface[0] * surface[0] + surface[1] * surface[1] - radsq;

      myScalar partial = b*b - 4*a*c;
      if (partial < 0) continue;
      partial = sqrt (partial);

      myScalar t = (-b + partial) / (2*a);
      myScalar t2 = (-b - partial) / (2*a);
      if (t2 > t) { t = t2; }
...
void Image::draw_triangle(myScalar *x, myScalar *y, myScalar *z, myScalar *surfaceColor)
{
  myScalar d1[3], d1len, d2[3], d2len, normal[3], invndotd;
  myScalar xlocal[3], ylocal[3], zlocal[3];
  myScalar surface[3];
  myScalar depth;
  ...
  myScalar rasterLeft = r[0] - MIN(r[0],MIN(r[1],r[2]));
  myScalar rasterRight = MAX(r[0],MAX(r[1],r[2])) - r[0];
  myScalar rasterDown = u[0] - MIN(u[0],MIN(u[1],u[2]));
  myScalar rasterUp = MAX(u[0],MAX(u[1],u[2])) - u[0];
  ...
  myScalar pixelLeftFull = rasterLeft / pixelWidth;
  myScalar pixelRightFull = rasterRight / pixelWidth;
  myScalar pixelDownFull = rasterDown / pixelWidth;
  myScalar pixelUpFull = rasterUp / pixelWidth;
  ...
  for (int iy = yc - pixelDown; iy <= yc + pixelUp; iy ++) {
    for (int ix = xc - pixelLeft; ix <= xc + pixelRight; ix ++) {
      ...
      myScalar t = -MathExtra::dot3(normal,surface) * invndotd;
...
void Image::draw_pixel(int ix, int iy, myScalar depth,
                           myScalar *surface, myScalar *surfaceColor)
{
  myScalar diffuseKey,diffuseFill,diffuseBack,specularKey;
...
void Image::compute_SSAO()
{
  ...
  myScalar delTheta = 2.0*MY_PI / SSAOSamples;

  // typical neighborhood value for shading

  myScalar pixelWdith_1 = tanPerPixel;
  myScalar pixelWdith_2 = -tanPerPixel / zoom;
  myScalar pixelWidth = (tanPerPixel > 0) ? pixelWdith_1 : pixelWdith_2;

  //int pixelRadius = (int) trunc (SSAORadius / pixelWidth + 0.5);
  int pixelRadius = static_cast<int>(SSAORadius / pixelWidth + 0.5);
  ...
  for (y = me * hPart; y < (me + 1) * hPart; y ++) {
    for (x = 0; x < width; x ++, index ++) {
      myScalar cdepth = depthBuffer[index];
      if (cdepth < 0) { continue; }

      myScalar sx = surfaceBuffer[index * 2 + 0];
      myScalar sy = surfaceBuffer[index * 2 + 1];
      myScalar sin_t = -sqrt(sx*sx + sy*sy);

      myScalar mytheta = random->uniform() * SSAOJitter;
      myScalar ao = 0.0;

      for (s = 0; s < SSAOSamples; s ++) {
        myScalar hx = cos(mytheta);
        myScalar hy = sin(mytheta);
        mytheta += delTheta;

        // multiply by z cross surface tangent
        // so that dot (aka cos) works here

        myScalar scaled_sin_t = sin_t * (hx*sy + hy*sx);
        ...
        if (peakLen > 0) {
          myScalar h = atan ((cdepth - minPeak) / peakLen);
          ao += saturate(sin (h) - scaled_sin_t);
        } else {
          ao += saturate(-scaled_sin_t);
        }
      }
      ao /= (myScalar)SSAOSamples;

      myScalar c[3];
      c[0] = (myScalar) (*(unsigned char *) &imageBuffer[index * 3 + 0]);
      c[1] = (myScalar) (*(unsigned char *) &imageBuffer[index * 3 + 1]);
      c[2] = (myScalar) (*(unsigned char *) &imageBuffer[index * 3 + 2]);
      c[0] *= (1.0 - ao);
      c[1] *= (1.0 - ao);
      c[2] *= (1.0 - ao);
...
int Image::map_minmax(int index, myScalar mindynamic, myScalar maxdynamic)
{
...
myScalar *Image::map_value2color(int index, myScalar value)
{
...
int Image::addcolor(char *name, myScalar r, myScalar g, myScalar b)
{
  int icolor;
...
myScalar *Image::color2rgb(const char *color, int index)
{
...
myScalar *Image::element2color(char *element)
{
  ...
  static myScalar rgb[NELEMENTS][3] = {
    {0.8, 0.8, 0.8},
...
myScalar Image::element2diam(char *element)
{
  static const char *name[NELEMENTS] = {
    "H", "He", "Li", "Be", "B", "C", "N", "O", "F", "Ne",
    "Na", "Mg", "Al", "Si", "P", "S", "Cl", "Ar", "K", "Ca",
  ...
  static myScalar diameter[NELEMENTS] = {
...
int ColorMap::minmax(myScalar mindynamic, myScalar maxdynamic)
{
...
myScalar *ColorMap::value2color(myScalar value)
{
  myScalar lo;//,hi;
  ...
  if (mstyle == CONTINUOUS) {
    for (int i = 0; i < nentry-1; i++)
      if (value >= mentry[i].svalue && value <= mentry[i+1].svalue) {
        myScalar fraction = (value-mentry[i].svalue) /
