  ...
  for (int m = 0; m < nvalues; m++) {
    if (which[m] == KEYWORD && kflag) (this->*pack_choice[m])(m);
      ...
      if (which[m] == COMPUTE) {
        ...
        if (j == 0) {
          myScalar *compute_vector = compute->vector_atom;
          for (i = 0; i < nlocal; i++)
            if (mask[i] & groupbit) values[i][m] = compute_vector[i];
        } else {
          int jm1 = j - 1;
          myScalar **compute_array = compute->array_atom;
          for (i = 0; i < nlocal; i++)
      ...
      // access fix fields, guaranteed to be ready
        ...
        if (j == 0) {
          myScalar *fix_vector = modify->fix[n]->vector_atom;
          for (i = 0; i < nlocal; i++)
            if (mask[i] & groupbit) values[i][m] = fix_vector[i];
        } else {
          int jm1 = j - 1;
          myScalar **fix_array = modify->fix[n]->array_atom;
          for (i = 0; i < nlocal; i++)
      ...
      // access custom atom property fields
        ...
        myScalar *dvector = atom->dvector[n];
        for (i = 0; i < nlocal; i++)
...
myScalar FixStoreState::memory_usage()
{
  myScalar bytes = atom->nmax*nvalues * sizeof(myScalar);
  return bytes;
}
...
int FixStoreState::pack_exchange(int i, myScalar *buf)
{
...
int FixStoreState::unpack_exchange(int nlocal, myScalar *buf)
{
...
int FixStoreState::pack_restart(int i, myScalar *buf)
{
  buf[0] = nvalues+1;
