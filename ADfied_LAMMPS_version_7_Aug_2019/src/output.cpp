...
Output::Output(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  restart_flag = restart_flag_single = restart_flag_myScalar = 0;
  restart_every_single = restart_every_myScalar = 0;
  last_restart = -1;
  restart1 = restart2a = restart2b = NULL;
  var_restart_single = var_restart_myScalar = NULL;
  restart = NULL;
...
void Output::init()
{
  ...
  if (restart_flag_myScalar && restart_every_myScalar == 0) {
    ivar_restart_myScalar = input->variable->find(var_restart_myScalar);
    if (ivar_restart_myScalar < 0)
      error->all(FLERR,"Variable name for restart does not exist");
    if (!input->variable->equalstyle(ivar_restart_myScalar))
      error->all(FLERR,"Variable for restart is invalid style");
  }
}
...
void Output::setup(int memflag)
{
  ...
  if (restart_flag && update->restrict_output == 0) {
    ...
    if (restart_flag_myScalar) {
      if (restart_every_myScalar)
        next_restart_myScalar =
          (ntimestep/restart_every_myScalar)*restart_every_myScalar +
          restart_every_myScalar;
      else {
        bigint nextrestart = static_cast<bigint>
          (input->variable->compute_equal(ivar_restart_myScalar));
        if (nextrestart <= ntimestep)
          error->all(FLERR,"Restart variable returned a bad timestep");
        next_restart_myScalar = nextrestart;
      }
    } else next_restart_myScalar = update->laststep + 1;
    next_restart = MIN(next_restart_single,next_restart_myScalar);
...
void Output::write(bigint ntimestep)
{
  ...
  if (next_restart == ntimestep) {
    ...
    if (next_restart_myScalar == ntimestep) {
      ...
      if (restart_every_myScalar) next_restart_myScalar += restart_every_myScalar;
      else {
        modify->clearstep_compute();
        bigint nextrestart = static_cast<bigint>
          (input->variable->compute_equal(ivar_restart_myScalar));
        if (nextrestart <= ntimestep)
          error->all(FLERR,"Restart variable returned a bad timestep");
        next_restart_myScalar = nextrestart;
        modify->addstep_compute(next_restart_myScalar);
      }
    }
  ...
  if (restart_flag_myScalar) {
    if (restart_every_myScalar) {
      next_restart_myScalar =
        (ntimestep/restart_every_myScalar)*restart_every_myScalar;
      if (next_restart_myScalar < ntimestep)
        next_restart_myScalar += restart_every_myScalar;
    } else {
      modify->clearstep_compute();
      update->ntimestep--;
      bigint nextrestart = static_cast<bigint>
...
/* ----------------------------------------------------------------------
   setup restart capability for single or myScalar output files
...
void Output::create_restart(int narg, char **arg)
{
  ...
  if (nfile == 2) {
    restart_flag = restart_flag_myScalar = 1;

    if (varflag) {
      delete [] var_restart_myScalar;
      int n = strlen(&arg[0][2]) + 1;
      var_restart_myScalar = new char[n];
      strcpy(var_restart_myScalar,&arg[0][2]);
      restart_every_myScalar = 0;
    } else restart_every_myScalar = every;
...
void Output::memory_usage()
{
  ...
  myScalar mbytes = bytes/1024.0/1024.0;
  myScalar mbavg,mbmin,mbmax;
