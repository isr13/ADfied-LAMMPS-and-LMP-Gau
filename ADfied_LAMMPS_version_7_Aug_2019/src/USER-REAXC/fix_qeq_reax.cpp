...
void FixQEqReax::pertype_parameters(char *arg)
{
  if (strcmp(arg,"reax/c") == 0) {
    ...
    int tmp;
    chi = (myScalar *) pair->extract("chi",tmp);
    eta = (myScalar *) pair->extract("eta",tmp);
    gamma = (myScalar *) pair->extract("gamma",tmp);
...
{
  myScalar d7, swa2, swa3, swb2, swb3;
...
void FixQEqReax::compute_H()
{
  ...
  int i, j, ii, jj, flag;
  myScalar dx, dy, dz, r_sqr;
  const myScalar SMALL = 0.0001;
...
myScalar FixQEqReax::calculate_H( myScalar r, myScalar gamma)
{
  myScalar Taper, denom;
...
int FixQEqReax::CG( myScalar *b, myScalar *x)
{
  int  i, j, imax;
  myScalar tmp, alpha, beta, b_norm;
  myScalar sig_old, sig_new;
...
void FixQEqReax::sparse_matvec( sparse_matrix *A, myScalar *x, myScalar *b)
{
  int i, j, itr_j;
  int nn, NN, ii;
  int *ilist;
...
int FixQEqReax::pack_forward_comm(int n, int *list, myScalar *buf,
...
void FixQEqReax::unpack_forward_comm(int n, int first, myScalar *buf)
{
  int i, m;
...
int FixQEqReax::pack_reverse_comm(int n, int first, myScalar *buf)
{
  int i, m;
  if (pack_flag == 5) {
    m = 0;
...
void FixQEqReax::unpack_reverse_comm(int n, int *list, myScalar *buf)
{
  if (pack_flag == 5) {
    int m = 0;
    for(int i = 0; i < n; i++) {
...
myScalar FixQEqReax::memory_usage()
{
  myScalar bytes;

  bytes = atom->nmax*nprev*2 * sizeof(myScalar); // s_hist & t_hist
  bytes += atom->nmax*11 * sizeof(myScalar); // storage
  bytes += n_cap*2 * sizeof(int); // matrix...
  bytes += m_cap * sizeof(int);
  bytes += m_cap * sizeof(myScalar);

  if (dual_enabled)
    bytes += atom->nmax*4 * sizeof(myScalar); // myScalar size for q, d, r, and p
...
int FixQEqReax::pack_exchange(int i, myScalar *buf)
{
...
int FixQEqReax::unpack_exchange(int nlocal, myScalar *buf)
{
...
myScalar FixQEqReax::parallel_norm( myScalar *v, int n)
{
  int  i;
  myScalar my_sum, norm_sqr;
  ...
  MY_MPI_Allreduce( &my_sum, &norm_sqr, 1, MPI_DOUBLE, MPI_SUM, world);
...
myScalar FixQEqReax::parallel_dot( myScalar *v1, myScalar *v2, int n)
{
  int  i;
  myScalar my_dot, res;
  ...
  MY_MPI_Allreduce( &my_dot, &res, 1, MPI_DOUBLE, MPI_SUM, world);
...
myScalar FixQEqReax::parallel_vector_acc( myScalar *v, int n)
{
  int  i;
  myScalar my_acc, res;
  ...
  MY_MPI_Allreduce( &my_acc, &res, 1, MPI_DOUBLE, MPI_SUM, world);
...
void FixQEqReax::vector_sum( myScalar* dest, myScalar c, myScalar* v,
                                myScalar d, myScalar* y, int k)
{
  int kk;
  int *ilist;
...
void FixQEqReax::vector_add( myScalar* dest, myScalar c, myScalar* v, int k)
{
  int kk;
  int *ilist;
