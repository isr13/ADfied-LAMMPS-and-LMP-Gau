                       ...
                       storage * workspace )
{
  ...
  myScalar safezone = system->safezone;
                        ...
                        char * /*msg*/ )
{
  ...
  total_real = total_cap * sizeof(myScalar);
  total_rvec = total_cap * sizeof(rvec);
  local_rvec = local_cap * sizeof(rvec);

  /* communication storage */
  for( i = 0; i < MAX_NBRS; ++i ) {
    workspace->tmp_dbl[i] = (myScalar*)
      scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "tmp_dbl");
  ...
  workspace->total_bond_order = (myScalar*) smalloc(control->error_ptr,  total_real, "total_bo");
  workspace->Deltap = (myScalar*) smalloc(control->error_ptr,  total_real, "Deltap");
  workspace->Deltap_boc = (myScalar*) smalloc(control->error_ptr,  total_real, "Deltap_boc");
  workspace->dDeltap_self = (rvec*) smalloc(control->error_ptr,  total_rvec, "dDeltap_self");
  workspace->Delta = (myScalar*) smalloc(control->error_ptr,  total_real, "Delta");
  workspace->Delta_lp = (myScalar*) smalloc(control->error_ptr,  total_real, "Delta_lp");
  workspace->Delta_lp_temp = (myScalar*)
    smalloc(control->error_ptr,  total_real, "Delta_lp_temp");
  workspace->dDelta_lp = (myScalar*) smalloc(control->error_ptr,  total_real, "dDelta_lp");
  workspace->dDelta_lp_temp = (myScalar*)
    smalloc(control->error_ptr,  total_real, "dDelta_lp_temp");
  workspace->Delta_e = (myScalar*) smalloc(control->error_ptr,  total_real, "Delta_e");
  workspace->Delta_boc = (myScalar*) smalloc(control->error_ptr,  total_real, "Delta_boc");
  workspace->Delta_val = (myScalar*) smalloc(control->error_ptr,  total_real, "Delta_val");
  workspace->nlp = (myScalar*) smalloc(control->error_ptr,  total_real, "nlp");
  workspace->nlp_temp = (myScalar*) smalloc(control->error_ptr,  total_real, "nlp_temp");
  workspace->Clp = (myScalar*) smalloc(control->error_ptr,  total_real, "Clp");
  workspace->vlpex = (myScalar*) smalloc(control->error_ptr,  total_real, "vlpex");
  ...
  /* QEq storage */
  workspace->Hdia_inv = (myScalar*)
    scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "Hdia_inv");
  workspace->b_s = (myScalar*) scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "b_s");
  workspace->b_t = (myScalar*) scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "b_t");
  workspace->b_prc = (myScalar*) scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "b_prc");
  workspace->b_prm = (myScalar*) scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "b_prm");
  workspace->s = (myScalar*) scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "s");
  workspace->t = (myScalar*) scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "t");
  workspace->droptol = (myScalar*)
    scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "droptol");
  ...
  /* GMRES storage */
  workspace->y = (myScalar*) scalloc(control->error_ptr,  RESTART+1, sizeof(myScalar), "y");
  workspace->z = (myScalar*) scalloc(control->error_ptr,  RESTART+1, sizeof(myScalar), "z");
  workspace->g = (myScalar*) scalloc(control->error_ptr,  RESTART+1, sizeof(myScalar), "g");
  workspace->h = (myScalar**) scalloc(control->error_ptr,  RESTART+1, sizeof(myScalar*), "h");
  workspace->hs = (myScalar*) scalloc(control->error_ptr,  RESTART+1, sizeof(myScalar), "hs");
  workspace->hc = (myScalar*) scalloc(control->error_ptr,  RESTART+1, sizeof(myScalar), "hc");
  workspace->v = (myScalar**) scalloc(control->error_ptr,  RESTART+1, sizeof(myScalar*), "v");

  for( i = 0; i < RESTART+1; ++i ) {
    workspace->h[i] = (myScalar*) scalloc(control->error_ptr,  RESTART+1, sizeof(myScalar), "h[i]");
    workspace->v[i] = (myScalar*) scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "v[i]");
  }

  /* CG storage */
  workspace->r = (myScalar*) scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "r");
  workspace->d = (myScalar*) scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "d");
  workspace->q = (myScalar*) scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "q");
  workspace->p = (myScalar*) scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "p");
  ...
  workspace->CdDelta = (myScalar*)
    scalloc(control->error_ptr,  total_cap, sizeof(myScalar), "CdDelta");

  // storage for reductions with multiple threads
#ifdef LMP_USER_OMP
  workspace->CdDeltaReduction = (myScalar *) scalloc(control->error_ptr, sizeof(myScalar), total_cap*control->nthreads,
                                                 "cddelta_reduce");
...
static int Reallocate_HBonds_List( reax_system *system, reax_list *hbonds )
{
  ...
  int mincap = system->mincap;
  myScalar saferzone = system->saferzone;
  ...
  total_hbonds = (int)(MAX( total_hbonds*ad::passive_value(saferzone), mincap*MIN_HBONDS ));
                                  ...
                                  int *total_bonds, int *est_3body )
{
  ...
  *total_bonds = (int)(MAX( *total_bonds * ad::passive_value(safezone), mincap*MIN_BONDS ));
  ...
  if (system->omp_active)
    for (i = 0; i < bonds->num_intrs; ++i)
      bonds->select.bond_list[i].bo_data.CdboReduction =
        (myScalar*) smalloc(system->error_ptr, sizeof(myScalar)*nthreads, "CdboReduction");
#endif
                 ...
                 simulation_data *data, storage *workspace, reax_list **lists )
{
  ...
  if (renbr) {
    ...
    if (Nflag || realloc->num_far >= far_nbrs->num_intrs * DANGER_ZONE) {
      ...
      newsize = static_cast<int>
        (MAX( realloc->num_far*ad::passive_value(safezone), mincap*MIN_NBRS ));
  ...
  if (control->hbond_cut > 0) {
    ...
    if( system->numH >= DANGER_ZONE * system->Hcap ||
      ...
      Hflag = 1;
      system->Hcap = int(MAX( system->numH * ad::passive_value(saferzone), mincap ));
    }
  ...
  if (realloc->num_3body > 0) {
    ...
    realloc->num_3body = (int)(MAX(realloc->num_3body*ad::passive_value(safezone), MIN_3BODIES));
