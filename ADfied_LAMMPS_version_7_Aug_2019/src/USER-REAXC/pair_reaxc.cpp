...
void PairReaxC::allocate( )
{
  ...
  chi = new myScalar[n+1];
  eta = new myScalar[n+1];
  gamma = new myScalar[n+1];
}
...
void PairReaxC::setup( )
{
  ...
  MY_MPI_Allreduce( &local_ngroup, &ngroup, 1, MPI_LMP_BIGINT, MPI_SUM, world );
}

/* ---------------------------------------------------------------------- */

myScalar PairReaxC::init_one(int i, int j)
{
...
void PairReaxC::get_distance( rvec xj, rvec xi, myScalar *d_sqr, rvec *dvec )
{
  (*dvec)[0] = xj[0] - xi[0];
  (*dvec)[1] = xj[1] - xi[1];
  (*dvec)[2] = xj[2] - xi[2];
...
int PairReaxC::estimate_reax_lists()
{
  ...
  return static_cast<int> (MAX( num_nbrs*ad::passive_value(safezone), mincap*MIN_NBRS ));
}
...
int PairReaxC::write_reax_lists()
{
  ...
  num_nbrs = 0;
  int inum = list->inum;
  dist = (myScalar*) calloc( system->N, sizeof(myScalar) );
...
myScalar PairReaxC::memory_usage()
{
  myScalar bytes = 0.0;

  // From pair_reax_c
  bytes += 1.0 * system->N * sizeof(int);
  bytes += 1.0 * system->N * sizeof(myScalar);

  // From reaxc_allocate: BO
  bytes += 1.0 * system->total_cap * sizeof(reax_atom);
  bytes += 19.0 * system->total_cap * sizeof(myScalar);
  ...
  if(fixspecies_flag)
    bytes += 2 * nmax * MAXSPECBOND * sizeof(myScalar);
