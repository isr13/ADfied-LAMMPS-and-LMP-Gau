  ...
  Region(lmp, narg, arg)
{
  ...
  myScalar rsq = normal[0]*normal[0] + normal[1]*normal[1] + normal[2]*normal[2];
...
int RegPlane::inside(myScalar x, myScalar y, myScalar z)
{
  myScalar dot = (x-xp)*normal[0] + (y-yp)*normal[1] + (z-zp)*normal[2];
...
int RegPlane::surface_interior(myScalar *x, myScalar cutoff)
{
  myScalar dot = (x[0]-xp)*normal[0] + (x[1]-yp)*normal[1] + (x[2]-zp)*normal[2];
...
int RegPlane::surface_exterior(myScalar *x, myScalar cutoff)
{
  myScalar dot = (x[0]-xp)*normal[0] + (x[1]-yp)*normal[1] + (x[2]-zp)*normal[2];
  dot = -dot;
  if (dot < cutoff && dot >= 0.0) {
