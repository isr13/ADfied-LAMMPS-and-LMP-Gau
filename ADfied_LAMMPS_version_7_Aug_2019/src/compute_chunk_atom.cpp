...
void ComputeChunkAtom::init()
{
  ...
  if (which == MOLECULE) {
    ...
    tagint maxall;
    MY_MPI_Allreduce(&maxone,&maxall,1,MPI_LMP_TAGINT,MPI_MAX,world);
...
int ComputeChunkAtom::setup_chunks()
{
  ...
  if (which == TYPE) nchunk = atom->ntypes;
    ...
    MY_MPI_Allreduce(&hi,&nchunk,1,MPI_INT,MPI_MAX,world);
...
void ComputeChunkAtom::assign_chunk_ids()
{
  ...
  if (binflag) {
    ...
    if (argindex == 0) {
      myScalar *vec = cchunk->vector_atom;
      for (i = 0; i < nlocal; i++) {
        if (exclude[i]) continue;
        ichunk[i] = static_cast<int> (vec[i]);
      }
    } else {
      myScalar **array = cchunk->array_atom;
    ...
    if (argindex == 0) {
      myScalar *vec = fchunk->vector_atom;
      for (i = 0; i < nlocal; i++) {
        if (exclude[i]) continue;
        ichunk[i] = static_cast<int> (vec[i]);
      }
    } else {
      myScalar **array = fchunk->array_atom;
      int argm1 = argindex - 1;
      for (i = 0; i < nlocal; i++) {
        if (exclude[i]) continue;
        ichunk[i] = static_cast<int> (array[i][argm1]);
...
void ComputeChunkAtom::compress_chunk_ids()
{
  ...
  bigint nbone = n;
  bigint nball;
  MY_MPI_Allreduce(&nbone,&nball,1,MPI_LMP_BIGINT,MPI_SUM,world);
...
int ComputeChunkAtom::setup_sphere_bins()
{
  ...
  if (pbcflag) {
    myScalar *prd_half = domain->prd_half;
...
int ComputeChunkAtom::setup_cylinder_bins()
{
  ...
  myScalar **axiscoord = coord;
  memory->create(coord,ncbin,1,"chunk/atom:coord");
  myScalar **radcoord = coord;
...
void ComputeChunkAtom::bin_volumes()
{
  if (which == BIN1D || which == BIN2D || which == BIN3D) {
    ...
    myScalar slabthick = domain->prd[dim[0]] * delta[0]/prd[dim[0]];

    // area lo/hi of concentric circles in radial direction

    int iradbin;
    myScalar rlo,rhi,arealo,areahi;
    for (int i = 0; i < nchunk; i++) {
      iradbin = i / ncplane;
...
myScalar ComputeChunkAtom::memory_usage()
{
  myScalar bytes = 2*MAX(nmaxint,0) * sizeof(int);   // ichunk,exclude
  bytes += nmax * sizeof(myScalar);                  // chunk
  bytes += ncoord*nchunk * sizeof(myScalar);         // coord
