...
void Min::setup(int flag)
{
  ...
  if (nextra_global) {
    fextra = new myScalar[nextra_global];
    if (comm->me == 0 && screen)
  ...
  MY_MPI_Allreduce(&ndofme,&ndoftotal,1,MPI_LMP_BIGINT,MPI_SUM,world);
  ...
  if (stop_condition != MAXITER) {
    ...
    if (update->restrict_output == 0) {
      ...
      if (output->restart_flag) {
        ...
        if (output->restart_every_myScalar)
          output->next_restart_myScalar = update->ntimestep;
      }
    }
...
myScalar Min::energy_force(int resetflag)
{
  ...
  myScalar energy = pe_compute->compute_scalar();
...
void Min::force_clear()
{
  ...
  size_t nbytes = sizeof(myScalar) * atom->nlocal;
...
int Min::request(Pair *pair, int peratom, myScalar maxvalue)
{
  int n = nextra_atom + 1;
  xextra_atom = (myScalar **) memory->srealloc(xextra_atom,n*sizeof(myScalar *),
                                             "min:xextra_atom");
  fextra_atom = (myScalar **) memory->srealloc(fextra_atom,n*sizeof(myScalar *),
                                             "min:fextra_atom");
...
myScalar Min::fnorm_sqr()
{
  int i,n;
  myScalar *fatom;

  myScalar local_norm2_sqr = 0.0;
  ...
  myScalar norm2_sqr = 0.0;
  MY_MPI_Allreduce(&local_norm2_sqr,&norm2_sqr,1,MPI_DOUBLE,MPI_SUM,world);
...
myScalar Min::fnorm_inf()
{
  int i,n;
  myScalar *fatom;

  myScalar local_norm_inf = 0.0;
  for (i = 0; i < nvec; i++)
    local_norm_inf = MAX(fabs(ad::passive_value(fvec[i])),ad::passive_value(local_norm_inf));
  if (nextra_atom) {
    for (int m = 0; m < nextra_atom; m++) {
      fatom = fextra_atom[m];
      n = extra_nlen[m];
      for (i = 0; i < n; i++)
        local_norm_inf = MAX(fabs(ad::passive_value(fatom[i])),ad::passive_value(local_norm_inf));
    }
  }

  myScalar norm_inf = 0.0;
  MY_MPI_Allreduce(&local_norm_inf,&norm_inf,1,MPI_DOUBLE,MPI_MAX,world);

  if (nextra_global)
    for (i = 0; i < nextra_global; i++)
      norm_inf = MAX(fabs(ad::passive_value(fextra[i])),ad::passive_value(norm_inf));
