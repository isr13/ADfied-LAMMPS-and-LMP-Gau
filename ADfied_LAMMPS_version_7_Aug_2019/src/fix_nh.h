...
class FixNH : public Fix {
 public:
  ...
  virtual myScalar compute_vector(int);
  void write_restart(FILE *);
  virtual int pack_restart_data(myScalar *); // pack restart data
  virtual void restart(char *);
  int modify_param(int, char **);
  void reset_target(myScalar);
  void reset_dt();
 ...
 protected:
  int dimension,which;
  myScalar dtv,dtf,dthalf,dt4,dt8,dto;
  myScalar boltz,nktv2p,tdof;
  myScalar vol0;                      // reference volume
  myScalar t0;                        // reference temperature
                                    // used for barostat mass
  myScalar t_start,t_stop;
  myScalar t_current,t_target,ke_target;
  myScalar t_freq;
  ...
  myScalar p_start[6],p_stop[6];
  myScalar p_freq[6],p_target[6];
  myScalar omega[6],omega_dot[6];
  myScalar omega_mass[6];
  myScalar p_current[6];
  myScalar drag,tdrag_factor;        // drag factor on particle thermostat
  myScalar pdrag_factor;             // drag factor on barostat
  ...
  myScalar *eta,*eta_dot;            // chain thermostat for particles
  myScalar *eta_dotdot;
  myScalar *eta_mass;
  int mtchain;                     // length of chain
  int mtchain_default_flag;        // 1 = mtchain is default

  myScalar *etap;                    // chain thermostat for barostat
  myScalar *etap_dot;
  myScalar *etap_dotdot;
  myScalar *etap_mass;
  int mpchain;                     // length of chain

  int mtk_flag;                    // 0 if using Hoover barostat
  int pdim;                        // number of barostatted dims
  myScalar p_freq_max;               // maximum barostat frequency

  myScalar p_hydro;                  // hydrostatic target pressure

  int nc_tchain,nc_pchain;
  myScalar factor_eta;
  myScalar sigma[6];                 // scaled target stress
  myScalar fdev[6];                  // deviatoric force on barostat
  int deviatoric_flag;             // 0 if target stress tensor is hydrostatic
  myScalar h0_inv[6];                // h_inv of reference (zero strain) box
  int nreset_h0;                   // interval for resetting h0

  myScalar mtk_term1,mtk_term2;      // Martyna-Tobias-Klein corrections
  ...
  myScalar fixedpoint[3];            // location of dilation fixed-point
  ...
  myScalar compute_strain_energy();
