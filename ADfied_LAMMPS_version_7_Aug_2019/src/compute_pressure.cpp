...
void ComputePressure::init()
{
  ...
  nvirial = 0;
  vptr = NULL;
 // Count all the forces that contribute to the pressure tensor
 ...
 // Put all the contributions previously counted together into a pointer called vptr
  if (nvirial) {
    vptr = new myScalar*[nvirial];
    nvirial = 0;
...
myScalar ComputePressure::compute_scalar()
{
...
void ComputePressure::virial_compute(int n, int ndiag)
{
  ...
  for (j = 0; j < nvirial; j++) {
    vcomponent = vptr[j];
       for (i = 0; i < n; i++){
	 v[i] += vcomponent[i];
//std::cout<<vcomponent[i]<<"Component  ----> "<< i <<" <---- "<<std::endl;//
  ...
  MY_MPI_Allreduce(v,virial,n,MPI_DOUBLE,MPI_SUM,world);
	/*for(int i = 0; i<n; i++){
	virial[i] = v[i];
	}*/
