...
void ImbalanceTime::compute(myScalar *weight)
{
  ...
  myScalar maxcost;
  MY_MPI_Allreduce(&cost,&maxcost,1,MPI_DOUBLE,MPI_MAX,world);
  if (maxcost <= 0.0) return;

  int nlocal = atom->nlocal;
  myScalar localwt = 0.0;
  ...
  if (factor != 1.0) {
    ...
    MY_MPI_Allreduce(&localwt,&wtlo,1,MPI_DOUBLE,MPI_MIN,world);
    if (localwt == BIG) localwt = 0.0;
    MY_MPI_Allreduce(&localwt,&wthi,1,MPI_DOUBLE,MPI_MAX,world);
    if (wtlo == wthi) return;

    myScalar newhi = wthi*factor;
