...
{
  int i1,i2;
  myScalar delx,dely,delz,fbond;
  myScalar rsq,r,dr,rk;
  ...
  myScalar k = kstart[m] + delta * (kstop[m] - kstart[m]);
...
myScalar FixRestrain::compute_scalar()
{
  MY_MPI_Allreduce(&energy,&energy_all,1,MPI_DOUBLE,MPI_SUM,world);
  return energy_all;
}
...
myScalar FixRestrain::compute_vector(int n)
{
  if (n == 0) {
    MY_MPI_Allreduce(&ebond,&ebond_all,1,MPI_DOUBLE,MPI_SUM,world);
    return ebond_all;
  } else if (n == 1) {
    MY_MPI_Allreduce(&eangle,&eangle_all,1,MPI_DOUBLE,MPI_SUM,world);
    return eangle_all;
  } else if (n == 2) {
    MY_MPI_Allreduce(&edihed,&edihed_all,1,MPI_DOUBLE,MPI_SUM,world);
    return edihed_all;
  } else {
    return 0.0;
  }
