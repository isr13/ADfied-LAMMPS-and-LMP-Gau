...
void FixRecenter::initial_integrate(int /*vflag*/)
{
  ...
  myScalar xtarget,ytarget,ztarget;
  ...
  shift[0] = xflag ? (xtarget - xcm[0]) : static_cast<myScalar>(0.0);
  shift[1] = yflag ? (ytarget - xcm[1]) : static_cast<myScalar>(0.0);
  shift[2] = zflag ? (ztarget - xcm[2]) : static_cast<myScalar>(0.0);
...
myScalar FixRecenter::compute_scalar()
{
  return distance;
}
...
myScalar FixRecenter::compute_vector(int n)
{
  return shift[n];
}
