  ...
  ipage_atom(NULL), dpage_atom(NULL), ipage_neigh(NULL), dpage_neigh(NULL)
{
  ...
  dnumbytes = dnum * sizeof(myScalar);

  zeroes = new myScalar[dnum];
...
void FixNeighHistory::allocate_pages()
{
  ...
  if (create) {
    ...
    dpage_atom = new MyPage<myScalar>[nmypage];
    ipage_neigh = new MyPage<int>[nmypage];
    dpage_neigh = new MyPage<myScalar>[nmypage];
...
void FixNeighHistory::pre_exchange_newton()
{
  ...
  int *ilist,*jlist,*numneigh,**firstneigh;
  int *allflags;
  myScalar *allvalues,*onevalues,*jvalues;
...
void FixNeighHistory::post_neighbor()
{
  ...
  if (maxatom < nlocal) {
    ...
    firstvalue = (myScalar **)
      memory->smalloc(maxatom*sizeof(myScalar *),"neighbor_history:firstvalue");
  }
...
myScalar FixNeighHistory::memory_usage()
{
  int nmax = atom->nmax;
  myScalar bytes = nmax * sizeof(int);    // npartner
  bytes += nmax * sizeof(tagint *);     // partner
  bytes += nmax * sizeof(myScalar *);     // valuepartner
  bytes += maxatom * sizeof(int *);     // firstflag
  bytes += maxatom * sizeof(myScalar *);  // firstvalue
...
void FixNeighHistory::grow_arrays(int nmax)
{
  ...
  valuepartner = (myScalar **) memory->srealloc(valuepartner,
                                              nmax*sizeof(myScalar *),
...
int FixNeighHistory::pack_reverse_comm(int n, int first, myScalar *buf)
{
  int i,k,last;
...
void FixNeighHistory::unpack_reverse_comm(int n, int *list, myScalar *buf)
{
  int i,j,k,kk,ncount;
...
int FixNeighHistory::pack_exchange(int i, myScalar *buf)
{
  int m = 0;
  buf[m++] = npartner[i];
...
int FixNeighHistory::unpack_exchange(int nlocal, myScalar *buf)
{
...
int FixNeighHistory::pack_restart(int i, myScalar *buf)
{
  int m = 1;
  buf[m++] = npartner[i];
  for (int n = 0; n < npartner[i]; n++) {
...
int FixNeighHistory::maxsize_restart()
{
  ...
  int maxpartner_all;
  MY_MPI_Allreduce(&maxpartner,&maxpartner_all,1,MPI_INT,MPI_MAX,world);
