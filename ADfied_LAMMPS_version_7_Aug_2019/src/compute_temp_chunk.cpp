...
myScalar ComputeTempChunk::compute_scalar()
{
  int i,index;
  ...
  MY_MPI_Allreduce(&t,&scalar,1,MPI_DOUBLE,MPI_SUM,world);
  myScalar rcount = mycount;
  myScalar allcount;
  MY_MPI_Allreduce(&rcount,&allcount,1,MPI_DOUBLE,MPI_SUM,world);

  myScalar dof = nchunk*cdof + adof*allcount;
  myScalar tfactor = 0.0;
...
void ComputeTempChunk::compute_vector()
{
  ...
  MY_MPI_Allreduce(t,vector,6,MPI_DOUBLE,MPI_SUM,world);
...
void ComputeTempChunk::vcm_compute()
{
  ...
  MY_MPI_Allreduce(&vcm[0][0],&vcmall[0][0],3*nchunk,MPI_DOUBLE,MPI_SUM,world);
  MY_MPI_Allreduce(massproc,masstotal,nchunk,MPI_DOUBLE,MPI_SUM,world);
...
void ComputeTempChunk::temperature(int icol)
{
  ...
  MY_MPI_Allreduce(sum,sumall,nchunk,MPI_DOUBLE,MPI_SUM,world);
  MY_MPI_Allreduce(count,countall,nchunk,MPI_INT,MPI_SUM,world);

  // normalize temperatures by per-chunk DOF

  myScalar dof,tfactor;
...
void ComputeTempChunk::remove_bias(int i, myScalar *v)
{
...
void ComputeTempChunk::restore_bias(int i, myScalar *v)
{
  int index = cchunk->ichunk[i]-1;
  if (index < 0) return;
  v[0] += vcmall[index][0];
...
myScalar ComputeTempChunk::memory_usage()
{
  myScalar bytes = (bigint) maxchunk * 2 * sizeof(myScalar);
  bytes += (bigint) maxchunk * 2 * sizeof(int);
  bytes += (bigint) maxchunk * nvalues * sizeof(myScalar);
  if (comflag || nvalues) {
    bytes += (bigint) maxchunk * 2 * sizeof(myScalar);
    bytes += (bigint) maxchunk * 2*3 * sizeof(myScalar);
  }
  return bytes;
}
