...
class FixBoxRelax : public Fix {
 public:
  ...
  myScalar min_energy(myScalar *);
  void min_store();
  void min_clearstore();
  void min_pushstore();
  void min_popstore();
  int min_reset_ref();
  void min_step(myScalar, myScalar *);
  myScalar max_alpha(myScalar *);
  int min_dof();
 ...
 private:
  ...
  int dimension;
  myScalar p_target[6],p_current[6];
  myScalar vol0,xprdinit,yprdinit,zprdinit;
  myScalar vmax,pv2e,pflagsum;
  int kspace_flag;

  int current_lifo;              // LIFO stack pointer
  myScalar boxlo0[2][3];           // box bounds at start of line search
  myScalar boxhi0[2][3];
  myScalar boxtilt0[2][3];         // xy,xz,yz tilts at start of line search
  myScalar ds[6];                  // increment in scale matrix
  ...
  myScalar utsigma[3];               // weighting for upper-tri elements
                                   // of modified sigma
  int sigmamod_flag;               // 1 if modified sigma to be used
  myScalar fdev[6];                  // Deviatoric force on cell
  int deviatoric_flag;             // 0 if target stress tensor is hydrostatic
  myScalar h0[6];                    // h_inv of reference (zero strain) box
  myScalar h0_inv[6];                // h_inv of reference (zero strain) box
  int nreset_h0;                   // interval for resetting h0
  myScalar p_hydro;                  // hydrostatic component of target stress
