...
void ComputeStressAtom::compute_peratom()
{
  ...
  if (pairflag && force->pair) {
    myScalar **vatom = force->pair->vatom;
    for (i = 0; i < npair; i++)
      for (j = 0; j < 6; j++)
  ...
  if (bondflag && force->bond) {
    myScalar **vatom = force->bond->vatom;
    for (i = 0; i < nbond; i++)
      for (j = 0; j < 6; j++)
        stress[i][j] += vatom[i][j];
  }
  ...
  if (angleflag && force->angle) {
    myScalar **vatom = force->angle->vatom;
    for (i = 0; i < nbond; i++)
      for (j = 0; j < 6; j++)
        stress[i][j] += vatom[i][j];
  }
  ...
  if (dihedralflag && force->dihedral) {
    myScalar **vatom = force->dihedral->vatom;
    for (i = 0; i < nbond; i++)
      for (j = 0; j < 6; j++)
        stress[i][j] += vatom[i][j];
  }
  ...
  if (improperflag && force->improper) {
    myScalar **vatom = force->improper->vatom;
    for (i = 0; i < nbond; i++)
      for (j = 0; j < 6; j++)
        stress[i][j] += vatom[i][j];
  }
  ...
  if (kspaceflag && force->kspace) {
    myScalar **vatom = force->kspace->vatom;
    for (i = 0; i < nkspace; i++)
      for (j = 0; j < 6; j++)
        stress[i][j] += vatom[i][j];
  }
  ...
  if (fixflag) {
    for (int ifix = 0; ifix < modify->nfix; ifix++)
      if (modify->fix[ifix]->virial_flag) {
        myScalar **vatom = modify->fix[ifix]->vatom;
        if (vatom)
          for (i = 0; i < nlocal; i++)
            for (j = 0; j < 6; j++)
              stress[i][j] += vatom[i][j];
...
int ComputeStressAtom::pack_reverse_comm(int n, int first, myScalar *buf)
{
  int i,m,last;
...
void ComputeStressAtom::unpack_reverse_comm(int n, int *list, myScalar *buf)
{
  int i,j,m;
...
myScalar ComputeStressAtom::memory_usage()
{
  myScalar bytes = nmax*6 * sizeof(myScalar);
  return bytes;
}
