...
class CommTiled : public Comm {
 public:
  ...
  virtual void forward_comm_array(int, myScalar **);          // forward comm of array
  virtual int exchange_variable(int, myScalar *, myScalar *&);  // exchange on neigh stencil

  void coord2proc_setup();
  int coord2proc(myScalar *, int &, int &, int &);
 ...
 private:
  ...
  myScalar ***sendbox;            // bounding box of atoms to send per swap/proc
  ...
  myScalar *buf_send;             // send buffer for all comm
  myScalar *buf_recv;             // recv buffer for all comm
  ...
  struct RCBinfo {
    myScalar mysplit[3][2];      // fractional RCB bounding box for one proc
    myScalar cutfrac;            // fractional position of cut this proc owns
  ...
  myScalar *prd;                 // local ptrs to Domain attributes
  myScalar *boxlo,*boxhi;
  myScalar *sublo,*subhi;
  int dimension;
  ...
  typedef void (CommTiled::*BoxDropPtr)(int, myScalar *, myScalar *, int &);
  BoxDropPtr box_drop;
  void box_drop_brick(int, myScalar *, myScalar *, int &);
  void box_drop_tiled(int, myScalar *, myScalar *, int &);
  void box_drop_tiled_recurse(myScalar *, myScalar *, int, int, int &);

  typedef void (CommTiled::*BoxOtherPtr)(int, int, int, myScalar *, myScalar *);
  BoxOtherPtr box_other;
  void box_other_brick(int, int, int, myScalar *, myScalar *);
  void box_other_tiled(int, int, int, myScalar *, myScalar *);
  ...
  typedef int (CommTiled::*PointDropPtr)(int, myScalar *);
  PointDropPtr point_drop;
  int point_drop_brick(int, myScalar *);
  int point_drop_tiled(int, myScalar *);
  int point_drop_tiled_recurse(myScalar *, int, int);
  int closer_subbox_edge(int, myScalar *);
