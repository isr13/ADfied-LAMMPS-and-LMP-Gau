...
bigint Thermo::lost_check()
{
  ...
  MY_MPI_Allreduce(&nblocal,&ntotal,1,MPI_LMP_BIGINT,MPI_SUM,world);
  if (ntotal < 0)
...
/* ----------------------------------------------------------------------
   ...
   return value as myScalar in answer
   return 0 if str is recognized keyword, 1 if unrecognized
   customize a new keyword by adding to if statement
------------------------------------------------------------------------- */

int Thermo::evaluate_keyword(char *word, myScalar *answer)
{
  // turn off normflag if natoms = 0 to avoid divide by 0
...
/* ----------------------------------------------------------------------
   ...
   set ivalue/dvalue/bivalue if value is int/myScalar/bigint
...
{
  myScalar new_cpu;
  myScalar new_time = update->ntimestep * update->dt;
  ...
  if (firststep == 0) {
    ...
    myScalar cpu_diff = new_cpu - last_tpcpu;
    myScalar time_diff = new_time - last_time;
...
void Thermo::compute_spcpu()
{
  ...
  if (firststep == 0) {
    ...
    new_cpu = timer->elapsed(Timer::TOTAL);
    myScalar cpu_diff = new_cpu - last_spcpu;
...
{
  compute_pe();
  myScalar ke = temperature->scalar;
...
{
  compute_etotal();
  myScalar etmp = dvalue;

  compute_vol();
  myScalar vtmp = dvalue;
  if (normflag) vtmp /= natoms;

  compute_press();
  myScalar ptmp = dvalue;
...
void Thermo::compute_evdwl()
{
  ...
  MY_MPI_Allreduce(&tmp,&dvalue,1,MPI_DOUBLE,MPI_SUM,world);

  if (force->pair && force->pair->tail_flag) {
    myScalar volume = domain->xprd * domain->yprd * domain->zprd;
...
{
  if (force->bond) {
    myScalar tmp = force->bond->energy;
    MY_MPI_Allreduce(&tmp,&dvalue,1,MPI_DOUBLE,MPI_SUM,world);
    if (normflag) dvalue /= natoms;
  } else dvalue = 0.0;
}
...
{
  if (force->angle) {
    myScalar tmp = force->angle->energy;
    MY_MPI_Allreduce(&tmp,&dvalue,1,MPI_DOUBLE,MPI_SUM,world);
    if (normflag) dvalue /= natoms;
  } else dvalue = 0.0;
}
...
{
  if (force->dihedral) {
    myScalar tmp = force->dihedral->energy;
    MY_MPI_Allreduce(&tmp,&dvalue,1,MPI_DOUBLE,MPI_SUM,world);
    if (normflag) dvalue /= natoms;
  } else dvalue = 0.0;
}
...
{
  if (force->improper) {
    myScalar tmp = force->improper->energy;
    MY_MPI_Allreduce(&tmp,&dvalue,1,MPI_DOUBLE,MPI_SUM,world);
    if (normflag) dvalue /= natoms;
  } else dvalue = 0.0;
}
...
void Thermo::compute_fmax()
{
  ...
  for (int i = 0; i < nlocal; i++) {
    max = MAX(max,fabs(ad::passive_value(f[i][0])));
    max = MAX(max,fabs(ad::passive_value(f[i][1])));
    max = MAX(max,fabs(ad::passive_value(f[i][2])));
  }
  myScalar maxall;
...
void Thermo::compute_fnorm()
{
  ...
  myScalar dotall;
  MY_MPI_Allreduce(&dot,&dotall,1,MPI_DOUBLE,MPI_SUM,world);
  dvalue = sqrt(dotall);
}
...
void Thermo::compute_cellalpha()
{
  if (!domain->triclinic)
    ...
    myScalar* h = domain->h;
    myScalar cosalpha = (h[5]*h[4]+h[1]*h[3])/
...
void Thermo::compute_cellbeta()
{
  if (!domain->triclinic)
    ...
    myScalar* h = domain->h;
    myScalar cosbeta = h[4]/sqrt(h[2]*h[2]+h[3]*h[3]+h[4]*h[4]);
...
void Thermo::compute_cellgamma()
{
  if (!domain->triclinic)
    ...
    myScalar* h = domain->h;
    myScalar cosgamma = h[5]/sqrt(h[1]*h[1]+h[5]*h[5]);
