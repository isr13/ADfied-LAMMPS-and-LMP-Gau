...
void PairBornCoulDSF::compute(int eflag, int vflag)
{
  ...
  myScalar r,rsq,r2inv,r6inv,forcecoul,forceborn,factor_coul,factor_lj;
  myScalar prefactor,erfcc,erfcd,arg;
  myScalar rexp;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (eflag) {
      myScalar e_self = -(e_shift/2.0 + alpha/MY_PIS) * qtmp*qtmp*qqrd2e;
...
void PairBornCoulDSF::coeff(int narg, char **arg)
{
  ...
  myScalar a_one = force->numeric(FLERR,arg[2]);
  myScalar rho_one = force->numeric(FLERR,arg[3]);
  myScalar sigma_one = force->numeric(FLERR,arg[4]);
  if (rho_one <= 0) error->all(FLERR,"Incorrect args for pair coefficients");
  myScalar c_one = force->numeric(FLERR,arg[5]);
  myScalar d_one = force->numeric(FLERR,arg[6]);

  myScalar cut_lj_one = cut_lj_global;
...
void PairBornCoulDSF::init_style()
{
  ...
  myScalar erfcc = erfc(alpha*cut_coul);
  myScalar erfcd = exp(-alpha*alpha*cut_coul*cut_coul);
...
myScalar PairBornCoulDSF::init_one(int i, int j)
{
  if (setflag[i][j] == 0) error->all(FLERR,"All pair coeffs are not set");

  myScalar cut = MAX(cut_lj[i][j],cut_coul);
  ...
  if (offset_flag && (cut_lj[i][j] > 0.0)) {
    myScalar rexp = exp((sigma[i][j]-cut_lj[i][j])*rhoinv[i][j]);
...
void PairBornCoulDSF::write_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        fwrite(&a[i][j],sizeof(myScalar),1,fp);
        fwrite(&rho[i][j],sizeof(myScalar),1,fp);
        fwrite(&sigma[i][j],sizeof(myScalar),1,fp);
        fwrite(&c[i][j],sizeof(myScalar),1,fp);
        fwrite(&d[i][j],sizeof(myScalar),1,fp);
        fwrite(&cut_lj[i][j],sizeof(myScalar),1,fp);
      }
    }
}
...
void PairBornCoulDSF::read_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          fread(&a[i][j],sizeof(myScalar),1,fp);
          fread(&rho[i][j],sizeof(myScalar),1,fp);
          fread(&sigma[i][j],sizeof(myScalar),1,fp);
          fread(&c[i][j],sizeof(myScalar),1,fp);
          fread(&d[i][j],sizeof(myScalar),1,fp);
          fread(&cut_lj[i][j],sizeof(myScalar),1,fp);
        }
...
{
  fwrite(&alpha,sizeof(myScalar),1,fp);
  fwrite(&cut_lj_global,sizeof(myScalar),1,fp);
...
{
  if (comm->me == 0) {
    fread(&alpha,sizeof(myScalar),1,fp);
    fread(&cut_lj_global,sizeof(myScalar),1,fp);
...
myScalar PairBornCoulDSF::single(int i, int j, int itype, int jtype,
                                myScalar rsq,
                                myScalar factor_coul, myScalar factor_lj,
                                myScalar &fforce)
{
  myScalar r2inv,r6inv,r,prefactor,rexp;
  myScalar forcecoul,forceborn,phicoul,phiborn;
  myScalar erfcc,erfcd,arg;
