...
myScalar PairLJSmooth::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
  ...
  if (cut_inner[i][j] != cut[i][j]) {
    myScalar r6inv = 1.0/pow(cut_inner[i][j],6.0);
    myScalar t = cut[i][j] - cut_inner[i][j];
    myScalar tsq = t*t;
    myScalar ratio = sigma[i][j] / cut_inner[i][j];
...
myScalar PairLJSmooth::single(int /*i*/, int /*j*/, int itype, int jtype, myScalar rsq,
                            myScalar /*factor_coul*/, myScalar factor_lj,
                            myScalar &fforce)
{
  myScalar r2inv,r6inv,forcelj,philj,r,t,tsq,fskin;
