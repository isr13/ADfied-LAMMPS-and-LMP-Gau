...
class Timer : protected Pointers {
 public:
  ...
  myScalar elapsed(enum ttype);
  myScalar cpu(enum ttype);

  myScalar get_cpu(enum ttype which) const {
    return cpu_array[which]; };
  myScalar get_wall(enum ttype which) const {
    return wall_array[which]; };

  void set_wall(enum ttype, myScalar);
 ...
 private:
  myScalar cpu_array[NUM_TIMER];
  myScalar wall_array[NUM_TIMER];
  myScalar previous_cpu;
  myScalar previous_wall;
  myScalar timeout_start;
