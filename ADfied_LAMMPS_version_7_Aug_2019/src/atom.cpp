...
void Atom::tag_check()
{
  ...
  MY_MPI_Allreduce(&min,&minall,1,MPI_LMP_TAGINT,MPI_MIN,world);
  MY_MPI_Allreduce(&max,&maxall,1,MPI_LMP_TAGINT,MPI_MAX,world);
...
void Atom::tag_extend()
{
  ...
  tagint maxtag_all;
  MY_MPI_Allreduce(&maxtag,&maxtag_all,1,MPI_LMP_TAGINT,MPI_MAX,world);
  ...
  bigint notag_total;
  MY_MPI_Allreduce(&notag,&notag_total,1,MPI_LMP_BIGINT,MPI_SUM,world);
...
int Atom::tag_consecutive()
{
  ...
  MY_MPI_Allreduce(&idmin,&idminall,1,MPI_LMP_TAGINT,MPI_MIN,world);
  MY_MPI_Allreduce(&idmax,&idmaxall,1,MPI_LMP_TAGINT,MPI_MAX,world);
...
void Atom::bonus_check()
{
  ...
  MY_MPI_Allreduce(&local_ellipsoids,&num_global,1,MPI_LMP_BIGINT,MPI_SUM,world);
  ...
  MY_MPI_Allreduce(&local_lines,&num_global,1,MPI_LMP_BIGINT,MPI_SUM,world);
  if (nlines != num_global)
    error->all(FLERR,"Inconsistent 'lines' header value and number of "
               "atoms with enabled line flags");

  MY_MPI_Allreduce(&local_tris,&num_global,1,MPI_LMP_BIGINT,MPI_SUM,world);
  if (ntris != num_global)
    error->all(FLERR,"Inconsistent 'tris' header value and number of "
               "atoms with enabled tri flags");

  MY_MPI_Allreduce(&local_bodies,&num_global,1,MPI_LMP_BIGINT,MPI_SUM,world);
...
void Atom::data_atoms(int n, char *buf, tagint id_offset, tagint mol_offset,
                      int type_offset, int shiftflag, myScalar *shift)
{
  int m,xptr,iptr;
  imageint imagedata;
  //typedef ad::gt1s<double>::type ADtype;
  //std::vector<ADtype> x_test(3);
  myScalar xdata[3],lamda[3];
  myScalar *coord;
  char *next;
...
void Atom::data_bodies(int n, char *buf, AtomVecBody *avec_body,
                       tagint id_offset)
{
  int j,m,nvalues,tagdata,ninteger,nmyScalar;

  int maxint = 0;
  int maxmyScalar = 0;
  int *ivalues = NULL;
  myScalar *dvalues = NULL;
  ...
  for (int i = 0; i < n; i++) {
    ...
    nmyScalar = force->inumeric(FLERR,strtok(NULL," \t\n\r\f"));
    ...
    if ((m = map(tagdata)) >= 0) {
      ...
      if (nmyScalar > maxmyScalar) {
        delete [] dvalues;
        maxmyScalar = nmyScalar;
        dvalues = new myScalar[maxmyScalar];
      }

      for (j = 0; j < ninteger; j++)
        ivalues[j] = force->inumeric(FLERR,strtok(NULL," \t\n\r\f"));
      for (j = 0; j < nmyScalar; j++)
        dvalues[j] = force->numeric(FLERR,strtok(NULL," \t\n\r\f"));

      avec_body->data_body(m,ninteger,nmyScalar,ivalues,dvalues);

    } else {
      nvalues = ninteger + nmyScalar;    // number of values to skip
      for (j = 0; j < nvalues; j++)
        strtok(NULL," \t\n\r\f");
    }
  }
...
void Atom::set_mass(const char *file, int line, int itype, myScalar value)
{
  if (mass == NULL) error->all(file,line,"Cannot set mass for this atom style");
  if (itype < 1 || itype > ntypes)
    error->all(file,line,"Invalid type for mass set");
...
void Atom::set_mass(myScalar *values)
{
...
int Atom::radius_consistency(int itype, myScalar &rad)
{
  myScalar value = -1.0;
  int flag = 0;
  for (int i = 0; i < nlocal; i++) {
  ...
  int flagall;
  MY_MPI_Allreduce(&flag,&flagall,1,MPI_INT,MPI_SUM,world);
  if (flagall) return 0;

  MY_MPI_Allreduce(&value,&rad,1,MPI_DOUBLE,MPI_MAX,world);
  return 1;
}
...
int Atom::shape_consistency(int itype,
                            myScalar &shapex, myScalar &shapey, myScalar &shapez)
{
  myScalar zero[3] = {0.0, 0.0, 0.0};
  myScalar one[3] = {-1.0, -1.0, -1.0};
  myScalar *shape;
  ...
  myScalar oneall[3];
  MY_MPI_Allreduce(one,oneall,3,MPI_DOUBLE,MPI_MAX,world);
  shapex = oneall[0];
  shapey = oneall[1];
  shapez = oneall[2];
  return 1;
...
void Atom::setup_sort_bins()
{
  ...
  if (intel_neigh && userbinsize == 0.0) {
    ...
    myScalar nx_low = neighbor->bboxlo[0];
    myScalar ny_low = neighbor->bboxlo[1];
    myScalar nz_low = neighbor->bboxlo[2];
    myScalar nxbbox = neighbor->bboxhi[0] - nx_low;
    myScalar nybbox = neighbor->bboxhi[1] - ny_low;
    myScalar nzbbox = neighbor->bboxhi[2] - nz_low;
    ...
    myScalar binsizex = nxbbox/nnbinx;
    myScalar binsizey = nybbox/nnbiny;
    myScalar binsizez = nzbbox/nnbinz;
    ...
    bboxlo[0] = nx_low + static_cast<myScalar>(lxo) / bininvx;
    bboxlo[1] = ny_low + static_cast<myScalar>(lyo) / bininvy;
    bboxlo[2] = nz_low + static_cast<myScalar>(lzo) / bininvz;
    ...
    bboxhi[0] = bboxlo[0] + static_cast<myScalar>(nbinx) / bininvx;
    bboxhi[1] = bboxlo[1] + static_cast<myScalar>(nbiny) / bininvy;
    bboxhi[2] = bboxlo[2] + static_cast<myScalar>(nbinz) / bininvz;
  }
  #endif
...
/* ----------------------------------------------------------------------
   ...
   return index if found, and flag = 0/1 for int/myScalar
...
/* ----------------------------------------------------------------------
   add a custom variable with name of type flag = 0/1 for int/myScalar
...
int Atom::add_custom(const char *name, int flag)
{
  ...
  if (flag == 0) {
    ...
    dvector = (myScalar **) memory->srealloc(dvector,ndvector*sizeof(myScalar *),
                                           "atom:dvector");
...
/* ----------------------------------------------------------------------
   remove a custom variable of type flag = 0/1 for int/myScalar at index
