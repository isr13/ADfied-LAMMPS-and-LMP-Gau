...
void FixGravity::set_acceleration()
{
  if (style == CHUTE || style == SPHERICAL) {
    ...
    if (domain->dimension == 3) {
      myScalar length = sqrt(xdir*xdir + ydir*ydir + zdir*zdir);
      xgrav = xdir/length;
      ygrav = ydir/length;
      zgrav = zdir/length;
    } else {
      myScalar length = sqrt(xdir*xdir + ydir*ydir);
      xgrav = xdir/length;
      ygrav = ydir/length;
      zgrav = 0.0;
    }
...
myScalar FixGravity::compute_scalar()
{
  // only sum across procs one time

  if (eflag == 0) {
    MY_MPI_Allreduce(&egrav,&egrav_all,1,MPI_DOUBLE,MPI_SUM,world);
    eflag = 1;
  }
  return egrav_all;
}
