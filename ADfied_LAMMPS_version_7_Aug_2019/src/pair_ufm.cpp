...
myScalar PairUFM::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
  ...
  if (offset_flag) {
    myScalar ratio = pow(cut[i][j] / sigma[i][j],2.0);
...
myScalar PairUFM::single(int /*i*/, int /*j*/, int itype, int jtype, myScalar rsq,
                         myScalar /*factor_coul*/, myScalar factor_lj,
                         myScalar &fforce)
{
  myScalar expuf,phiuf;
  expuf = exp(- rsq * uf2[itype][jtype]);
