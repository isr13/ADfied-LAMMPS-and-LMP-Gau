  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        py_rsq = PyFloat_FromDouble(ad::passive_value(rsq));
...
myScalar PairPython::init_one(int, int)
{
  return cut_global;
}
...
myScalar PairPython::single(int /* i */, int /* j */, int itype, int jtype,
                         myScalar rsq, myScalar /* factor_coul */,
                         myScalar factor_lj, myScalar &fforce)
{
  // with hybrid/overlay we might get called for skipped types
  ...
  myScalar evdwl = factor_lj*PyFloat_AsDouble(py_value);
  Py_DECREF(py_value);
