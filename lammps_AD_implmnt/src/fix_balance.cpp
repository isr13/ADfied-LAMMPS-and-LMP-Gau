...
myScalar FixBalance::compute_scalar()
{
  return imbfinal;
}
...
myScalar FixBalance::compute_vector(int i)
{
  if (i == 0) return maxloadperproc;
  if (i == 1) return (myScalar) itercount;
  return imbprev;
}
...
myScalar FixBalance::memory_usage()
{
  myScalar bytes = irregular->memory_usage();
