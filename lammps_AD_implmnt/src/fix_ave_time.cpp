  ...
  column(NULL), array(NULL), array_total(NULL), array_list(NULL)
{
  ...
  if (mode == SCALAR) {
    vector = new myScalar[nvalues];
    vector_total = new myScalar[nvalues];
    if (ave == WINDOW)
  ...
  for (j = 0; j < nvalues; j++) {
    ...
    if (which[j] == COMPUTE) {
      ...
      if (argindex[j] == 0) {
        ...
        myScalar *cvector = compute->vector;
        for (i = 0; i < nrows; i++)
          column[i] = cvector[i];
...
myScalar FixAveTime::compute_scalar()
{
...
myScalar FixAveTime::compute_vector(int i)
{
...
myScalar FixAveTime::compute_array(int i, int j)
{
  if (i >= nrows) return 0.0;
