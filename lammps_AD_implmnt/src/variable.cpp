...
/* ----------------------------------------------------------------------
   ...
   this is checked before call to compute_equal() to return a myScalar
------------------------------------------------------------------------- */
...
/* ----------------------------------------------------------------------
   ...
   this is checked before call to compute_atom() to return a vector of myScalars
------------------------------------------------------------------------- */
...
/* ----------------------------------------------------------------------
   ...
   this is checked before call to compute_vector() to return a vector of myScalars
------------------------------------------------------------------------- */
...
char *Variable::retrieve(char *name)
{
  ...
  if (style[ivar] == INDEX || style[ivar] == WORLD ||
    ...
    myScalar answer = evaluate(data[ivar][0],NULL,ivar);
    ...
    myScalar answer = compute_equal(jvar);
...
myScalar Variable::compute_equal(int ivar)
{
  if (eval_in_progress[ivar])
    print_var_error(FLERR,"Variable has a circular dependency",ivar);
...
myScalar Variable::compute_equal(char *str)
{
...
void Variable::compute_atom(int ivar, int igroup,
                            myScalar *result, int stride, int sumflag)
{
  Tree *tree = NULL;
  myScalar *vstore;
...
int Variable::compute_vector(int ivar, myScalar **result)
{
  Tree *tree = NULL;
...
void Variable::internal_set(int ivar, myScalar value)
{
  dvalue[ivar] = value;
}
...
/* ----------------------------------------------------------------------
   ...
   equal-style variables passes in tree = NULL:
     evaluate the formula, return result as a myScalar
...
myScalar Variable::evaluate(char *str, Tree **tree, int ivar)
{
  int op,opprevious;
  myScalar value1,value2;
  char onechar;
  char *ptr;

  myScalar argstack[MAXLEVEL];
      ...
      while (nopstack && precedence[opstack[nopstack-1]] >= precedence[op]) {
        ...
        if (tree) {
          ...
          if (opprevious == ADD)
            ...
            argstack[nargstack++] = fmod(ad::passive_value(value1),ad::passive_value(value2));
...
myScalar Variable::collapse_tree(Tree *tree)
{
  myScalar arg1,arg2;
  ...
  if (tree->type == MODULO) {
    ...
    tree->value = fmod(ad::passive_value(arg1),ad::passive_value(arg2));
    return tree->value;
  }
  ...
  if (tree->type == NORMAL) {
    collapse_tree(tree->first);
    myScalar sigma = collapse_tree(tree->second);
    if (sigma < 0.0)
      error->one(FLERR,"Invalid math function in variable formula");
    if (randomatom == NULL) {
      int seed = static_cast<int> (collapse_tree(tree->extra[0]));
  ...
  if (tree->type == RAMP) {
    ...
    if (tree->first->type != VALUE || tree->second->type != VALUE) return 0.0;
    tree->type = VALUE;
    myScalar delta = update->ntimestep - update->beginstep;
  ...
  if (tree->type == LOGFREQ2) {
    ...
    if (update->ntimestep < ivalue1) tree->value = ivalue1;
      ...
      tree->value = ivalue1;
      myScalar delta = ivalue1*(ivalue3-1.0)/ivalue2;
      int count = 0;
  ...
  if (tree->type == LOGFREQ3) {
    ...
    //else if (update->ntimestep <= ivalue3){
      ...
      tree->value = ivalue1;
      myScalar logsp = ivalue1;
      myScalar factor = pow(((myScalar)ivalue3)/ivalue1, 1.0/(ivalue2-1));
      int linsp = ivalue1;
      while (update->ntimestep >= (tree->value)) {
        logsp *= factor;
        linsp++;
  ...
  if (tree->type == VDISPLACE) {
    myScalar arg1 = collapse_tree(tree->first);
    myScalar arg2 = collapse_tree(tree->second);
    if (tree->first->type != VALUE || tree->second->type != VALUE) return 0.0;
    tree->type = VALUE;
    myScalar delta = update->ntimestep - update->beginstep;
  ...
  if (tree->type == SWIGGLE) {
    myScalar arg1 = collapse_tree(tree->first);
    myScalar arg2 = collapse_tree(tree->second);
    myScalar arg3 = collapse_tree(tree->extra[0]);
    if (tree->first->type != VALUE || tree->second->type != VALUE ||
        tree->extra[0]->type != VALUE) return 0.0;
    tree->type = VALUE;
    if (arg3 == 0.0)
...
myScalar Variable::eval_tree(Tree *tree, int i)
{
  myScalar arg,arg1,arg2,arg3;

  if (tree->type == VALUE) return tree->value;
  if (tree->type == ATOMARRAY) return tree->array[i*tree->nstride];
  if (tree->type == TYPEARRAY) return tree->array[atom->type[i]];
  if (tree->type == INTARRAY) return (myScalar) tree->iarray[i*tree->nstride];
  if (tree->type == BIGINTARRAY) return (myScalar) tree->barray[i*tree->nstride];
  ...
  if (tree->type == MULTIPLY)
    return eval_tree(tree->first,i) * eval_tree(tree->second,i);
  if (tree->type == DIVIDE) {
    myScalar denom = eval_tree(tree->second,i);
  ...
  if (tree->type == MODULO) {
    ...
    return fmod(ad::passive_value(eval_tree(tree->first,i)),ad::passive_value(denom));
  }
  if (tree->type == CARAT) {
    myScalar exponent = eval_tree(tree->second,i);
  ...
  if (tree->type == RANDOM) {
    myScalar lower = eval_tree(tree->first,i);
    myScalar upper = eval_tree(tree->second,i);
    if (randomatom == NULL) {
  ...
  if (tree->type == NORMAL) {
    myScalar mu = eval_tree(tree->first,i);
    myScalar sigma = eval_tree(tree->second,i);
    if (sigma < 0.0)
      error->one(FLERR,"Invalid math function in variable formula");
    if (randomatom == NULL) {
      int seed = static_cast<int> (eval_tree(tree->extra[0],i));
...
int Variable::math_function(char *word, char *contents, Tree **tree,
                            ...
                            myScalar *argstack, int &nargstack, int ivar)
{
  ...
  if (strcmp(word,"sqrt") == 0) {
    ...
    if (tree) newtree->type = RAMP;
      ...
      myScalar delta = update->ntimestep - update->beginstep;
      if (delta != 0.0) delta /= update->endstep - update->beginstep;
      myScalar value = value1 + delta*(value2-value1);
    ...
    if (tree) newtree->type = VDISPLACE;
      ...
      myScalar delta = update->ntimestep - update->beginstep;
      myScalar value = value1 + value2*delta*update->dt;
      argstack[nargstack++] = value;
    }
    ...
    if (tree) newtree->type = CWIGGLE;
      ...
      if (values[0] == 0.0)
        print_var_error(FLERR,"Invalid math function in variable formula",ivar);
      myScalar delta = update->ntimestep - update->beginstep;
      myScalar omega = 2.0*MY_PI/values[0];
      myScalar value = value1 + value2*sin(omega*delta*update->dt);
      argstack[nargstack++] = value;
    }
    ...
    if (tree) newtree->type = CWIGGLE;
      ...
      myScalar delta = update->ntimestep - update->beginstep;
      myScalar omega = 2.0*MY_PI/values[0];
      myScalar value = value1 + value2*(1.0-cos(omega*delta*update->dt));
      argstack[nargstack++] = value;
    }
  }
                             ...
                             myScalar *argstack, int &nargstack, int ivar)
{
  ...
  if (strcmp(word,"count") == 0) {
    ...
    myScalar xcm[3];
    if (narg == 2) {
      myScalar masstotal = group->mass(igroup);
      group->xcm(igroup,masstotal,xcm);
    } else if (narg == 3) {
      int iregion = region_function(args[2],ivar);
      myScalar masstotal = group->mass(igroup,iregion);
    ...
    atom->check_mass(FLERR);
    myScalar xcm[3],angmom[3],inertia[3][3],omega[3];
    if (narg == 2) {
      myScalar masstotal = group->mass(igroup);
      group->xcm(igroup,masstotal,xcm);
                               ...
                               myScalar *argstack, int &nargstack, int ivar)
{
  ...
  if (strcmp(word,"sum") == 0 || strcmp(word,"min") == 0 ||
    ...
    if (method == SLOPE) {
      myScalar numerator = nvec*sxy - sx*sy;
      myScalar denominator = nvec*sxx - sx*sx;
  ...
  // special function for file-style or atomfile-style variables
    ...
    if (style[ivar] == SCALARFILE) {
      myScalar value = atof(data[ivar][0]);
      int done = reader[ivar]->read_scalar(data[ivar][0]);
      if (done) remove(ivar);
    ...
    // set selfalloc = 1 so result will be deleted by free_tree() after eval
      ...
      myScalar *result;
      memory->create(result,atom->nlocal,"variable:result");
      memcpy(result,reader[ivar]->fixstore->vstore,atom->nlocal*sizeof(myScalar));
...
void Variable::peratom2global(int flag, char *word,
                              myScalar *vector, int nstride, tagint id,
                              Tree **tree, Tree **treestack, int &ntreestack,
                              myScalar *argstack, int &nargstack)
{
  ...
  myScalar value;
  MY_MPI_Allreduce(&mine,&value,1,MPI_DOUBLE,MPI_SUM,world);
...
myScalar Variable::constant(char *word)
{
...
myScalar Variable::evaluate_boolean(char *str)
{
  ...
  struct Arg {
    ...
    myScalar value;      // stored numeric value
...
int VarReader::read_peratom()
{
  ...
  myScalar *vstore = fixstore->vstore;
