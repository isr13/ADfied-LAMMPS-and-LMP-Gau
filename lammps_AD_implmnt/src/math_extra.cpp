...
int mldivide3(const myScalar m[3][3], const myScalar *v, myScalar *ans)
{
  ...
  for (unsigned i = 0; i < 2; i++) {
    ...
    for (unsigned j = i+1; j < 3; j++) {
      if (fabs(aug[j][i]) > fabs(aug[i][i])) {
        myScalar tempv[4];
        memcpy(tempv,aug[i],4*sizeof(myScalar));
        memmove(aug[i],aug[j],4*sizeof(myScalar));
        memcpy(aug[j],tempv,4*sizeof(myScalar));
      }
    }
    ...
    if (p == 3) return 1;
      ...
      if (p != i) {
        myScalar tempv[4];
        memcpy(tempv,aug[i],4*sizeof(myScalar));
        memmove(aug[i],aug[p],4*sizeof(myScalar));
        memcpy(aug[p],tempv,4*sizeof(myScalar));
      }

    for (unsigned j = i+1; j < 3; j++) {
      myScalar m = aug[j][i]/aug[i][i];
...
{
  int i,j,k;
  myScalar tresh,theta,tau,t,sm,s,h,g,c,b[3],z[3];
...
{
  myScalar g = matrix[i][j];
  myScalar h = matrix[k][l];
...
void no_squish_rotate(int k, myScalar *p, myScalar *q, myScalar *inertia,
                      myScalar dt)
{
  myScalar phi,c_phi,s_phi,kp[4],kq[4];
...
void exyz_to_q(myScalar *ex, myScalar *ey, myScalar *ez, myScalar *q)
{
  ...
  myScalar q0sq = 0.25 * (ex[0] + ey[1] + ez[2] + 1.0);
  myScalar q1sq = q0sq - 0.5 * (ey[1] + ez[2]);
  myScalar q2sq = q0sq - 0.5 * (ex[0] + ez[2]);
  myScalar q3sq = q0sq - 0.5 * (ex[0] + ey[1]);
...
{
  myScalar w2 = quat[0]*quat[0];
  myScalar i2 = quat[1]*quat[1];
  myScalar j2 = quat[2]*quat[2];
  myScalar k2 = quat[3]*quat[3];
  myScalar twoij = 2.0*quat[1]*quat[2];
  myScalar twoik = 2.0*quat[1]*quat[3];
  myScalar twojk = 2.0*quat[2]*quat[3];
  myScalar twoiw = 2.0*quat[1]*quat[0];
  myScalar twojw = 2.0*quat[2]*quat[0];
  myScalar twokw = 2.0*quat[3]*quat[0];
...
void inertia_ellipsoid(myScalar *radii, myScalar *quat, myScalar mass,
                       myScalar *inertia)
{
  myScalar p[3][3],ptrans[3][3],itemp[3][3],tensor[3][3];
  myScalar idiag[3];
...
void inertia_line(myScalar length, myScalar theta, myScalar mass, myScalar *inertia)
{
  myScalar p[3][3],ptrans[3][3],itemp[3][3],tensor[3][3];
  myScalar q[4],idiag[3];
...
{
  myScalar s[3][3] = {{2.0, 1.0, 1.0}, {1.0, 2.0, 1.0}, {1.0, 1.0, 2.0}};
  myScalar v[3][3],sv[3][3],vtsv[3][3];
  myScalar vvv[3],v1mv0[3],v2mv0[3],normal[3];
  ...
  myScalar sum = lensq3(v0) + lensq3(v1) + lensq3(v2);
  ...
  myScalar a = len3(normal);
  myScalar inv24 = mass/24.0;
...
{
  const myScalar angleSq = angle * angle;
  const myScalar cosAngle = (1.0 - angleSq * 0.25) / (1.0 + angleSq * 0.25);
  const myScalar sinAngle = angle / (1.0 + angleSq * 0.25);
