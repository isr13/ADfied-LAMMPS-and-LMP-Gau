...
void PairDPDTstat::coeff(int narg, char **arg)
{
  ...
  myScalar a0_one = 0.0;
  myScalar gamma_one = force->numeric(FLERR,arg[2]);
...
{
  fwrite(&t_start,sizeof(myScalar),1,fp);
  fwrite(&t_stop,sizeof(myScalar),1,fp);
...
{
  if (comm->me == 0) {
    fread(&t_start,sizeof(myScalar),1,fp);
    fread(&t_stop,sizeof(myScalar),1,fp);
