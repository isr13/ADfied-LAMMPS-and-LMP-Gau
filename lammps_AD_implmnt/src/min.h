...
class Min : protected Pointers {
 public:
  myScalar einitial,efinal,eprevious;
  myScalar fnorm2_init,fnorminf_init,fnorm2_final,fnorminf_final;
  myScalar alpha_final;
  int niter,neval;
  int stop_condition;
  char *stopstr;
  ...
  void run(int);
  void cleanup();
  int request(class Pair *, int, myScalar);
 ...
 protected:
  ...
  myScalar dmax;                // max dist to move any atom in one step
  ...
  myScalar ecurrent;                  // current potential energy

  bigint ndoftotal;           // total dof for entire problem

  int nvec;                   // local atomic dof = length of xvec
  myScalar *xvec;               // variables for atomic dof, as 1d vector
  myScalar *fvec;               // force vector for atomic dof, as 1d vector

  int nextra_global;          // # of extra global dof due to fixes
  myScalar *fextra;             // force vector for extra global dof
                              // xextra is stored by fix

  int nextra_atom;            // # of extra per-atom variables
  myScalar **xextra_atom;       // ptr to the variable
  myScalar **fextra_atom;       // ptr to the force on the variable
  int *extra_peratom;         // # of values in variable, e.g. 3 in x
  int *extra_nlen;            // total local length of variable, e.g 3*nlocal
  myScalar *extra_max;          // max allowed change per iter for atom's var
  ...
  myScalar energy_force(int);
  void force_clear();

  myScalar compute_force_norm_sqr();
  myScalar compute_force_norm_inf();
