...
/* ----------------------------------------------------------------------
   ...
   with MY_MPI_Allreduce, e.g. xcm() or count()
------------------------------------------------------------------------- */
...
int Region::match(myScalar x, myScalar y, myScalar z)
{
...
int Region::surface(myScalar x, myScalar y, myScalar z, myScalar cutoff)
{
  int ncontact;
  myScalar xs,ys,zs;
  myScalar xnear[3],xorig[3];
...
void Region::add_contact(int n, myScalar *x, myScalar xp, myScalar yp, myScalar zp)
{
  myScalar delx = x[0] - xp;
  myScalar dely = x[1] - yp;
  myScalar delz = x[2] - zp;
...
void Region::forward_transform(myScalar &x, myScalar &y, myScalar &z)
{
...
void Region::inverse_transform(myScalar &x, myScalar &y, myScalar &z)
{
  if (moveflag) {
    x -= dx;
    y -= dy;
...
void Region::rotate(myScalar &x, myScalar &y, myScalar &z, myScalar angle)
{
  myScalar a[3],b[3],c[3],d[3],disp[3];

  myScalar sine = sin(angle);
  myScalar cosine = cos(angle);
  d[0] = x - point[0];
  d[1] = y - point[1];
  d[2] = z - point[2];
  myScalar x0dotr = d[0]*runit[0] + d[1]*runit[1] + d[2]*runit[2];
  c[0] = x0dotr * runit[0];
  c[1] = x0dotr * runit[1];
  c[2] = x0dotr * runit[2];
  a[0] = d[0] - c[0];
...
void Region::point_on_line_segment(myScalar *a, myScalar *b,
                                   myScalar *c, myScalar *d)
{
  myScalar ba[3],ca[3];

  MathExtra::sub3(b,a,ba);
  MathExtra::sub3(c,a,ca);
  myScalar t = MathExtra::dot3(ca,ba) / MathExtra::dot3(ba,ba);
  if (t <= 0.0) {
    d[0] = a[0];
    d[1] = a[1];
    d[2] = a[2];
...
void Region::set_velocity()
{
  ...
  if (rotateflag) {
    ...
    rpoint[2] = point[2] + dz;
    if (update->ntimestep > 0) {
      myScalar angvel = (theta-prev[3]) / update->dt;
...
void Region::velocity_contact(myScalar *vwall, myScalar *x, int ic)
{
  myScalar xc[3];
...
void Region::write_restart(FILE *fp)
{
  ...
  fwrite(prev,sizeof(myScalar),size_restart,fp);
}
...
int Region::restart(char *buf, int &n)
{
  ...
  memcpy(prev,&buf[n],size_restart*sizeof(myScalar));
  return 1;
}
