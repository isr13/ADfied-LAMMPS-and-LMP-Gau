...
void Special::combine()
{
  ...
  MY_MPI_Allreduce(&maxspecial,&atom->maxspecial,1,MPI_INT,MPI_MAX,world);
...
void Special::angle_trim()
{
  ...
  myScalar onethreecount = 0.0;
  for (i = 0; i < nlocal; i++) onethreecount += nspecial[i][1];
  myScalar allcount;
  MY_MPI_Allreduce(&onethreecount,&allcount,1,MPI_DOUBLE,MPI_SUM,world);
...
void Special::dihedral_trim()
{
  ...
  myScalar onefourcount = 0.0;
  for (i = 0; i < nlocal; i++) onefourcount += nspecial[i][2];
  myScalar allcount;
  MY_MPI_Allreduce(&onefourcount,&allcount,1,MPI_DOUBLE,MPI_SUM,world);
...
void Special::timer_output(myScalar time1)
{
