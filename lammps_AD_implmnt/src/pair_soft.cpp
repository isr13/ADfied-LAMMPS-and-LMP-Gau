...
void PairSoft::coeff(int narg, char **arg)
{
  ...
  myScalar prefactor_one = force->numeric(FLERR,arg[2]);
...
myScalar PairSoft::init_one(int i, int j)
{
...
void PairSoft::write_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        fwrite(&prefactor[i][j],sizeof(myScalar),1,fp);
...
void PairSoft::read_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          fread(&prefactor[i][j],sizeof(myScalar),1,fp);
...
myScalar PairSoft::single(int /*i*/, int /*j*/, int itype, int jtype, myScalar rsq,
                        myScalar /*factor_coul*/, myScalar factor_lj,
                        myScalar &fforce)
{
  myScalar r,arg,philj;
