...
myScalar ComputeTempRegion::compute_scalar()
{
  ...
  myScalar tarray[2],tarray_all[2];
  tarray[0] = count;
  tarray[1] = t;
  MY_MPI_Allreduce(tarray,tarray_all,2,MPI_DOUBLE,MPI_SUM,world);
...
void ComputeTempRegion::remove_bias(int i, myScalar *v)
{
  myScalar *x = atom->x[i];
...
void ComputeTempRegion::remove_bias_thr(int i, myScalar *v, myScalar *b)
{
  myScalar *x = atom->x[i];
  if (domain->regions[iregion]->match(x[0],x[1],x[2]))
    b[0] = b[1] = b[2] = 0.0;
...
void ComputeTempRegion::restore_bias(int /*i*/, myScalar *v)
{
  v[0] += vbias[0];
  v[1] += vbias[1];
  v[2] += vbias[2];
...
void ComputeTempRegion::restore_bias_thr(int /*i*/, myScalar *v, myScalar *b)
{
  v[0] += b[0];
  v[1] += b[1];
  v[2] += b[2];
...
myScalar ComputeTempRegion::memory_usage()
{
