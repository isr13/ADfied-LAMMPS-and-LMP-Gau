  ...
  etap_mass(NULL)
{
  ...
  if (tstat_flag) {
    int ich;
    eta = new myScalar[mtchain];

    // add one extra dummy thermostat, set to zero

    eta_dot = new myScalar[mtchain+1];
    eta_dot[mtchain] = 0.0;
    eta_dotdot = new myScalar[mtchain];
    for (ich = 0; ich < mtchain; ich++) {
      eta[ich] = eta_dot[ich] = eta_dotdot[ich] = 0.0;
    }
    eta_mass = new myScalar[mtchain];
  ...
  if (pstat_flag) {
    ...
    if (mpchain) {
      int ich;
      etap = new myScalar[mpchain];

      // add one extra dummy thermostat, set to zero

      etap_dot = new myScalar[mpchain+1];
      etap_dot[mpchain] = 0.0;
      etap_dotdot = new myScalar[mpchain];
      for (ich = 0; ich < mpchain; ich++) {
        etap[ich] = etap_dot[ich] =
          etap_dotdot[ich] = 0.0;
      }
      etap_mass = new myScalar[mpchain];
...
void FixNH::setup(int /*vflag*/)
{
  ...
  if (pstat_flag) {
    myScalar kt = boltz * t_target;
    myScalar nkt = (atom->natoms + 1) * kt;
...
void FixNH::couple()
{
  myScalar *tensor = pressure->vector;

  if (pstyle == ISO)
    p_current[0] = p_current[1] = p_current[2] = pressure->scalar;
  else if (pcouple == XYZ) {
    myScalar ave = 1.0/3.0 * (tensor[0] + tensor[1] + tensor[2]);
    p_current[0] = p_current[1] = p_current[2] = ave;
  } else if (pcouple == XY) {
    myScalar ave = 0.5 * (tensor[0] + tensor[1]);
    p_current[0] = p_current[1] = ave;
    p_current[2] = tensor[2];
  } else if (pcouple == YZ) {
    myScalar ave = 0.5 * (tensor[1] + tensor[2]);
    p_current[1] = p_current[2] = ave;
    p_current[0] = tensor[0];
  } else if (pcouple == XZ) {
    myScalar ave = 0.5 * (tensor[0] + tensor[2]);
  ...
  if (!std::isfinite(ad::passive_value(p_current[0])) || !std::isfinite(ad::passive_value(p_current[1])) || !std::isfinite(ad::passive_value(p_current[2])))
  ...
  if (pstyle == TRICLINIC) {
    ...
    if (!std::isfinite(ad::passive_value(p_current[3])) || !std::isfinite(ad::passive_value(p_current[4])) || !std::isfinite(ad::passive_value(p_current[5])))
      error->all(FLERR,"Non-numeric pressure - simulation unstable");
  }
}
...
void FixNH::remap()
{
  ...
  myScalar dto2 = dto/2.0;
  myScalar dto4 = dto/4.0;
  myScalar dto8 = dto/8.0;
...
void FixNH::write_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    int size = nsize * sizeof(myScalar);
    fwrite(&size,sizeof(int),1,fp);
    fwrite(list,sizeof(myScalar),nsize,fp);
  }
...
int FixNH::pack_restart_data(myScalar *list)
{
  int n = 0;
...
myScalar FixNH::compute_scalar()
{
  int i;
  myScalar volume;
  myScalar energy;
  myScalar kt = boltz * t_target;
  myScalar lkt_press = 0.0;
  int ich;
...
myScalar FixNH::compute_vector(int n)
{
  int ilen;
...
void FixNH::reset_target(myScalar t_new)
{
...
void FixNH::nhc_temp_integrate()
{
  int ich;
  myScalar expfac;
  myScalar kecurrent = tdof * boltz * t_current;
...
{
  int ich,i,pdof;
  myScalar expfac,factor_etap,kecurrent;
  myScalar kt = boltz * t_target;
  myScalar lkt_press;
...
myScalar FixNH::compute_strain_energy()
{
  ...
  myScalar energy = 0.5*(d0+d1+d2)/nktv2p;
  return energy;
}
...
void FixNH::nh_omega_dot()
{
  ...
  if (mtk_flag) {
    if (pstyle == ISO) {
      ...
      myScalar *mvv_current = temperature->vector;
      for (int i = 0; i < 3; i++)
        if (p_flag[i])
...
{
  myScalar xprd = domain->xprd;
  myScalar yprd = domain->yprd;

  // flip is only triggered when tilt exceeds 0.5 by DELTAFLIP
  // this avoids immediate re-flipping due to tilt oscillations

  myScalar xtiltmax = (0.5+DELTAFLIP)*xprd;
  myScalar ytiltmax = (0.5+DELTAFLIP)*yprd;
