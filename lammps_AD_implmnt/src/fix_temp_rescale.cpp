...
void FixTempRescale::end_of_step()
{
  ...
  if (fabs(t_current-t_target) > t_window) {
    ...
    myScalar factor = sqrt(t_target/t_current);
    myScalar efactor = 0.5 * force->boltz * temperature->dof;
...
void FixTempRescale::reset_target(myScalar t_new)
{
...
myScalar FixTempRescale::compute_scalar()
{
  return energy;
}
