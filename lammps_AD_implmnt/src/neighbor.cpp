...
void Neighbor::init()
{
  ...
  if (cutneighsq == NULL) {
    ...
    cuttype = new myScalar[n+1];
    cuttypesq = new myScalar[n+1];
  }

  myScalar cutoff,delta,cut;
  cutneighmin = BIG;
  cutneighmax = 0.0;
  ...
  if (respa) {
    myScalar *cut_respa = ((Respa *) update->integrate)->cutoff;
...
int Neighbor::init_pair()
{
  ...
  // start myScalar loop check again whenever a swap is made
  // done when entire myScalar loop test results in no swaps
...
void Neighbor::init_topology()
{
  ...
  int onoff = bond_off;
  MY_MPI_Allreduce(&onoff,&bond_off,1,MPI_INT,MPI_MAX,world);
  onoff = angle_off;
  MY_MPI_Allreduce(&onoff,&angle_off,1,MPI_INT,MPI_MAX,world);
  onoff = dihedral_off;
  MY_MPI_Allreduce(&onoff,&dihedral_off,1,MPI_INT,MPI_MAX,world);
  onoff = improper_off;
  MY_MPI_Allreduce(&onoff,&improper_off,1,MPI_INT,MPI_MAX,world);
...
void Neighbor::print_pairwise_info()
{
  ...
  const myScalar cutghost = MAX(cutneighmax,comm->cutghostuser);

  myScalar binsize, bbox[3];
...
int Neighbor::check_distance()
{
  ...
  myScalar delta,deltasq,delta1,delta2;
