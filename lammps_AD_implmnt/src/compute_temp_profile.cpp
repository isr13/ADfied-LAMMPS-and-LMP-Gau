...
myScalar ComputeTempProfile::compute_scalar()
{
  int ibin;
  myScalar vthermal[3];
...
void ComputeTempProfile::compute_array()
{
  ...
  MY_MPI_Allreduce(tbin,tbinall,nbins,MPI_DOUBLE,MPI_SUM,world);
...
void ComputeTempProfile::remove_bias(int i, myScalar *v)
{
  int ibin = bin[i];
...
void ComputeTempProfile::remove_bias_thr(int i, myScalar *v, myScalar *)
{
  remove_bias(i,v);
}
...
void ComputeTempProfile::restore_bias(int i, myScalar *v)
{
  int ibin = bin[i];
  if (xflag) v[0] += binave[ibin][ivx];
  if (yflag) v[1] += binave[ibin][ivy];
...
void ComputeTempProfile::restore_bias_thr(int i, myScalar *v, myScalar *)
{
  restore_bias(i,v);
}
...
void ComputeTempProfile::bin_average()
{
  ...
  MY_MPI_Allreduce(vbin[0],binave[0],nbins*ncount,MPI_DOUBLE,MPI_SUM,world);
...
myScalar ComputeTempProfile::memory_usage()
{
  myScalar bytes = maxatom * sizeof(int);
  bytes += nbins*ncount * sizeof(myScalar);
  return bytes;
}
