...
void DisplaceAtoms::command(int narg, char **arg)
{
  ...
  if (style == RANDOM) {
    ...
    myScalar dx = xscale*force->numeric(FLERR,arg[2]);
    myScalar dy = yscale*force->numeric(FLERR,arg[3]);
    myScalar dz = zscale*force->numeric(FLERR,arg[4]);
  ...
  if (style == ROTATE) {
    myScalar theta_new;
    myScalar axis[3],point[3],qrotate[4],qnew[4];
    myScalar a[3],b[3],c[3],d[3],disp[3],runit[3];
    myScalar *quat;
    ...
    myScalar theta = force->numeric(FLERR,arg[8]);
    ...
    myScalar angle = MY_PI*theta/180.0;
    myScalar cosine = cos(angle);
    myScalar sine = sin(angle);

    myScalar qcosine = cos(0.5*angle);
    myScalar qsine = sin(0.5*angle);
    qrotate[0] = qcosine;
    ...
    for (i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        if (theta_flag && line[i] >= 0.0) {
          theta_new = fmod(ad::passive_value(avec_line->bonus[line[i]].theta+angle),ad::passive_value(MY_2PI));
...
void DisplaceAtoms::move(int idim, char *arg, myScalar scale)
{
  myScalar **x = atom->x;
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
  ...
  if (strstr(arg,"v_") != arg) {
    myScalar delta = scale*force->numeric(FLERR,arg);
    ...
    if (input->variable->equalstyle(ivar)) {
      myScalar delta = scale * input->variable->compute_equal(ivar);
      for (int i = 0; i < nlocal; i++)
        if (mask[i] & groupbit) x[i][idim] += delta;
