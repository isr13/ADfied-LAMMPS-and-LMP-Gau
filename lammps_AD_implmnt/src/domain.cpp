...
void Domain::set_lamda_box()
{
  if (comm->layout != Comm::LAYOUT_TILED) {
    ...
    myScalar *xsplit = comm->xsplit;
    myScalar *ysplit = comm->ysplit;
    myScalar *zsplit = comm->zsplit;
    ...
    myScalar (*mysplit)[2] = comm->mysplit;
...
void Domain::reset_box()
{
  ...
  if (nonperiodic == 2) {
    ...
    MY_MPI_Allreduce(extent,all,6,MPI_DOUBLE,MPI_MAX,world);
...
void Domain::pbc()
{
  ...
  int flag = 0;
  for (i = 0; i < n3; i++)
    if (!std::isfinite(ad::passive_value(*coord++))) flag = 1;
...
int Domain::inside_nonperiodic(myScalar* x)
{
  myScalar *lo,*hi;
  myScalar lamda[3];
...
void Domain::image_check()
{
  ...
  int flagall;
  MY_MPI_Allreduce(&flag,&flagall,1,MPI_INT,MPI_MAX,world);
  if (flagall && comm->me == 0)
    error->warning(FLERR,"Inconsistent image flags");

  if (lostbond == Thermo::WARN) {
    int all;
    MY_MPI_Allreduce(&nmissing,&all,1,MPI_INT,MPI_SUM,world);
    if (all && comm->me == 0)
...
void Domain::box_too_small_check()
{
  ...
  myScalar delx,dely,delz,rsq;
  myScalar maxbondme = 0.0;
  ...
  myScalar maxbondall;
  MY_MPI_Allreduce(&maxbondme,&maxbondall,1,MPI_DOUBLE,MPI_MAX,world);
  ...
  myScalar maxdelta = maxbondall * BONDSTRETCH;
...
void Domain::subbox_too_small_check(myScalar thresh)
{
  int flag = 0;
  if (!triclinic) {
    ...
    myScalar delta = subhi_lamda[0] - sublo_lamda[0];
...
void Domain::minimum_image(myScalar &dx, myScalar &dy, myScalar &dz)
{
  if (triclinic == 0) {
    if (xperiodic) {
...
void Domain::minimum_image(myScalar *delta)
{
  if (triclinic == 0) {
    if (xperiodic) {
...
void Domain::minimum_image_once(myScalar *delta)
{
  if (triclinic == 0) {
    if (xperiodic) {
...
int Domain::closest_image(int i, int j)
{
  ...
  int closest = j;
  myScalar delx = xi[0] - x[j][0];
  myScalar dely = xi[1] - x[j][1];
  myScalar delz = xi[2] - x[j][2];
  myScalar rsqmin = delx*delx + dely*dely + delz*delz;
  myScalar rsq;
...
int Domain::closest_image(const myScalar * const pos, int j)
{
  if (j < 0) return j;

  const int * const sametag = atom->sametag;
  const myScalar * const * const x = atom->x;

  int closest = j;
  myScalar delx = pos[0] - x[j][0];
  myScalar dely = pos[1] - x[j][1];
  myScalar delz = pos[2] - x[j][2];
  myScalar rsqmin = delx*delx + dely*dely + delz*delz;
  myScalar rsq;
...
void Domain::closest_image(const myScalar * const xi, const myScalar * const xj, myScalar * const xjimage)
{
  myScalar dx = xj[0] - xi[0];
  myScalar dy = xj[1] - xi[1];
  myScalar dz = xj[2] - xi[2];
...
void Domain::remap(myScalar *x, imageint &image)
{
  myScalar *lo,*hi,*period,*coord;
  myScalar lamda[3];
  imageint idim,otherdims;
...
void Domain::remap_near(myScalar *xnew, myScalar *xold)
{
  int n;
  myScalar *coordnew,*coordold,*period,*half;
  myScalar lamdanew[3],lamdaold[3];
...
void Domain::unmap(myScalar *x, imageint image)
{
...
void Domain::unmap(const myScalar *x, imageint image, myScalar *y)
{
  int xbox = (image & IMGMASK) - IMGMAX;
  int ybox = (image >> IMGBITS & IMGMASK) - IMGMAX;
  int zbox = (image >> IMG2BITS) - IMGMAX;
...
int Domain::ownatom(int /*id*/, myScalar *x, imageint *image, int shrinkexceed)
{
  myScalar lamda[3];
  myScalar *coord,*blo,*bhi,*slo,*shi;
...
void Domain::lamda2x(myScalar *lamda, myScalar *x)
{
...
void Domain::x2lamda(myScalar *x, myScalar *lamda)
{
  myScalar delta[3];
  delta[0] = x[0] - boxlo[0];
  delta[1] = x[1] - boxlo[1];
...
void Domain::x2lamda(myScalar *x, myScalar *lamda,
                     myScalar *my_boxlo, myScalar *my_h_inv)
{
  myScalar delta[3];
...
void Domain::bbox(myScalar *lo, myScalar *hi, myScalar *bboxlo, myScalar *bboxhi)
{
  myScalar x[3];
...
void Domain::lamda_box_corners(myScalar *lo, myScalar *hi)
{
