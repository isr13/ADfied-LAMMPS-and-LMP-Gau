  ...
  distsq(NULL), nearest(NULL), qnarray(NULL)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"degree") == 0) {
      ...
      ndegree = ad::passive_value(force->numeric(FLERR,arg[iarg+1]));
      if (ndegree < 0)
...
inline void ComputeHexOrderAtom::calc_qn_complex(myScalar delx, myScalar dely, myScalar &u, myScalar &v) {
  myScalar rinv = 1.0/sqrt(delx*delx+dely*dely);
  myScalar x = delx*rinv;
  myScalar y = dely*rinv;
  std::complex<myScalar> z(x, y);
  std::complex<myScalar> zn = pow(z, ndegree);
  u = real(zn);
  v = imag(zn);
}
...
inline void ComputeHexOrderAtom::calc_qn_trig(myScalar delx, myScalar dely, myScalar &u, myScalar &v) {
  myScalar ntheta;
...
void ComputeHexOrderAtom::select2(int k, int n, myScalar *arr, int *iarr)
{
...
myScalar ComputeHexOrderAtom::memory_usage()
{
  myScalar bytes = ncol*nmax * sizeof(myScalar);
  bytes += maxneigh * sizeof(myScalar);
