...
void ComputePEAtom::compute_peratom()
{
  ...
  if (pairflag && force->pair) {
    myScalar *eatom = force->pair->eatom;
    for (i = 0; i < npair; i++) energy[i] += eatom[i];
  }

  if (bondflag && force->bond) {
    myScalar *eatom = force->bond->eatom;
    for (i = 0; i < nbond; i++) energy[i] += eatom[i];
  }

  if (angleflag && force->angle) {
    myScalar *eatom = force->angle->eatom;
    for (i = 0; i < nbond; i++) energy[i] += eatom[i];
  }

  if (dihedralflag && force->dihedral) {
    myScalar *eatom = force->dihedral->eatom;
    for (i = 0; i < nbond; i++) energy[i] += eatom[i];
  }

  if (improperflag && force->improper) {
    myScalar *eatom = force->improper->eatom;
    for (i = 0; i < nbond; i++) energy[i] += eatom[i];
  }

  if (kspaceflag && force->kspace) {
    myScalar *eatom = force->kspace->eatom;
...
int ComputePEAtom::pack_reverse_comm(int n, int first, myScalar *buf)
{
  int i,m,last;
...
void ComputePEAtom::unpack_reverse_comm(int n, int *list, myScalar *buf)
{
  int i,j,m;
...
myScalar ComputePEAtom::memory_usage()
{
