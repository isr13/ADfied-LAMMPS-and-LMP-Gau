...
class FixBalance : public Fix {
 ...
 private:
  ...
  myScalar thresh,stopthresh;
  char bstr[4];
  int wtflag;                   // 1 for weighted balancing

  myScalar imbnow;                // current imbalance factor
  myScalar imbprev;               // imbalance factor before last rebalancing
  myScalar imbfinal;              // imbalance factor after last rebalancing
  myScalar maxloadperproc;        // max load on any processor
