  ...
  displace(NULL), velocity(NULL)
{
  ...
  if ((mstyle == LINEAR || mstyle == WIGGLE || mstyle == ROTATE) &&
      scaleflag) {
    myScalar xscale = domain->lattice->xlattice;
    myScalar yscale = domain->lattice->ylattice;
    myScalar zscale = domain->lattice->zlattice;
  ...
  if (mstyle == ROTATE) {
    myScalar len = sqrt(axis[0]*axis[0] + axis[1]*axis[1] + axis[2]*axis[2]);
    if (len == 0.0)
...
{
  int flag;
  myScalar ddotr,dx,dy,dz;
  myScalar dtfm,theta_new;
  myScalar xold[3],a[3],b[3],c[3],d[3],disp[3],w[3],ex[3],ey[3],ez[3];
  myScalar inertia_ellipsoid[3],qrotate[4];
  myScalar *quat,*inertia,*shape;

  myScalar delta = (update->ntimestep - time_origin) * dt;
  ...
  // for wiggle: X = X0 + A sin(w*dt)
    ...
    myScalar arg = omega_rotate * delta;
    myScalar sine = sin(arg);
    myScalar cosine = cos(arg);
  ...
  // V = w R0 cross (A cos(w*dt) + B sin(w*dt))
    ...
    myScalar qcosine = cos(0.5*arg);
    myScalar qsine = sin(0.5*arg);
    qrotate[0] = qcosine;
      ...
      if (mask[i] & groupbit) {
        ...
        if (extra_flag) {
          ...
          if (theta_flag && line[i] >= 0.0) {
            theta_new = fmod(ad::passive_value(toriginal[i]+arg),ad::passive_value(MY_2PI));
...
myScalar FixMove::memory_usage()
{
  myScalar bytes = atom->nmax*3 * sizeof(myScalar);
  if (theta_flag) bytes += atom->nmax * sizeof(myScalar);
  if (quat_flag) bytes += atom->nmax*4 * sizeof(myScalar);
  if (displaceflag) bytes += atom->nmax*3 * sizeof(myScalar);
  if (velocityflag) bytes += atom->nmax*3 * sizeof(myScalar);
  return bytes;
}
...
void FixMove::write_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    int size = n * sizeof(myScalar);
    fwrite(&size,sizeof(int),1,fp);
    fwrite(list,sizeof(myScalar),n,fp);
  }
}
...
void FixMove::set_arrays(int i)
{
  ...
  domain->unmap(x[i],image[i],xoriginal[i]);
  myScalar delta = (update->ntimestep - time_origin) * update->dt;
  ...
  if (mstyle == LINEAR) {
    ...
    myScalar a[3],b[3],c[3],d[3],disp[3],ddotr;
    myScalar arg = - omega_rotate * delta;
    myScalar sine = sin(arg);
    myScalar cosine = cos(arg);
    d[0] = x[i][0] - point[0];
...
int FixMove::pack_exchange(int i, myScalar *buf)
{
  int n = 0;
  buf[n++] = xoriginal[i][0];
  buf[n++] = xoriginal[i][1];
...
int FixMove::unpack_exchange(int nlocal, myScalar *buf)
{
  int n = 0;
...
int FixMove::pack_restart(int i, myScalar *buf)
{
  int n = 1;
  buf[n++] = xoriginal[i][0];
  buf[n++] = xoriginal[i][1];
