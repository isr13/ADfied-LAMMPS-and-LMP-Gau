...
class Balance : protected Pointers {
 public:
  ...
  void set_weights();
  myScalar imbalance_factor(myScalar &);
  void shift_setup(char *, int, myScalar);
  int shift();
 ...
 private:
  ...
  myScalar thresh;                                    // threshold to perform LB
  int style;                                        // style of LB
  int xflag,yflag,zflag;                            // xyz LB flags
  myScalar *user_xsplit,*user_ysplit,*user_zsplit;    // params for xyz LB
  ...
  myScalar *onecost;           // work vector of counts in one dim
  myScalar *allcost;           // counts for slices in one dim
  myScalar *sum;               // cumulative count for slices in one dim
  myScalar *target;            // target sum for slices in one dim
  myScalar *lo,*hi;            // lo/hi split coords that bound each target
  myScalar *losum,*hisum;      // cumulative counts at lo/hi coords
  int rho;                   // 0 for geometric recursion
                             // 1 for density weighted recursion

  myScalar *proccost;          // particle cost per processor
  myScalar *allproccost;       // proccost summed across procs

  int nimbalance;                 // number of user-specified weight styles
  class Imbalance **imbalances;   // list of Imb classes, one per weight style
  myScalar *weight;                 // ptr to FixStore weight vector

  FILE *fp;                  // balance output file
  int firststep;

  myScalar imbalance_splits();
  void shift_setup_static(char *);
  void tally(int, int, myScalar *);
  int adjust(int, myScalar *);
  int binary(myScalar, int, myScalar *);
#ifdef BALANCE_DEBUG
  void debug_shift_output(int, int, int, myScalar *);
#endif
};
