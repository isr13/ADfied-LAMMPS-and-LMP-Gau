  ...
  pstyle(NULL), pair(NULL), one(NULL)
{
  ...
  if (npair) {
    ...
    size_vector = npair;
    extvector = 1;
    one = new myScalar[npair];
    vector = new myScalar[npair];
...
myScalar ComputePair::compute_scalar()
{
  ...
  MY_MPI_Allreduce(&eng,&scalar,1,MPI_DOUBLE,MPI_SUM,world);
  return scalar;
}
...
void ComputePair::compute_vector()
{
  ...
  MY_MPI_Allreduce(one,vector,npair,MPI_DOUBLE,MPI_SUM,world);
}
