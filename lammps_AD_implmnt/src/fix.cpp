...
void Fix::ev_tally(int n, int *list, myScalar total, myScalar eng, myScalar *v)
{
  if (eflag_atom) {
    myScalar fraction = eng/total;
    for (int i = 0; i < n; i++)
...
void Fix::v_tally(int n, int *list, myScalar total, myScalar *v)
{
  int m;

  if (vflag_global) {
    myScalar fraction = n/total;
...
void Fix::v_tally(int i, myScalar *v)
{
  if (vflag_global) {
    virial[0] += v[0];
    virial[1] += v[1];
...
void Fix::v_tally(int n, int i, myScalar vn)
{
  if (vflag_global)
    virial[n] += vn;
