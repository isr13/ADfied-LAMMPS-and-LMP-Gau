...
void FixMinimize::add_vector(int n)
{
  ...
  vectors = (myScalar **)
    memory->srealloc(vectors,(nvector+1)*sizeof(myScalar *),"minimize:vectors");
...
myScalar *FixMinimize::request_vector(int m)
{
  return vectors[m];
}
...
void FixMinimize::reset_coords()
{
  ...
  myScalar **x = atom->x;
  myScalar *x0 = vectors[0];
  int nlocal = atom->nlocal;
  myScalar dx,dy,dz,dx0,dy0,dz0;
...
myScalar FixMinimize::memory_usage()
{
  myScalar bytes = 0.0;
  for (int m = 0; m < nvector; m++)
    bytes += atom->nmax*peratom[m]*sizeof(myScalar);
  return bytes;
}
...
int FixMinimize::pack_exchange(int i, myScalar *buf)
{
  int m,iper,nper,ni;
...
int FixMinimize::unpack_exchange(int nlocal, myScalar *buf)
{
  int m,iper,nper,ni;
