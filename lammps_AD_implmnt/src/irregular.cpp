...
/* ----------------------------------------------------------------------
   ...
   n = # of atoms to send
   sizes = # of myScalars for each atom
   proclist = proc to send each atom to (not including self)
   sortflag = flag for sorting order of received messages by proc ID
   return total # of myScalars I will recv (not including self)
------------------------------------------------------------------------- */
...
int Irregular::create_atom(int n, int *sizes, int *proclist, int sortflag)
{
  ...
  MY_MPI_Allreduce(MPI_IN_PLACE,work1,nprocs,MPI_INT,MPI_SUM,world);
  nrecv_proc = work1[me];
#else
#ifdef LAMMPS_RS_ALLREDUCE
  MY_MPI_Allreduce(work1,work2,nprocs,MPI_INT,MPI_SUM,world);
  nrecv_proc = work2[me];
#else
  ...
  // length_send = # of myScalars I send to each proc
  ...
  // sendmax_proc = # of myScalars I send in largest single message
  ...
  // length_recv = # of myScalars each proc sends me
...
void Irregular::exchange_atom(myScalar *sendbuf, int *sizes, myScalar *recvbuf)
{
  int i,m,n,count;
  bigint offset;
  ...
  for (int isend = 0; isend < nsend_proc; isend++) {
    ...
    for (i = 0; i < count; i++) {
      m = index_send[n++];
      memcpy(&dbuf[offset],&sendbuf[offset_send[m]],sizes[m]*sizeof(myScalar));
      offset += sizes[m];
    }
...
{
  bigint bytes = 0;
  bytes += maxsend*sizeof(myScalar);   // buf_send
  bytes += maxrecv*sizeof(myScalar);   // buf_recv
  bytes += maxdbuf*sizeof(myScalar);   // dbuf
  bytes += maxbuf;                   // buf
