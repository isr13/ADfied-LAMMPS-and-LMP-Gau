...
void ComputeGyrationChunk::compute_vector()
{
  ...
  MY_MPI_Allreduce(rg,rgall,nchunk,MPI_DOUBLE,MPI_SUM,world);
...
void ComputeGyrationChunk::compute_array()
{
  ...
  if (nchunk)
    MY_MPI_Allreduce(&rgt[0][0],&rgtall[0][0],nchunk*6,MPI_DOUBLE,MPI_SUM,world);
...
myScalar ComputeGyrationChunk::memory_usage()
{
  myScalar bytes = (bigint) maxchunk * 2 * sizeof(myScalar);
  bytes += (bigint) maxchunk * 2*3 * sizeof(myScalar);
  if (tensor) bytes += (bigint) maxchunk * 2*6 * sizeof(myScalar);
  else bytes += (bigint) maxchunk * 2 * sizeof(myScalar);
  return bytes;
}
