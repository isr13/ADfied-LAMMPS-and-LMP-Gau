...
void ReadRestart::command(int narg, char **arg)
{
    ...
    for (int iproc = me; iproc < multiproc_file; iproc += nprocs) {
      ...
      *ptr = '%';
      ...
      for (int i = 0; i < procsperfile; i++) {
        ...
        fread(buf,sizeof(myScalar),n,fp);
  ...
  if (multiproc || mpiioflag) {
    ...
    if (nextra) {
      ...
      int *count = fix->count;
      myScalar **extra = fix->extra;
      myScalar **atom_extra = atom->extra;
      int nlocal = atom->nlocal;
      for (int i = 0; i < nlocal; i++)
...
void ReadRestart::header(int incompatible)
{
  ...
  while (flag >= 0) {
    ...
    // create new AtomVec class using any stored args
      ...
      read_int();
      read_myScalar_vec(3,domain->boxlo);
    } else if (flag == BOXHI) {
      read_int();
      read_myScalar_vec(3,domain->boxhi);
    } else if (flag == XY) {
      domain->xy = read_myScalar();
    } else if (flag == XZ) {
      domain->xz = read_myScalar();
    } else if (flag == YZ) {
      domain->yz = read_myScalar();

    } else if (flag == SPECIAL_LJ) {
      read_int();
      read_myScalar_vec(3,&force->special_lj[1]);
    } else if (flag == SPECIAL_COUL) {
      read_int();
      read_myScalar_vec(3,&force->special_coul[1]);

    } else if (flag == TIMESTEP) {
      update->dt = read_myScalar();
      ...
      atom->userbinsize = read_myScalar();

    } else if (flag == COMM_MODE) {
      comm->mode = read_int();
    } else if (flag == COMM_CUTOFF) {
      comm->cutghostuser = read_myScalar();
...
void ReadRestart::type_arrays()
{
  ...
  while (flag >= 0) {
    ...
    if (flag == MASS) {
      read_int();
      myScalar *mass = new myScalar[atom->ntypes+1];
      read_myScalar_vec(atom->ntypes,&mass[1]);
      atom->set_mass(mass);
      delete [] mass;
      ...
      if (mpiioflag) {
        ...
        if (me == 0) {
          ...
          if ((nprocs != nprocs_file) && !(atom->nextra_store)) {
            ...
            base_ct += 1;
            bigint base_ByteOffset = base_ct * (perAtomSize * sizeof(myScalar));
            ...
            base_ct -= 1;
            base_ByteOffset -= (perAtomSize * sizeof(myScalar));
            for (; ndx < nprocs; ++ndx) {
              nproc_chunk_offsets[ndx] = current_ByteOffset;
              nproc_chunk_sizes[ndx] = base_ct * perAtomSize;
              current_ByteOffset += base_ByteOffset;
...
/* ----------------------------------------------------------------------
   read a myScalar from restart file and bcast it
------------------------------------------------------------------------- */

myScalar ReadRestart::read_myScalar()
{
  myScalar value;
  if ((me == 0) && (fread(&value,sizeof(myScalar),1,fp) < 1))
    value = 0.0;
...
/* ----------------------------------------------------------------------
   read vector of N myScalars from restart file and bcast them
------------------------------------------------------------------------- */

void ReadRestart::read_myScalar_vec(int n, myScalar *vec)
{
  if (n < 0) error->all(FLERR,"Illegal size myScalar vector read requested");
  if (me == 0) fread(vec,sizeof(myScalar),n,fp);
