...
void ComputeCentroAtom::compute_peratom()
{
  ...
  myScalar xtmp,ytmp,ztmp,delx,dely,delz,rsq,value;
  ...
  myScalar *pairs = new myScalar[npairs];
  ...
  myScalar **x = atom->x;
  int *mask = atom->mask;
  myScalar cutsq = force->pair->cutforce * force->pair->cutforce;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (mask[i] & groupbit) {
      ...
      if (!axes_flag) {
        ...
        myScalar* r1 = &array_atom[i][1];
        myScalar* r2 = &array_atom[i][4];
        myScalar* r3 = &array_atom[i][7];
...
void ComputeCentroAtom::select(int k, int n, myScalar *arr)
{
  int i,ir,j,l,mid;
  myScalar a,tmp;
...
void ComputeCentroAtom::select2(int k, int n, myScalar *arr, int *iarr)
{
...
myScalar ComputeCentroAtom::memory_usage()
{
  myScalar bytes = nmax * sizeof(myScalar);
  if (axes_flag) bytes += size_peratom_cols*nmax * sizeof(myScalar);
  return bytes;
}
