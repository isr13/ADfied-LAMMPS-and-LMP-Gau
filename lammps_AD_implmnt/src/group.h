...
class Group : protected Pointers {
 public:
  ...
  myScalar mass(int);                        // total mass of atoms in group
  myScalar mass(int,int);
  myScalar charge(int);                      // total charge of atoms in group
  myScalar charge(int,int);
  void bounds(int, myScalar *);              // bounds of atoms in group
  void bounds(int, myScalar *, int);
  void xcm(int, myScalar, myScalar *);         // center-of-mass coords of group
  void xcm(int, myScalar, myScalar *, int);
  void vcm(int, myScalar, myScalar *);         // center-of-mass velocity of group
  void vcm(int, myScalar, myScalar *, int);
  void fcm(int, myScalar *);                 // total force on group
  void fcm(int, myScalar *, int);
  myScalar ke(int);                          // kinetic energy of group
  myScalar ke(int, int);
  myScalar gyration(int, myScalar, myScalar *);  // radius-of-gyration of group
  myScalar gyration(int, myScalar, myScalar *, int);
  void angmom(int, myScalar *, myScalar *);    // angular momentum of group
  void angmom(int, myScalar *, myScalar *, int);
  void torque(int, myScalar *, myScalar *);    // torque on group
  void torque(int, myScalar *, myScalar *, int);
  void inertia(int, myScalar *, myScalar [3][3]);     // inertia tensor
  void inertia(int, myScalar *, myScalar [3][3], int);
  void omega(myScalar *, myScalar [3][3], myScalar *);  // angular velocity
