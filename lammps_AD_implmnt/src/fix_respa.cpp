...
myScalar FixRespa::memory_usage()
{
  myScalar bytes = atom->nmax*nlevels*3 * sizeof(myScalar);
  if (store_torque) bytes += atom->nmax*nlevels*3 * sizeof(myScalar);
  return bytes;
}
...
int FixRespa::pack_exchange(int i, myScalar *buf)
{
  int m = 0;
  for (int k = 0; k < nlevels; k++) {
    buf[m++] = f_level[i][k][0];
...
int FixRespa::unpack_exchange(int nlocal, myScalar *buf)
{
  int m = 0;
  for (int k = 0; k < nlevels; k++) {
