  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"origin") == 0) {
      ...
      if (style != CUSTOM)
        error->all(FLERR,
                   "Invalid option in lattice command for non-custom style");
      myScalar x = force->numeric(FLERR,arg[iarg+1]);
      myScalar y = force->numeric(FLERR,arg[iarg+2]);
      myScalar z = force->numeric(FLERR,arg[iarg+3]);
...
void Lattice::setup_transform()
{
  ...
  myScalar determinant = primitive[0][0]*primitive[1][1]*primitive[2][2] +
...
void Lattice::lattice2box(myScalar &x, myScalar &y, myScalar &z)
{
  myScalar x1 = primitive[0][0]*x + primitive[0][1]*y + primitive[0][2]*z;
  myScalar y1 = primitive[1][0]*x + primitive[1][1]*y + primitive[1][2]*z;
  myScalar z1 = primitive[2][0]*x + primitive[2][1]*y + primitive[2][2]*z;

  x1 *= scale;
  y1 *= scale;
  z1 *= scale;

  myScalar xnew = rotaterow[0][0]*x1 + rotaterow[0][1]*y1 + rotaterow[0][2]*z1;
  myScalar ynew = rotaterow[1][0]*x1 + rotaterow[1][1]*y1 + rotaterow[1][2]*z1;
  myScalar znew = rotaterow[2][0]*x1 + rotaterow[2][1]*y1 + rotaterow[2][2]*z1;
...
void Lattice::box2lattice(myScalar &x, myScalar &y, myScalar &z)
{
  x -= xlattice*origin[0];
  y -= ylattice*origin[1];
  z -= zlattice*origin[2];

  myScalar x1 = rotatecol[0][0]*x + rotatecol[0][1]*y + rotatecol[0][2]*z;
  myScalar y1 = rotatecol[1][0]*x + rotatecol[1][1]*y + rotatecol[1][2]*z;
  myScalar z1 = rotatecol[2][0]*x + rotatecol[2][1]*y + rotatecol[2][2]*z;
...
void Lattice::add_basis(myScalar x, myScalar y, myScalar z)
{
...
myScalar Lattice::dot(myScalar *x, myScalar *y)
{
...
void Lattice::cross(myScalar *x, myScalar *y, myScalar *z)
{
  z[0] = x[1]*y[2] - x[2]*y[1];
  z[1] = x[2]*y[0] - x[0]*y[2];
  z[2] = x[0]*y[1] - x[1]*y[0];
...
void Lattice::bbox(int flag, myScalar x, myScalar y, myScalar z,
                   myScalar &xmin, myScalar &ymin, myScalar &zmin,
                   myScalar &xmax, myScalar &ymax, myScalar &zmax)
{
