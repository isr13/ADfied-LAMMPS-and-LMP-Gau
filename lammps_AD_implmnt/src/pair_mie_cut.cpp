...
void PairMIECut::compute(int eflag, int vflag)
{
  ...
  myScalar xtmp,ytmp,ztmp,delx,dely,delz,evdwl,fpair;
  myScalar rsq,r2inv,rgamR,rgamA,forcemie,factor_mie;
  ...
  myScalar *special_mie = force->special_lj;
...
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  myScalar xtmp,ytmp,ztmp,delx,dely,delz,fpair;
  myScalar rsq,r2inv,rgamR,rgamA,forcemie,factor_mie,rsw;
  int *ilist,*jlist,*numneigh,**firstneigh;
  ...
  myScalar cut_out_on = cut_respa[0];
  myScalar cut_out_off = cut_respa[1];

  myScalar cut_out_diff = cut_out_off - cut_out_on;
  myScalar cut_out_on_sq = cut_out_on*cut_out_on;
  myScalar cut_out_off_sq = cut_out_off*cut_out_off;
...
void PairMIECut::compute_middle()
{
  ...
  myScalar cut_in_off = cut_respa[0];
  myScalar cut_in_on = cut_respa[1];
  myScalar cut_out_on = cut_respa[2];
  myScalar cut_out_off = cut_respa[3];

  myScalar cut_in_diff = cut_in_on - cut_in_off;
  myScalar cut_out_diff = cut_out_off - cut_out_on;
  myScalar cut_in_off_sq = cut_in_off*cut_in_off;
  myScalar cut_in_on_sq = cut_in_on*cut_in_on;
  myScalar cut_out_on_sq = cut_out_on*cut_out_on;
  myScalar cut_out_off_sq = cut_out_off*cut_out_off;
...
void PairMIECut::compute_outer(int eflag, int vflag)
{
  ...
  myScalar cut_in_off = cut_respa[2];
  myScalar cut_in_on = cut_respa[3];
...
void PairMIECut::coeff(int narg, char **arg)
{
  ...
  myScalar epsilon_one = force->numeric(FLERR,arg[2]);
  myScalar sigma_one = force->numeric(FLERR,arg[3]);
  myScalar gamR_one = force->numeric(FLERR,arg[4]);
  myScalar gamA_one = force->numeric(FLERR,arg[5]);

  myScalar cut_one = cut_global;
...
myScalar PairMIECut::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
  ...
  if (offset_flag && (cut[i][j] > 0.0)) {
    myScalar ratio = sigma[i][j] / cut[i][j];
  ...
  if (tail_flag) {
    ...
    myScalar count[2],all[2];
    count[0] = count[1] = 0.0;
    for (int k = 0; k < nlocal; k++) {
      if (type[k] == i) count[0] += 1.0;
      if (type[k] == j) count[1] += 1.0;
    }
    MY_MPI_Allreduce(count,all,2,MPI_DOUBLE,MPI_SUM,world);

    myScalar siggamA = pow(sigma[i][j],gamA[i][j]);
    myScalar siggamR = pow(sigma[i][j],gamR[i][j]);
    myScalar rcgamA = pow(cut[i][j],(gamA[i][j]-3.0));
    myScalar rcgamR = pow(cut[i][j],(gamR[i][j]-3.0));
...
void PairMIECut::write_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        fwrite(&epsilon[i][j],sizeof(myScalar),1,fp);
        fwrite(&sigma[i][j],sizeof(myScalar),1,fp);
        fwrite(&gamR[i][j],sizeof(myScalar),1,fp);
        fwrite(&gamA[i][j],sizeof(myScalar),1,fp);
        fwrite(&cut[i][j],sizeof(myScalar),1,fp);
      }
    }
}
...
void PairMIECut::read_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          fread(&epsilon[i][j],sizeof(myScalar),1,fp);
          fread(&sigma[i][j],sizeof(myScalar),1,fp);
          fread(&gamR[i][j],sizeof(myScalar),1,fp);
          fread(&gamA[i][j],sizeof(myScalar),1,fp);
          fread(&cut[i][j],sizeof(myScalar),1,fp);
        }
...
{
  fwrite(&cut_global,sizeof(myScalar),1,fp);
...
void PairMIECut::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
    fread(&cut_global,sizeof(myScalar),1,fp);
...
myScalar PairMIECut::single(int /*i*/, int /*j*/, int itype, int jtype, myScalar rsq,
                           myScalar /*factor_coul*/, myScalar factor_mie,
                           myScalar &fforce)
{
  myScalar r2inv,rgamR,rgamA,forcemie,phimie;
