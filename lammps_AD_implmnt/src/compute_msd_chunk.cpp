...
myScalar ComputeMSDChunk::memory_usage()
{
  myScalar bytes = (bigint) nchunk * 2 * sizeof(myScalar);
  bytes += (bigint) nchunk * 2*3 * sizeof(myScalar);
  bytes += (bigint) nchunk * 4 * sizeof(myScalar);
  return bytes;
}
