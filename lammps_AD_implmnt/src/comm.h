...
class Comm : protected Pointers {
 public:
  ...
  myScalar cutghost[3];               // cutoffs used for acquiring ghost atoms
  myScalar cutghostuser;              // user-specified ghost cutoff (mode == 0)
  myScalar *cutusermulti;            // per type user ghost cutoff (mode == 1)
  ...
  myScalar *xsplit,*ysplit,*zsplit;   // fractional (0-1) sub-domain sizes
  int ***grid2proc;                 // which proc owns i,j,k loc in 3d grid

  // public settings specific to layout = TILED

  int rcbnew;                       // 1 if just reset by rebalance, else 0
  myScalar mysplit[3][2];             // fractional (0-1) bounds of my sub-domain
  myScalar rcbcutfrac;                // fractional RCB cut by this proc
  ...
  myScalar get_comm_cutoff();     // determine communication cutoff
  ...
  virtual void forward_comm_array(int, myScalar **) = 0;
  virtual int exchange_variable(int, myScalar *, myScalar *&) = 0;
  int binary(myScalar, int, myScalar *);

  // map a point to a processor, based on current decomposition

  virtual void coord2proc_setup() {}
  virtual int coord2proc(myScalar *, int &, int &, int &);
