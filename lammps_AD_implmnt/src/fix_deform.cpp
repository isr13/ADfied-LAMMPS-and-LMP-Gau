...
void FixDeform::init()
{
  ...
  myScalar delt = (update->endstep - update->beginstep) * update->dt;
...
void FixDeform::end_of_step()
{
  ...
  for (i = 0; i < 3; i++) {
    if (set[i].style == NONE) {
      ...
      myScalar delt = (update->ntimestep - update->beginstep) * update->dt;
      ...
      myScalar del = input->variable->compute_equal(set[i].hvar);
  ...
  if (triclinic) {
    ...
    for (i = 3; i < 6; i++) {
      if (set[i].style == NONE) {
        ...
        myScalar delta_tilt = input->variable->compute_equal(set[i].hvar);
      ...
      myScalar denom = set[idenom].hi_target - set[idenom].lo_target;

      myScalar current = h[i]/h[idenom];
  ...
  if (triclinic && flipflag) {
    myScalar xprd = set[0].hi_target - set[0].lo_target;
    myScalar yprd = set[1].hi_target - set[1].lo_target;
    myScalar xprdinv = 1.0 / xprd;
    myScalar yprdinv = 1.0 / yprd;
...
myScalar FixDeform::memory_usage()
{
  myScalar bytes = 0.0;
