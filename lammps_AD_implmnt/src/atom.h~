...
class Atom : protected Pointers {
 public:
  ...
  int *spin;
  myScalar *eradius,*ervel,*erforce,*ervelforce;
  myScalar *cs,*csforce,*vforce;
  int *etag;

  // USER-SPH package

  myScalar *rho,*drho,*e,*de,*cv;
  myScalar **vest;

  // USER-SMD package

  myScalar *contact_radius;
  myScalar **smd_data_9;
  myScalar **smd_stress;
  myScalar *eff_plastic_strain;
  myScalar *eff_plastic_strain_rate;
  myScalar *damage;

  // USER-DPD package

  myScalar *uCond,*uMech,*uChem,*uCGnew,*uCG;
  myScalar *duChem;
  myScalar *dpdTheta;
  int nspecies_dpd;
  ...
  myScalar **cc, **cc_flux;        // cc = chemical concentration
  myScalar *edpd_temp,*edpd_flux;  // temperature and heat flux
  myScalar *edpd_cv;               // heat capacity
  int cc_species;
  ...
  myScalar userbinsize;       // requested sort bin size
  ...
  void data_atoms(int, char *, tagint, tagint, int, int, myScalar *);
  ...
  void set_mass(const char *, int, int, myScalar);
  void set_mass(const char *, int, int, char **);
  void set_mass(myScalar *);
  void check_mass(const char *, int);

  int radius_consistency(int, myScalar &);
  int shape_consistency(int, myScalar &, myScalar &, myScalar &);
 ...
 protected:
  ...
  myScalar bininvx,bininvy,bininvz; // inverse actual bin sizes
  myScalar bboxlo[3],bboxhi[3];     // bounding box of my sub-domain
