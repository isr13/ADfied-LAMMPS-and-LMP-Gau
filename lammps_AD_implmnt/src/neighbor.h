...
class Neighbor : protected Pointers {
 public:
  ...
  myScalar skin;                     // skin distance
  myScalar cutneighmin;              // min neighbor cutoff for all type pairs
  myScalar cutneighmax;              // max neighbor cutoff for all type pairs
  myScalar cutneighmaxsq;            // cutneighmax squared
  myScalar **cutneighsq;             // neighbor cutneigh sq for each type pair
  myScalar **cutneighghostsq;        // cutneigh sq for each ghost type pair
  myScalar *cuttype;                 // for each type, max neigh cut w/ others
  myScalar *cuttypesq;               // cuttype squared
  myScalar cut_inner_sq;             // outer cutoff for inner neighbor list
  myScalar cut_middle_sq;            // outer cutoff for middle neighbor list
  myScalar cut_middle_inside_sq;     // inner cutoff for middle neighbor list

  int binsizeflag;                 // user-chosen bin size
  myScalar binsize_user;             // set externally by some accelerator pkgs
  ...
  myScalar *bboxlo,*bboxhi;          // ptrs to full domain bounding box
 ...
 protected:
  ...
  myScalar triggersq;                // trigger = build when atom moves this dist

  myScalar **xhold;                      // atom coords at last neighbor build
  int maxhold;                         // size of xhold array

  int boxcheck;                        // 1 if need to store box size
  myScalar boxlo_hold[3],boxhi_hold[3];  // box size at last neighbor build
  myScalar corners_hold[8][3];           // box corners at last neighbor build
  myScalar (*corners)[3];                // ptr to 8 corners of triclinic box

  myScalar inner[2],middle[2];       // rRESPA cutoffs for extra lists
