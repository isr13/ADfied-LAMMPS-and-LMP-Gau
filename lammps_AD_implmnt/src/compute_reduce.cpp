  ...
  owner(NULL), idregion(NULL), varatom(NULL)
{
  ...
  if (nvalues == 1) {
    ...
    vector = new myScalar[size_vector];
    onevec = new myScalar[size_vector];
...
myScalar ComputeReduce::compute_scalar()
{
  invoked_scalar = update->ntimestep;

  myScalar one = compute_one(0,-1);

  if (mode == SUM || mode == SUMSQ) {
    MY_MPI_Allreduce(&one,&scalar,1,MPI_DOUBLE,MPI_SUM,world);
  } else if (mode == MINN) {
    MY_MPI_Allreduce(&one,&scalar,1,MPI_DOUBLE,MPI_MIN,world);
  } else if (mode == MAXX) {
    MY_MPI_Allreduce(&one,&scalar,1,MPI_DOUBLE,MPI_MAX,world);
...
void ComputeReduce::compute_vector()
{
  ...
  if (mode == SUM || mode == SUMSQ) {
    for (int m = 0; m < nvalues; m++)
      MY_MPI_Allreduce(&onevec[m],&vector[m],1,MPI_DOUBLE,MPI_SUM,world);

  } else if (mode == MINN) {
    if (!replace) {
      for (int m = 0; m < nvalues; m++)
        MY_MPI_Allreduce(&onevec[m],&vector[m],1,MPI_DOUBLE,MPI_MIN,world);
      ...
      for (int m = 0; m < nvalues; m++)
        if (replace[m] < 0) {
          ...
          pairme.proc = me;
          MY_MPI_Allreduce(&pairme,&pairall,1,MPI_DOUBLE_INT,MPI_MINLOC,world);
    ...
    if (!replace) {
      for (int m = 0; m < nvalues; m++)
        MY_MPI_Allreduce(&onevec[m],&vector[m],1,MPI_DOUBLE,MPI_MAX,world);
      ...
      for (int m = 0; m < nvalues; m++)
        if (replace[m] < 0) {
          pairme.value = onevec[m];
          pairme.proc = me;
          MY_MPI_Allreduce(&pairme,&pairall,1,MPI_DOUBLE_INT,MPI_MAXLOC,world);
          vector[m] = pairall.value;
          owner[m] = pairall.proc;
        }
      for (int m = 0; m < nvalues; m++)
...
myScalar ComputeReduce::compute_one(int m, int flag)
{
  int i;
  ...
  // invoke compute if not previously invoked
    ...
    if (flavor[m] == PERATOM) {
      ...
      if (aidx == 0) {
        myScalar *comp_vec = compute->vector_atom;
        int n = nlocal;
        if (flag < 0) {
          for (i = 0; i < n; i++)
        ...
        myScalar **carray_atom = compute->array_atom;
        int n = nlocal;
        int aidxm1 = aidx - 1;
        if (flag < 0) {
          for (i = 0; i < n; i++)
      ...
      if (aidx == 0) {
        myScalar *comp_vec = compute->vector_local;
        ...
        myScalar **carray_local = compute->array_local;
        int n = compute->size_local_rows;
        int aidxm1 = aidx - 1;
        if (flag < 0)
          for (i = 0; i < n; i++)
  ...
  // access fix fields, check if fix frequency is a match
    ...
    if (flavor[m] == PERATOM) {
      if (aidx == 0) {
        myScalar *fix_vector = fix->vector_atom;
        int n = nlocal;
        if (flag < 0) {
          for (i = 0; i < n; i++)
        ...
        myScalar **fix_array = fix->array_atom;
        int aidxm1 = aidx - 1;
        if (flag < 0) {
          for (i = 0; i < nlocal; i++)
      ...
      if (aidx == 0) {
        myScalar *fix_vector = fix->vector_local;
        ...
        myScalar **fix_array = fix->array_local;
        int n = fix->size_local_rows;
        int aidxm1 = aidx - 1;
        if (flag < 0)
          for (i = 0; i < n; i++)
...
bigint ComputeReduce::count(int m)
{
  ...
  if (which[m] == X || which[m] == V || which[m] == F)
    ...
    if (flavor[m] == PERATOM) {
      ...
      bigint ncountall;
      MY_MPI_Allreduce(&ncount,&ncountall,1,MPI_LMP_BIGINT,MPI_SUM,world);
      return ncountall;
    }
  } else if (which[m] == FIX) {
    Fix *fix = modify->fix[vidx];
...
void ComputeReduce::combine(myScalar &one, myScalar two, int i)
{
...
myScalar ComputeReduce::memory_usage()
{
  myScalar bytes = maxatom * sizeof(myScalar);
  return bytes;
}
