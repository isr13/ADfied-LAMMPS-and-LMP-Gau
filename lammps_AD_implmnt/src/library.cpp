...
void lammps_extract_box(void *ptr, myScalar *boxlo, myScalar *boxhi,
                        myScalar *xy, myScalar *yz, myScalar *xz,
...
/* ----------------------------------------------------------------------
   ...
   for global data, returns a pointer to the
     ...
     caller should cast it to (myScalar *) for a scalar or vector
     caller should cast it to (myScalar **) for an array
   for per-atom or local vector/array data, returns a pointer to the
     compute's internal data structure for the entity
     caller should cast it to (myScalar *) for a vector
     caller should cast it to (myScalar **) for an array
...
/* ----------------------------------------------------------------------
   ...
   for global data, returns a pointer to a memory location
     ...
     which the caller can cast to a (myScalar *) which points to the value
   ...
   IMPORTANT: for global data,
     this function allocates a myScalar to store the value in,
     so the caller must free this memory to avoid a leak, e.g.
       myScalar *dptr = (myScalar *) lammps_extract_fix();
       myScalar value = *dptr;
       lammps_free(dptr);
  ...
  BEGIN_CAPTURE
  {
    ...
    if (style == 0) {
      if (type == 0) {
        ...
        myScalar *dptr = (myScalar *) malloc(sizeof(myScalar));
...
/* ----------------------------------------------------------------------
   ...
   for atom-style variable, returns a pointer to the
     ...
     which the caller can cast to a (myScalar *) which points to the values
   ...
   IMPORTANT: for both equal-style and atom-style variables,
     ...
     e.g. for equal-style variables
       myScalar *dptr = (myScalar *) lammps_extract_variable();
       myScalar value = *dptr;
       lammps_free(dptr);
     e.g. for atom-style variables
       myScalar *vector = (myScalar *) lammps_extract_variable();
...
void *lammps_extract_variable(void *ptr, char *name, char *group)
{
  ...
  BEGIN_CAPTURE
  {
    ...
    if (lmp->input->variable->atomstyle(ivar)) {
      ...
      myScalar *vector = (myScalar *) malloc(nlocal*sizeof(myScalar));
...
/* ----------------------------------------------------------------------
   return the current value of a thermo keyword as a myScalar
...
myScalar lammps_get_thermo(void *ptr, char *name)
{
  LAMMPS *lmp = (LAMMPS *) ptr;
  myScalar dval = 0.0;
...
void lammps_reset_box(void *ptr, myScalar *boxlo, myScalar *boxhi,
                      myScalar xy, myScalar yz, myScalar xz)
{
  LAMMPS *lmp = (LAMMPS *) ptr;
  Domain *domain = lmp->domain;
...
/* ----------------------------------------------------------------------
   ...
   type = 0 for integer values, 1 for myScalar values
                         ...
                         int type, int count, void *data)
{
  ...
  BEGIN_CAPTURE
  {
    ...
    // MY_MPI_Allreduce with MPI_SUM to merge into data, ordered by atom ID
    ...
    if (type == 0) {
      ...
      MY_MPI_Allreduce(copy,data,count*natoms,MPI_INT,MPI_SUM,lmp->world);
      lmp->memory->destroy(copy);

    } else {
      myScalar *vector = NULL;
      myScalar **array = NULL;
      if (count == 1) vector = (myScalar *) vptr;
      else array = (myScalar **) vptr;

      myScalar *copy;
      myScalar *copyReduce;
      lmp->memory->create(copy,count*natoms,"lib/gather:copy");
      lmp->memory->create(copyReduce,count*natoms,"lib/gather:copy");

      for (i = 0; i < count*natoms; i++){
        copy[i] = 0.0;
        copyReduce[i] = 0.0;
      }
      ...
      MY_MPI_Allreduce(copy,copyReduce,count*natoms,MPI_DOUBLE,MPI_SUM,lmp->world);

      // Towara: extract and return passive data from AD data
      lmp->error->warning(FLERR,"converting AD data to double");
      for(int i=0; i<count*natoms; i++){
          reinterpret_cast<double*>(data)[i] = ad::passive_value(copyReduce[i]);
      }

      lmp->memory->destroy(copy);
      lmp->memory->destroy(copyReduce);
    }
  }
  END_CAPTURE
}
                                ...
                                int ndata, int *ids, void *data)
{
  ...
  BEGIN_CAPTURE
  {
    ...
    // use atom ID to insert each atom's values into copy
    // MY_MPI_Allreduce with MPI_SUM to merge into data
    ...
    if (type == 0) {
      ...
      MY_MPI_Allreduce(copy,data,count*ndata,MPI_INT,MPI_SUM,lmp->world);
      lmp->memory->destroy(copy);
      ...
      MY_MPI_Allreduce(copy,data,count*ndata,MPI_DOUBLE,MPI_SUM,lmp->world);
      lmp->memory->destroy(copy);
    }
  }
  END_CAPTURE
...
void lammps_create_atoms(void *ptr, int n, tagint *id, int *type,
                         myScalar *x, myScalar *v, imageint *image,
                         int shrinkexceed)
{
  LAMMPS *lmp = (LAMMPS *) ptr;
  ...
  BEGIN_CAPTURE
  {
    ...
    MY_MPI_Allreduce(&ncurrent,&lmp->atom->natoms,1,MPI_LMP_BIGINT,
                  MPI_SUM,lmp->world);
