...
void FixStore::write_restart(FILE *fp)
{
  ...
  rbuf[0] = nrow;
  rbuf[1] = ncol;
  if (vecflag) memcpy(&rbuf[2],vstore,nrow*sizeof(myScalar));
  else memcpy(&rbuf[2],&astore[0][0],nrow*ncol*sizeof(myScalar));

  int n = nrow*ncol + 2;
  if (comm->me == 0) {
    int size = n * sizeof(myScalar);
    fwrite(&size,sizeof(int),1,fp);
    fwrite(rbuf,sizeof(myScalar),n,fp);
  }
}
...
void FixStore::restart(char *buf)
{
  ...
  myScalar *dbuf = (myScalar *) buf;
  int nrow_restart = static_cast<int>(dbuf[0]);
  int ncol_restart = static_cast<int>(dbuf[1]);
  ...
  int n = nrow*ncol;
  if (vecflag) memcpy(vstore,&dbuf[2],n*sizeof(myScalar));
  else memcpy(&astore[0][0],&dbuf[2],n*sizeof(myScalar));
}
...
int FixStore::pack_exchange(int i, myScalar *buf)
{
  if (disable) return 0;
...
int FixStore::unpack_exchange(int nlocal, myScalar *buf)
{
  if (disable) return 0;
...
int FixStore::pack_restart(int i, myScalar *buf)
{
  if (disable) {
    buf[0] = 0;
    return 1;
...
myScalar FixStore::memory_usage()
{
  myScalar bytes = 0.0;
  if (flavor == GLOBAL) bytes += nrow*ncol * sizeof(myScalar);
  if (flavor == PERATOM) bytes += atom->nmax*nvalues * sizeof(myScalar);
  return bytes;
}
