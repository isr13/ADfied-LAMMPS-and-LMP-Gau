...
void FixWallRegion::init()
{
  ...
  if (style == LJ93) {
    ...
    myScalar rinv = 1.0/cutoff;
    myScalar r2inv = rinv*rinv;
    myScalar r4inv = r2inv*r2inv;
    ...
    myScalar r2inv = 1.0/(cutoff*cutoff);
    myScalar r6inv = r2inv*r2inv*r2inv;
...
{
  int i,m,n;
  myScalar rinv,fx,fy,fz,tooclose;
  myScalar delx, dely, delz, v[6];
...
myScalar FixWallRegion::compute_scalar()
{
  // only sum across procs one time

  if (eflag == 0) {
    MY_MPI_Allreduce(ewall,ewall_all,4,MPI_DOUBLE,MPI_SUM,world);
    eflag = 1;
  }
  return ewall_all[0];
}
...
myScalar FixWallRegion::compute_vector(int n)
{
  // only sum across procs one time
...
void FixWallRegion::lj93(myScalar r)
{
  myScalar rinv = 1.0/r;
  myScalar r2inv = rinv*rinv;
  myScalar r4inv = r2inv*r2inv;
  myScalar r10inv = r4inv*r4inv*r2inv;
...
void FixWallRegion::lj126(myScalar r)
{
  myScalar rinv = 1.0/r;
  myScalar r2inv = rinv*rinv;
  myScalar r6inv = r2inv*r2inv*r2inv;
...
void FixWallRegion::lj1043(myScalar r)
{
  myScalar rinv = 1.0/r;
  myScalar r2inv = rinv*rinv;
  myScalar r4inv = r2inv*r2inv;
...
void FixWallRegion::colloid(myScalar r, myScalar rad)
{
  myScalar new_coeff2 = coeff2*rad*rad*rad;
  myScalar diam = 2.0*rad;

  myScalar rad2 = rad*rad;
  myScalar rad4 = rad2*rad2;
  myScalar rad8 = rad4*rad4;
  myScalar delta2 = rad2 - r*r;
  myScalar rinv = 1.0/delta2;
  myScalar r2inv = rinv*rinv;
  myScalar r4inv = r2inv*r2inv;
  myScalar r8inv = r4inv*r4inv;
  fwall = coeff1*(rad8*rad + 27.0*rad4*rad2*rad*pow(r,2.0)
                  + 63.0*rad4*rad*pow(r,4.0)
                  + 21.0*rad2*rad*pow(r,6.0))*r8inv - new_coeff2*r2inv;

  myScalar r2 = 0.5*diam - r;
  myScalar rinv2 = 1.0/r2;
  myScalar r2inv2 = rinv2*rinv2;
  myScalar r4inv2 = r2inv2*r2inv2;
  myScalar r3 = r + 0.5*diam;
  myScalar rinv3 = 1.0/r3;
  myScalar r2inv3 = rinv3*rinv3;
  myScalar r4inv3 = r2inv3*r2inv3;
...
void FixWallRegion::harmonic(myScalar r)
{
  myScalar dr = cutoff - r;
