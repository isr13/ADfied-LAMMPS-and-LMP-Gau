  ...
  aggregateID(NULL)
{
  ...
  myScalar cutoff = force->numeric(FLERR,arg[3]);
  cutsq = cutoff*cutoff;
  ...
  while (1) {
    ...
    while (1) {
      ...
      for (int ii = 0; ii < inum; ii++) {
        ...
        const myScalar xtmp = x[i][0];
        const myScalar ytmp = x[i][1];
        const myScalar ztmp = x[i][2];
        ...
        for (int jj = 0; jj < jnum; jj++) {
          ...
          const myScalar delx = xtmp - x[j][0];
          const myScalar dely = ytmp - x[j][1];
          const myScalar delz = ztmp - x[j][2];
          const myScalar rsq = delx*delx + dely*dely + delz*delz;
          if (rsq < cutsq) {
...
int ComputeAggregateAtom::pack_forward_comm(int n, int *list, myScalar *buf,
...
void ComputeAggregateAtom::unpack_forward_comm(int n, int first, myScalar *buf)
{
  int i,m,last;
...
int ComputeAggregateAtom::pack_reverse_comm(int n, int first, myScalar *buf)
{
  int i,m,last;
...
void ComputeAggregateAtom::unpack_reverse_comm(int n, int *list, myScalar *buf)
{
  int i,j,m;
...
myScalar ComputeAggregateAtom::memory_usage()
{
  myScalar bytes = nmax * sizeof(myScalar);
  return bytes;
}
