...
void CreateAtoms::command(int narg, char **arg)
{
  ...
  if (mode == MOLECULE) {
    ...
    bigint nmoltotal;
    MY_MPI_Allreduce(&nmolme,&nmoltotal,1,MPI_LMP_BIGINT,MPI_SUM,world);
    ...
    if (molecule_flag) {
      ...
      tagint maxmol;
      MY_MPI_Allreduce(&max,&maxmol,1,MPI_LMP_TAGINT,MPI_MAX,world);
...
{
  myScalar xlo,ylo,zlo,xhi,yhi,zhi,zmid;
  myScalar lamda[3],*coord;
  myScalar *boxlo,*boxhi;
...
void CreateAtoms::add_lattice()
{
  ...
  if ((style == REGION) && domain->regions[nregion]->bboxflag) {
    ...
    const myScalar rxmin = domain->regions[nregion]->extent_xlo;
    const myScalar rxmax = domain->regions[nregion]->extent_xhi;
    const myScalar rymin = domain->regions[nregion]->extent_ylo;
    const myScalar rymax = domain->regions[nregion]->extent_yhi;
    const myScalar rzmin = domain->regions[nregion]->extent_zlo;
    const myScalar rzmax = domain->regions[nregion]->extent_zhi;
  ...
  myScalar xmin,ymin,zmin,xmax,ymax,zmax;
  xmin = ymin = zmin = BIG;
  xmax = ymax = zmax = -BIG;
  ...
  const myScalar * const * const basis = domain->lattice->basis;

  // rough estimate of total time used for create atoms.
  // one inner loop takes about 25ns on a typical desktop CPU core in 2019
  myScalar testimate = 2.5e-8/3600.0; // convert seconds to hours
  testimate *= static_cast<myScalar>(khi-klo+1);
  testimate *= static_cast<myScalar>(jhi-jlo+1);
  testimate *= static_cast<myScalar>(ihi-ilo+1);
  testimate *= static_cast<myScalar>(nbasis);
  myScalar maxestimate = 0.0;
...
void CreateAtoms::add_molecule(myScalar *center, myScalar *quat_user)
{
  int n;
  myScalar r[3],rotmat[3][3],quat[4],xnew[3];
  ...
  if (quat_user) {
    ...
    MathExtra::norm3(r);
    myScalar theta = ranmol->uniform() * MY_2PI;
...
int CreateAtoms::vartest(myScalar *x)
{
  ...
  myScalar value = input->variable->compute_equal(vvar);
