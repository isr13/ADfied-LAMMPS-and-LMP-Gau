...
void AtomVecTri::set_equilateral(int i, myScalar size)
{
  ...
  if (tri[i] < 0) {
    ...
    myScalar *c1 = bonus[nlocal_bonus].c1;
    myScalar *c2 = bonus[nlocal_bonus].c2;
    myScalar *c3 = bonus[nlocal_bonus].c3;
    ...
    myScalar *c1 = bonus[tri[i]].c1;
    myScalar *c2 = bonus[tri[i]].c2;
    myScalar *c3 = bonus[tri[i]].c3;
    myScalar *inertia = bonus[tri[i]].inertia;
    c1[0] = -size/2.0;
    c1[1] = -sqrt(3.0)/2.0 * size / 3.0;
    c1[2] = 0.0;
    c2[0] = size/2.0;
...
int AtomVecTri::pack_comm(int n, int *list, myScalar *buf,
...
int AtomVecTri::pack_comm_vel(int n, int *list, myScalar *buf,
                              int pbc_flag, int *pbc)
{
  int i,j,m;
...
int AtomVecTri::pack_comm_hybrid(int n, int *list, myScalar *buf)
{
  int i,j,m;
  myScalar *quat;
...
void AtomVecTri::unpack_comm(int n, int first, myScalar *buf)
{
  int i,m,last;
  myScalar *quat;
...
void AtomVecTri::unpack_comm_vel(int n, int first, myScalar *buf)
{
  int i,m,last;
  myScalar *quat;
...
int AtomVecTri::unpack_comm_hybrid(int n, int first, myScalar *buf)
{
  int i,m,last;
  myScalar *quat;
...
int AtomVecTri::pack_reverse(int n, int first, myScalar *buf)
{
  int i,m,last;
...
int AtomVecTri::pack_reverse_hybrid(int n, int first, myScalar *buf)
{
  int i,m,last;
...
void AtomVecTri::unpack_reverse(int n, int *list, myScalar *buf)
{
  int i,j,m;
...
int AtomVecTri::unpack_reverse_hybrid(int n, int *list, myScalar *buf)
{
  int i,j,m;
...
int AtomVecTri::pack_border(int n, int *list, myScalar *buf,
                            int pbc_flag, int *pbc)
{
  int i,j,m;
  myScalar dx,dy,dz;
  myScalar *quat,*c1,*c2,*c3,*inertia;
...
int AtomVecTri::pack_border_vel(int n, int *list, myScalar *buf,
                                int pbc_flag, int *pbc)
{
  int i,j,m;
  myScalar dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecTri::pack_border_hybrid(int n, int *list, myScalar *buf)
{
  int i,j,m;
  myScalar *quat,*c1,*c2,*c3,*inertia;
...
void AtomVecTri::unpack_border(int n, int first, myScalar *buf)
{
  int i,j,m,last;
  myScalar *quat,*c1,*c2,*c3,*inertia;
...
void AtomVecTri::unpack_border_vel(int n, int first, myScalar *buf)
{
  int i,j,m,last;
  myScalar *quat,*c1,*c2,*c3,*inertia;
...
int AtomVecTri::unpack_border_hybrid(int n, int first, myScalar *buf)
{
  int i,j,m,last;
  myScalar *quat,*c1,*c2,*c3,*inertia;
...
int AtomVecTri::pack_exchange(int i, myScalar *buf)
{
  int m = 1;
  buf[m++] = x[i][0];
  buf[m++] = x[i][1];
  ...
  if (tri[i] < 0) buf[m++] = ubuf(0).d;
    ...
    myScalar *c1 = bonus[j].c1;
    myScalar *c2 = bonus[j].c2;
    myScalar *c3 = bonus[j].c3;
...
int AtomVecTri::unpack_exchange(myScalar *buf)
{
...
int AtomVecTri::pack_restart(int i, myScalar *buf)
{
  int m = 1;
  buf[m++] = x[i][0];
  buf[m++] = x[i][1];
...
int AtomVecTri::unpack_restart(myScalar *buf)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) {
    grow(0);
...
void AtomVecTri::create_atom(int itype, myScalar *coord)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecTri::data_atom(myScalar *coord, imageint imagetmp, char **values)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecTri::data_atom_bonus(int m, char **values)
{
  ...
  myScalar c2mc1[3],c3mc1[3];
  MathExtra::sub3(c2,c1,c2mc1);
  MathExtra::sub3(c3,c1,c3mc1);
  myScalar size = MAX(MathExtra::len3(c2mc1),MathExtra::len3(c3mc1));
  ...
  myScalar dx = centroid[0] - x[m][0];
  myScalar dy = centroid[1] - x[m][1];
  myScalar dz = centroid[2] - x[m][2];
  myScalar delta = sqrt(dx*dx + dy*dy + dz*dz);
  ...
  myScalar area = 0.5 * MathExtra::len3(norm);
  rmass[m] *= area;
  ...
  myScalar tensor[3][3],evectors[3][3];
  ...
  myScalar ex_space[3],ey_space[3],ez_space[3];
...
void AtomVecTri::pack_data(myScalar **buf)
{
  myScalar c2mc1[3],c3mc1[3],norm[3];
  myScalar area;
...
int AtomVecTri::pack_data_hybrid(int i, myScalar *buf)
{
...
void AtomVecTri::write_data(FILE *fp, int n, myScalar **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecTri::write_data_hybrid(FILE *fp, myScalar *buf)
{
...
void AtomVecTri::pack_vel(myScalar **buf)
{
  int nlocal = atom->nlocal;
  for (int i = 0; i < nlocal; i++) {
    buf[i][0] = ubuf(tag[i]).d;
...
int AtomVecTri::pack_vel_hybrid(int i, myScalar *buf)
{
  buf[0] = omega[i][0];
  buf[1] = omega[i][1];
  buf[2] = omega[i][2];
...
void AtomVecTri::write_vel(FILE *fp, int n, myScalar **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecTri::write_vel_hybrid(FILE *fp, myScalar *buf)
{
