  ...
  group2(NULL), id_orientorder(NULL), normv(NULL)
{
  ...
  if (strcmp(arg[3],"cutoff") == 0) {
    cstyle = CUTOFF;
    myScalar cutoff = force->numeric(FLERR,arg[4]);
    cutsq = cutoff*cutoff;
...
int ComputeCoordAtom::pack_forward_comm(int n, int *list, myScalar *buf,
...
void ComputeCoordAtom::unpack_forward_comm(int n, int first, myScalar *buf)
{
  int i,last,m=0,j;
  last = first + n;
...
myScalar ComputeCoordAtom::memory_usage()
{
