...
static void mpi_timings(const char *label, Timer *t, enum Timer::ttype tt,
                        ...
                        const int me, myScalar time_loop, FILE *scr, FILE *log);
...
void Finish::end(int flag)
{
  ...
  myScalar time,tmp,ave,max,min;
  myScalar time_loop,time_other,cpu_loop;
  ...
  MY_MPI_Allreduce(&nblocal,&atom->natoms,1,MPI_LMP_BIGINT,MPI_SUM,world);
  ...
  if (timer->has_loop()) {
    ...
    MY_MPI_Allreduce(&time_loop,&tmp,1,MPI_DOUBLE,MPI_SUM,world);
    time_loop = tmp/nprocs;
    MY_MPI_Allreduce(&cpu_loop,&tmp,1,MPI_DOUBLE,MPI_SUM,world);
    cpu_loop = tmp/nprocs;
    ...
    if (me == 0) {
      ...
      if ( timeflag && !minflag && !prdflag && !tadflag &&
        ...
        myScalar one_fs = force->femtosecond;
        myScalar t_step = ((myScalar) time_loop) / ((myScalar) update->nsteps);
        myScalar step_t = 1.0/t_step;

        if (strcmp(update->unit_style,"lj") == 0) {
          myScalar tau_day = 24.0*3600.0 / t_step * update->dt / one_fs;
          ...
          myScalar hrs_fs = t_step / update->dt * one_fs / 3600.0;
          myScalar fs_day = 24.0*3600.0 / t_step * update->dt / one_fs;
          const char perf[] =
          ...
          myScalar hrs_ns = t_step / update->dt * 1000000.0 * one_fs / 3600.0;
          myScalar ns_day = 24.0*3600.0 / t_step * update->dt / one_fs/1000000.0;
          const char perf[] =
  ...
  if (prdflag) {
    ...
    MY_MPI_Allreduce(&time,&tmp,1,MPI_DOUBLE,MPI_SUM,world);
    time = tmp/nprocs;
    if (me == 0) {
      if (screen)
  ...
  if (fftflag) {
    ...
    MY_MPI_Allreduce(&time3d,&tmp,1,MPI_DOUBLE,MPI_SUM,world);
    time3d = tmp/nprocs;
    ...
    MY_MPI_Allreduce(&time1d,&tmp,1,MPI_DOUBLE,MPI_SUM,world);
    time1d = tmp/nprocs;

    myScalar time_kspace = timer->get_wall(Timer::KSPACE);
    MY_MPI_Allreduce(&time_kspace,&tmp,1,MPI_DOUBLE,MPI_SUM,world);
    time_kspace = tmp/nprocs;

    myScalar ntotal = 1.0 * force->kspace->nx_pppm *
      force->kspace->ny_pppm * force->kspace->nz_pppm;
    myScalar nflops = 5.0 * ntotal * log(ntotal);

    myScalar fraction,flop3,flop1;
    if (nsteps) {
      if (ad::passive_value(time_kspace)) fraction = time3d/time_kspace*100.0;
      else fraction = 0.0;
  ...
  if (neighflag) {
    ...
    myScalar nall;
    MY_MPI_Allreduce(&tmp,&nall,1,MPI_DOUBLE,MPI_SUM,world);

    int nspec;
    myScalar nspec_all = 0;
    if (atom->molecular == 1) {
      ...
      tmp = nspec;
      MY_MPI_Allreduce(&tmp,&nspec_all,1,MPI_DOUBLE,MPI_SUM,world);
...
void Finish::stats(int n, myScalar *data,
                   myScalar *pave, myScalar *pmax, myScalar *pmin,
                   int nhisto, int *histo)
{
  ...
  int ntotal;
  MY_MPI_Allreduce(&n,&ntotal,1,MPI_INT,MPI_SUM,world);
  myScalar tmp;
  MY_MPI_Allreduce(&ave,&tmp,1,MPI_DOUBLE,MPI_SUM,world);
  ave = tmp/ntotal;
  MY_MPI_Allreduce(&min,&tmp,1,MPI_DOUBLE,MPI_MIN,world);
  min = tmp;
  MY_MPI_Allreduce(&max,&tmp,1,MPI_DOUBLE,MPI_MAX,world);
  max = tmp;

  for (i = 0; i < nhisto; i++) histo[i] = 0;

  myScalar del = max - min;
  for (i = 0; i < n; i++) {
    if (del == 0.0) m = 0;
  ...
  MY_MPI_Allreduce(histo,histotmp,nhisto,MPI_INT,MPI_SUM,world);
...
void mpi_timings(const char *label, Timer *t, enum Timer::ttype tt,
                        ...
                        const int me, myScalar time_loop, FILE *scr, FILE *log)
{
  myScalar tmp, time_max, time_min, time_sq;
  myScalar time = t->get_wall(tt);

  myScalar time_cpu = t->get_cpu(tt);
  ...
  MY_MPI_Allreduce(&time,&time_min,1,MPI_DOUBLE,MPI_MIN,world);
  MY_MPI_Allreduce(&time,&time_max,1,MPI_DOUBLE,MPI_MAX,world);
  time_sq = time*time;
  MY_MPI_Allreduce(&time,&tmp,1,MPI_DOUBLE,MPI_SUM,world);
  time = tmp/nprocs;
  MY_MPI_Allreduce(&time_sq,&tmp,1,MPI_DOUBLE,MPI_SUM,world);
  time_sq = tmp/nprocs;
  MY_MPI_Allreduce(&time_cpu,&tmp,1,MPI_DOUBLE,MPI_SUM,world);
...
{
  const char fmt[] = "%-8s|%- 12.5g|%- 12.5g|%- 12.5g|%6.1f |%6.2f\n";
  myScalar time_min, time_max, time_avg, time_total, time_std;
  ...
  for (int i=0; i < nthreads; ++i) {
    ...
    myScalar tmp=thr->get_time(which);
