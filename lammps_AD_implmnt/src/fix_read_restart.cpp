...
myScalar FixReadRestart::memory_usage()
{
  myScalar bytes = atom->nmax*nextra * sizeof(myScalar);
...
int FixReadRestart::pack_exchange(int i, myScalar *buf)
{
  buf[0] = count[i];
...
int FixReadRestart::unpack_exchange(int nlocal, myScalar *buf)
{
