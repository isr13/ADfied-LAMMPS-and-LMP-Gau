...
class RCB : protected Pointers {
 public:
  ...
  myScalar *lo,*hi;             // final bounding box of my RCB sub-domain
  myScalar cut;                 // single cut (in Tree) owned by this proc
  ...
  RCB(class LAMMPS *);
  ~RCB();
  void compute(int, int, myScalar **, myScalar *, myScalar *, myScalar *);
  void compute_old(int, int, myScalar **, myScalar *, myScalar *, myScalar *);
  ...
  struct Median {
    myScalar totallo,totalhi;   // weight in each half of active partition
    myScalar valuelo,valuehi;   // position of dot(s) nearest to cut
    myScalar wtlo,wthi;         // total weight of dot(s) at that position
    int countlo,counthi;      // # of dots at that position
    int proclo,prochi;        // unique proc who owns a nearest dot
  };

  struct BBox {
    myScalar lo[3],hi[3];       // corner points of a bounding box
  };
 ...
 private:
  ...
  struct Dot {
    myScalar x[3];          // coord of point
    myScalar wt;            // weight of point
  ...
  myScalar bboxlo[3]; // bounding box of final RCB sub-domain
  myScalar bboxhi[3];
