...
myScalar FixTempCSVR::gamdev(const int ia)
{
  int j;
  myScalar am,e,s,v1,v2,x,y;
...
/* -------------------------------------------------------------------
  ...
  (i.e. equivalent to summing the square of the return values of nn
   calls to gasdev)
---------------------------------------------------------------------- */
myScalar FixTempCSVR::sumnoises(int nn) {
  if (nn == 0) {
    return 0.0;
  } else if (nn == 1) {
    const myScalar rr = random->gaussian();
    return rr*rr;
  } else if (nn % 2 == 0) {
...
myScalar FixTempCSVR::resamplekin(myScalar ekin_old, myScalar ekin_new){
  const myScalar tdof = temperature->dof;
  const myScalar c1 = exp(-update->dt/t_period);
  const myScalar c2 = (1.0-c1)*ekin_new/ekin_old/tdof;
  const myScalar r1 = random->gaussian();
  const myScalar r2 = sumnoises(ad::passive_value(tdof - 1.0));

  const myScalar scale = c1 + c2*(r1*r1+r2) + 2.0*r1*sqrt(c1*c2);
  return sqrt(scale);
}
...
void FixTempCSVR::end_of_step()
{
  ...
  const myScalar t_current = temperature->compute_scalar();
  const myScalar efactor = 0.5 * temperature->dof * force->boltz;
  const myScalar ekin_old = t_current * efactor;
  const myScalar ekin_new = t_target * efactor;
...
void FixTempCSVR::reset_target(myScalar t_new)
{
...
myScalar FixTempCSVR::compute_scalar()
{
  return energy;
}
