...
class FixAveHisto : public Fix {
 ...
 protected:
  ...
  char **ids;
  FILE *fp;
  myScalar lo,hi,binsize,bininv;
  int kind,beyond,overwrite;
  long filepos;

  myScalar stats[4],stats_total[4],stats_all[4];
  myScalar **stats_list;

  int nbins;
  myScalar *bin,*bin_total,*bin_all;
  myScalar **bin_list;
  myScalar *coord;
  ...
  void bin_one(myScalar);
  void bin_vector(int, myScalar *, int);
  void bin_atoms(myScalar *, int);
