...
void Molecule::compute_center()
{
  ...
  molradius = 0.0;
  for (int i = 0; i < natoms; i++) {
    myScalar rad = MathExtra::len3(dx[i]);
...
void Molecule::compute_com()
{
  ...
  myScalar rsqmin = BIG;
  for (int i = 0; i < natoms; i++) {
    myScalar rsq = MathExtra::lensq3(dxcom[i]);
    if (rsq < rsqmin) {
      comatom = i;
      rsqmin = rsq;
    }
  ...
  myScalar rsqmax = 0.0;
  for (int i = 0; i < natoms; i++) {
    myScalar dx = x[comatom][0] - x[i][0];
    myScalar dy = x[comatom][1] - x[i][1];
    myScalar dz = x[comatom][2] - x[i][2];
    myScalar rsq = dx*dx + dy*dy + dz*dz;
...
void Molecule::read(int flag)
{
  ...
  while (1) {
    ...
    if (strstr(line,"atoms")) {
      ...
      nwant = 6;
      const myScalar scale5 = sizescale*sizescale*sizescale*sizescale*sizescale;
      itensor[0] *= scale5;
      itensor[1] *= scale5;
      itensor[2] *= scale5;
      itensor[3] *= scale5;
...
/* ----------------------------------------------------------------------
   ...
   pflag = 0/1 for integer/myScalar params
------------------------------------------------------------------------- */
