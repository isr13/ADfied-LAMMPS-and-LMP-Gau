...
myScalar ComputeTempDeform::compute_scalar()
{
  myScalar lamda[3],vstream[3],vthermal[3];
  ...
  myScalar *h_rate = domain->h_rate;
  myScalar *h_ratelo = domain->h_ratelo;
...
void ComputeTempDeform::remove_bias(int i, myScalar *v)
{
  myScalar lamda[3];
  myScalar *h_rate = domain->h_rate;
  myScalar *h_ratelo = domain->h_ratelo;
...
void ComputeTempDeform::remove_bias_thr(int i, myScalar *v, myScalar *b)
{
  myScalar lamda[3];
  myScalar *h_rate = domain->h_rate;
  myScalar *h_ratelo = domain->h_ratelo;
...
void ComputeTempDeform::restore_bias(int /*i*/, myScalar *v)
{
  v[0] += vbias[0];
  v[1] += vbias[1];
  v[2] += vbias[2];
...
void ComputeTempDeform::restore_bias_thr(int /*i*/, myScalar *v, myScalar *b)
{
  v[0] += b[0];
  v[1] += b[1];
  v[2] += b[2];
...
myScalar ComputeTempDeform::memory_usage()
{
  myScalar bytes = 3*maxbias * sizeof(myScalar);
  return bytes;
}
