...
class KSpace : protected Pointers {
  ...
  friend class FixOMP;
 public:
  myScalar energy;                 // accumulated energies
  myScalar energy_1,energy_6;
  myScalar virial[6];              // accumulated virial
  myScalar *eatom,**vatom;         // accumulated per-atom energy/virial
  myScalar e2group;                // accumulated group-group energy
  myScalar f2group[3];             // accumulated group-group force
  ...
  myScalar accuracy;                  // accuracy of KSpace solver (force units)
  myScalar accuracy_absolute;         // user-specified accuracy in force units
  myScalar accuracy_relative;         // user-specified dimensionless accuracy
                                    // accurary = acc_rel * two_charge_force
  myScalar accuracy_real_6;           // real space accuracy for
                                    // dispersion solver (force units)
  myScalar accuracy_kspace_6;         // reciprocal space accuracy for
                                    // dispersion solver (force units)
  int auto_disp_flag;           // use automatic parameter generation for pppm/disp
  myScalar two_charge_force;          // force in user units of two point
                                    // charges separated by 1 Angstrom

  myScalar g_ewald,g_ewald_6;
  ...
  myScalar splittol;                // tolerance for when to truncate splitting
  ...
  void x2lamdaT(myScalar *, myScalar *);
  void lamda2xT(myScalar *, myScalar *);
  void lamda2xvector(myScalar *, myScalar *);
  void kspacebbox(myScalar, myScalar *);
  ...
  virtual int timing(int, myScalar &, myScalar &) {return 0;}
  virtual int timing_1d(int, myScalar &) {return 0;}
  virtual int timing_3d(int, myScalar &) {return 0;}

  virtual int modify_param(int, char **) {return 0;}
  virtual myScalar memory_usage() {return 0.0;}
...
------------------------------------------------------------------------- */
  ...
  myScalar gamma(const myScalar &rho) const
  {
    if (rho <= 1.0) {
      const int split_order = order/2;
      const myScalar rho2 = rho*rho;
      myScalar g = gcons[split_order][0];
      myScalar rho_n = rho2;
...
------------------------------------------------------------------------- */
  ...
  myScalar dgamma(const myScalar &rho) const
  {
    if (rho <= 1.0) {
      const int split_order = order/2;
      const myScalar rho2 = rho*rho;
      myScalar dg = dgcons[split_order][0]*rho;
      myScalar rho_n = rho*rho2;
      for (int n = 1; n < split_order; n++) {
  ...
  myScalar **get_gcons() { return gcons; }
  myScalar **get_dgcons() { return dgcons; }
 ...
 protected:
  ...
  myScalar scale,qqrd2e;
  myScalar qsum,qsqsum,q2;
  myScalar **gcons,**dgcons;          // accumulated per-atom energy/virial
  ...
  myScalar estimate_table_accuracy(myScalar, myScalar);
};
