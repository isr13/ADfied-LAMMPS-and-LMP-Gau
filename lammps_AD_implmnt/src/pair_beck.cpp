...
void PairBeck::compute(int eflag, int vflag)
{
  ...
  myScalar rsq,r5,force_beck,factor_lj;
  myScalar r,rinv;
  myScalar aaij,alphaij,betaij;
  myScalar term1,term1inv,term2,term3,term4,term5,term6;
...
void PairBeck::coeff(int narg, char **arg)
{
  ...
  myScalar AA_one = force->numeric(FLERR,arg[2]);
  myScalar BB_one = force->numeric(FLERR,arg[3]);
  myScalar aa_one = force->numeric(FLERR,arg[4]);
  myScalar alpha_one = force->numeric(FLERR,arg[5]);
  myScalar beta_one = force->numeric(FLERR,arg[6]);
...
myScalar PairBeck::init_one(int i, int j)
{
...
void PairBeck::write_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        fwrite(&AA[i][j],sizeof(myScalar),1,fp);
        fwrite(&BB[i][j],sizeof(myScalar),1,fp);
        fwrite(&aa[i][j],sizeof(myScalar),1,fp);
        fwrite(&alpha[i][j],sizeof(myScalar),1,fp);
        fwrite(&beta[i][j],sizeof(myScalar),1,fp);
...
void PairBeck::read_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          fread(&AA[i][j],sizeof(myScalar),1,fp);
          fread(&BB[i][j],sizeof(myScalar),1,fp);
          fread(&aa[i][j],sizeof(myScalar),1,fp);
          fread(&alpha[i][j],sizeof(myScalar),1,fp);
          fread(&beta[i][j],sizeof(myScalar),1,fp);
...
myScalar PairBeck::single(int /*i*/, int /*j*/, int itype, int jtype,
                                  myScalar rsq,
                                  myScalar /*factor_coul*/, myScalar factor_lj,
                                  myScalar &fforce)
{
  myScalar phi_beck,r,rinv;
  myScalar r5,force_beck;
  myScalar aaij,alphaij,betaij;
  myScalar term1,term1inv,term2,term3,term4,term5,term6;
