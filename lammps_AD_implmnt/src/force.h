...
class Force : protected Pointers {
 public:
  myScalar boltz;                      // Boltzmann constant (eng/degree-K)
  myScalar hplanck;                    // Planck's constant (energy-time)
  myScalar mvv2e;                      // conversion of mv^2 to energy
  myScalar ftm2v;                      // conversion of ft/m to velocity
  myScalar mv2d;                       // conversion of mass/volume to density
  myScalar nktv2p;                     // conversion of NkT/V to pressure
  myScalar qqr2e;                      // conversion of q^2/r to energy
  myScalar qe2f;                       // conversion of qE to force
  myScalar vxmu2f;                     // conversion of vx dynamic-visc to force
  myScalar xxt2kmu;                    // conversion of xx/t to kinematic-visc
  myScalar dielectric;                 // dielectric constant
  myScalar qqrd2e;                     // q^2/r to energy w/ dielectric constant
  myScalar e_mass;                     // electron mass
  myScalar hhmrr2e;                    // conversion of (hbar)^2/(mr^2) to energy
  myScalar mvh2r;                      // conversion of mv/hbar to distance
                                     // hbar = h/(2*pi)
  myScalar angstrom;                   // 1 angstrom in native units
  myScalar femtosecond;                // 1 femtosecond in native units
  myScalar qelectron;                  // 1 electron charge abs() in native units

  myScalar qqr2e_lammps_real;          // different versions of this constant
  myScalar qqr2e_charmm_real;          // used by new CHARMM pair styles
  ...
  myScalar special_lj[4];      // 1-2, 1-3, 1-4 prefactors for LJ
  myScalar special_coul[4];    // 1-2, 1-3, 1-4 prefactors for Coulombics
  ...
  myScalar numeric(const char *, int, char *);
