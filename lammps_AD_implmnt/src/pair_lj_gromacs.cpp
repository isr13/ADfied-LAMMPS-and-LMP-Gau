...
void PairLJGromacs::coeff(int narg, char **arg)
{
  ...
  myScalar cut_inner_one = cut_inner_global;
...
myScalar PairLJGromacs::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
  ...
  myScalar r6inv = 1.0/pow(cut[i][j],6.0);
  myScalar r8inv = 1.0/pow(cut[i][j],8.0);
  myScalar t = cut[i][j] - cut_inner[i][j];
  myScalar t2inv = 1.0/(t*t);
  myScalar t3inv = t2inv/t;
  myScalar t3 = 1.0/t3inv;
  myScalar a6 = (7.0*cut_inner[i][j] - 10.0*cut[i][j])*r8inv*t2inv;
  myScalar b6 = (9.0*cut[i][j] -  7.0*cut_inner[i][j])*r8inv*t3inv;
  myScalar a12 = (13.0*cut_inner[i][j] - 16.0*cut[i][j])*r6inv*r8inv*t2inv;
  myScalar b12 = (15.0*cut[i][j] - 13.0*cut_inner[i][j])*r6inv*r8inv*t3inv;
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        ...
        fwrite(&cut_inner[i][j],sizeof(myScalar),1,fp);
    ...
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          ...
          fread(&cut_inner[i][j],sizeof(myScalar),1,fp);
...
{
  fwrite(&cut_inner_global,sizeof(myScalar),1,fp);
...
void PairLJGromacs::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
    fread(&cut_inner_global,sizeof(myScalar),1,fp);
...
myScalar PairLJGromacs::single(int /*i*/, int /*j*/, int itype, int jtype,
                             myScalar rsq,
                             myScalar /*factor_coul*/, myScalar factor_lj,
                             myScalar &fforce)
{
  myScalar r2inv,r6inv,forcelj,philj;
  myScalar r,t,fswitch,phiswitch;
