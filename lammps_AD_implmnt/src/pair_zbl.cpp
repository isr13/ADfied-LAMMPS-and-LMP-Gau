...
void PairZBL::compute(int eflag, int vflag)
{
  ...
  myScalar rsq,r,t,fswitch,eswitch;
...
myScalar PairZBL::init_one(int i, int j)
{
  if (setflag[i][j] == 0)
...
myScalar PairZBL::single(int /*i*/, int /*j*/, int itype, int jtype, myScalar rsq,
                         myScalar /*dummy1*/, myScalar /*dummy2*/,
                         myScalar &fforce)
{
  myScalar phi,r,t,eswitch,fswitch;
...
myScalar PairZBL::e_zbl(myScalar r, int i, int j) {

  myScalar d1aij = d1a[i][j];
  myScalar d2aij = d2a[i][j];
  myScalar d3aij = d3a[i][j];
  myScalar d4aij = d4a[i][j];
  myScalar zzeij = zze[i][j];
  myScalar rinv = 1.0/r;

  myScalar sum = c1*exp(-d1aij*r);
  sum += c2*exp(-d2aij*r);
  sum += c3*exp(-d3aij*r);
  sum += c4*exp(-d4aij*r);

  myScalar result = zzeij*sum*rinv;
...
myScalar PairZBL::dzbldr(myScalar r, int i, int j) {
  ...
  myScalar e1 = exp(-d1aij*r);
  myScalar e2 = exp(-d2aij*r);
  myScalar e3 = exp(-d3aij*r);
  myScalar e4 = exp(-d4aij*r);
  ...
  myScalar sum_p = -c1*d1aij*e1;
  sum_p -= c2*d2aij*e2;
  sum_p -= c3*d3aij*e3;
  sum_p -= c4*d4aij*e4;

  myScalar result = zzeij*(sum_p - sum*rinv)*rinv;
...
myScalar PairZBL::d2zbldr2(myScalar r, int i, int j) {
  ...
  myScalar sum_p = c1*e1*d1aij;
  sum_p += c2*e2*d2aij;
  sum_p += c3*e3*d3aij;
  sum_p += c4*e4*d4aij;

  myScalar sum_pp = c1*e1*d1aij*d1aij;
  sum_pp += c2*e2*d2aij*d2aij;
  sum_pp += c3*e3*d3aij*d3aij;
  sum_pp += c4*e4*d4aij*d4aij;

  myScalar result = zzeij*(sum_pp + 2.0*sum_p*rinv +
...
void PairZBL::set_coeff(int i, int j, myScalar zi, myScalar zj)
{
  myScalar ainv = (pow(zi,pzbl) + pow(zj,pzbl))/(a0*force->angstrom);
  d1a[i][j] = d1*ainv;
  d2a[i][j] = d2*ainv;
  d3a[i][j] = d3*ainv;
  d4a[i][j] = d4*ainv;
  ...
  myScalar tc = cut_global - cut_inner;
  myScalar fc = e_zbl(cut_global, i, j);
  myScalar fcp = dzbldr(cut_global, i, j);
  myScalar fcpp = d2zbldr2(cut_global, i, j);

  myScalar swa = (-3.0*fcp + tc*fcpp)/(tc*tc);
  myScalar swb = ( 2.0*fcp - tc*fcpp)/(tc*tc*tc);
  myScalar swc = -fc + (tc/2.0)*fcp - (tc*tc/12.0)*fcpp;
