...
void PairMorse::compute(int eflag, int vflag)
{
  ...
  myScalar rsq,r,dr,dexp,factor_lj;
...
void PairMorse::coeff(int narg, char **arg)
{
  ...
  myScalar d0_one = force->numeric(FLERR,arg[2]);
  myScalar alpha_one = force->numeric(FLERR,arg[3]);
  myScalar r0_one = force->numeric(FLERR,arg[4]);
...
myScalar PairMorse::init_one(int i, int j)
{
  ...
  if (offset_flag) {
    myScalar alpha_dr = -alpha[i][j] * (cut[i][j] - r0[i][j]);
...
void PairMorse::write_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        fwrite(&d0[i][j],sizeof(myScalar),1,fp);
        fwrite(&alpha[i][j],sizeof(myScalar),1,fp);
        fwrite(&r0[i][j],sizeof(myScalar),1,fp);
...
void PairMorse::read_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          fread(&d0[i][j],sizeof(myScalar),1,fp);
          fread(&alpha[i][j],sizeof(myScalar),1,fp);
          fread(&r0[i][j],sizeof(myScalar),1,fp);
...
myScalar PairMorse::single(int /*i*/, int /*j*/, int itype, int jtype, myScalar rsq,
                         myScalar /*factor_coul*/, myScalar factor_lj,
                         myScalar &fforce)
{
  myScalar r,dr,dexp,phi;
