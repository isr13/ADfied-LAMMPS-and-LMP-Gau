...
void PairLJGromacsCoulGromacs::compute(int eflag, int vflag)
{
  ...
  myScalar qtmp,xtmp,ytmp,ztmp,delx,dely,delz,evdwl,ecoul,fpair;
  myScalar rsq,r2inv,r6inv,forcecoul,forcelj,factor_coul,factor_lj;
  myScalar r,tlj,tc,fswitch,fswitchcoul,eswitch,ecoulswitch;
  ...
  myScalar *special_coul = force->special_coul;
  myScalar *special_lj = force->special_lj;
...
myScalar PairLJGromacsCoulGromacs::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
  ...
  myScalar cut = MAX(cut_lj,cut_coul);
  ...
  myScalar r6inv = 1.0/pow(cut_lj,6.0);
  myScalar r8inv = 1.0/pow(cut_lj,8.0);
  myScalar t = cut_lj - cut_lj_inner;
  myScalar t2inv = 1.0/(t*t);
  myScalar t3inv = t2inv/t;
  myScalar t3 = 1.0/t3inv;
  myScalar a6 = (7.0*cut_lj_inner - 10.0*cut_lj)*r8inv*t2inv;
  myScalar b6 = (9.0*cut_lj -  7.0*cut_lj_inner)*r8inv*t3inv;
  myScalar a12 = (13.0*cut_lj_inner - 16.0*cut_lj)*r6inv*r8inv*t2inv;
  myScalar b12 = (15.0*cut_lj - 13.0*cut_lj_inner)*r6inv*r8inv*t3inv;
  myScalar c6 = r6inv - t3*(6.0*a6/3.0 + 6.0*b6*t/4.0);
  myScalar c12 = r6inv*r6inv - t3*(12.0*a12/3.0 + 12.0*b12*t/4.0);
  ...
  myScalar r3inv = 1.0/pow(cut_coul,3.0);
  t = cut_coul - cut_coul_inner;
  t2inv = 1.0/(t*t);
  t3inv = t2inv/t;
  myScalar a1 = (2.0*cut_coul_inner - 5.0*cut_coul) * r3inv*t2inv;
  myScalar b1 = (4.0*cut_coul - 2.0*cut_coul_inner) * r3inv*t3inv;
  coulsw1 = a1;
  coulsw2 = b1;
  coulsw3 = -a1/3.0;
  coulsw4 = -b1/4.0;
...
{
  fwrite(&cut_lj_inner,sizeof(myScalar),1,fp);
  fwrite(&cut_lj,sizeof(myScalar),1,fp);
  fwrite(&cut_coul_inner,sizeof(myScalar),1,fp);
  fwrite(&cut_coul,sizeof(myScalar),1,fp);
...
{
  if (comm->me == 0) {
    fread(&cut_lj_inner,sizeof(myScalar),1,fp);
    fread(&cut_lj,sizeof(myScalar),1,fp);
    fread(&cut_coul_inner,sizeof(myScalar),1,fp);
    fread(&cut_coul,sizeof(myScalar),1,fp);
...
myScalar PairLJGromacsCoulGromacs::single(int i, int j, int itype, int jtype,
                                myScalar rsq,
                                myScalar factor_coul, myScalar factor_lj,
                                myScalar &fforce)
{
  myScalar r2inv,r6inv,forcecoul,forcelj,phicoul,philj;
  myScalar r,tlj,tc,fswitch,phiswitch,fswitchcoul,phiswitchcoul;
