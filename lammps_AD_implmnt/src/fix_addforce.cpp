  ...
  // wrap with clear/add
    ...
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        if (evflag) {
          v[0] = xstyle ? xvalue*unwrap[0] : static_cast<myScalar>(0.0);
          v[1] = ystyle ? yvalue*unwrap[1] : static_cast<myScalar>(0.0);
          v[2] = zstyle ? zvalue*unwrap[2] : static_cast<myScalar>(0.0);
          v[3] = xstyle ? xvalue*unwrap[1] : static_cast<myScalar>(0.0);
          v[4] = xstyle ? xvalue*unwrap[2] : static_cast<myScalar>(0.0);
          v[5] = ystyle ? yvalue*unwrap[2] : static_cast<myScalar>(0.0);
          v_tally(i, v);
        }
      }
    }
...
myScalar FixAddForce::compute_scalar()
{
  // only sum across procs one time

  if (force_flag == 0) {
    MY_MPI_Allreduce(foriginal,foriginal_all,4,MPI_DOUBLE,MPI_SUM,world);
    force_flag = 1;
  }
...
myScalar FixAddForce::compute_vector(int n)
{
  // only sum across procs one time
...
myScalar FixAddForce::memory_usage()
{
  myScalar bytes = 0.0;
  if (varflag == ATOM) bytes = maxatom*4 * sizeof(myScalar);
  return bytes;
}
