...
class Variable : protected Pointers {
 ...
 public:
  ...
  char *retrieve(char *);
  myScalar compute_equal(int);
  myScalar compute_equal(char *);
  void compute_atom(int, int, myScalar *, int, int);
  int compute_vector(int, myScalar **);
  void internal_set(int, myScalar);

  tagint int_between_brackets(char *&, int);
  myScalar evaluate_boolean(char *);
 ...
 private:
  ...
  myScalar *dvalue;          // single numeric value for internal variables
  ...
  struct Tree {            // parse tree for atom-style or vector-style vars
    myScalar value;          // single scalar
    myScalar *array;         // per-atom or per-type list of myScalars
  ...
  myScalar evaluate(char *, Tree **, int);
  myScalar collapse_tree(Tree *);
  myScalar eval_tree(Tree *, int);
