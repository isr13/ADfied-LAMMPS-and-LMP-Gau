...
myScalar ComputeGroupGroup::compute_scalar()
{
...
void ComputeGroupGroup::pair_contribution()
{
  ...
  myScalar xtmp,ytmp,ztmp,delx,dely,delz;
  myScalar rsq,eng,fpair,factor_coul,factor_lj;
  ...
  myScalar all[4];
  MY_MPI_Allreduce(one,all,4,MPI_DOUBLE,MPI_SUM,world);
  scalar += all[0];
...
{
  myScalar *vector_kspace = force->kspace->f2group;
  ...
  if (boundaryflag) {
    ...
    myScalar volume = xprd*yprd*zprd*force->kspace->slab_volfactor;
...
void ComputeGroupGroup::kspace_correction()
{
  ...
  myScalar qsqsum_group,qsum_A,qsum_B;
  ...
  myScalar tmp;
  MY_MPI_Allreduce(&qsqsum_group,&tmp,1,MPI_DOUBLE,MPI_SUM,world);
  qsqsum_group = tmp;

  MY_MPI_Allreduce(&qsum_A,&tmp,1,MPI_DOUBLE,MPI_SUM,world);
  qsum_A = tmp;

  MY_MPI_Allreduce(&qsum_B,&tmp,1,MPI_DOUBLE,MPI_SUM,world);
  qsum_B = tmp;

  myScalar g_ewald = force->kspace->g_ewald;

  myScalar scale = 1.0;
  const myScalar qscale = force->qqrd2e * scale;
