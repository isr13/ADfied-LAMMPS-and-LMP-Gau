...
void PairGauss::coeff(int narg, char **arg)
{
  ...
  myScalar b_one = force->numeric(FLERR,arg[3]);
...
myScalar PairGauss::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
    myScalar sign_bi = (b[i][i] >= 0.0) ? 1.0 : -1.0;
    myScalar sign_bj = (b[j][j] >= 0.0) ? 1.0 : -1.0;
    myScalar si = sqrt(0.5/fabs(b[i][i]));
    myScalar sj = sqrt(0.5/fabs(b[j][j]));
    myScalar sij = mix_distance(si, sj);
    b[i][j] = 0.5 / (sij*sij);
    ...
    myScalar sign_ai = (a[i][i] >= 0.0) ? 1.0 : -1.0;
    myScalar sign_aj = (a[j][j] >= 0.0) ? 1.0 : -1.0;
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        ...
        fwrite(&b[i][j],sizeof(myScalar),1,fp);
    ...
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          ...
          fread(&b[i][j],sizeof(myScalar),1,fp);
...
myScalar PairGauss::single(int /*i*/, int /*j*/, int itype, int jtype, myScalar rsq,
