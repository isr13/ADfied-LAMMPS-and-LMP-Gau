...
void PairBuckCoulCut::compute(int eflag, int vflag)
{
  ...
  myScalar rsq,r2inv,r6inv,forcecoul,forcebuck,factor_coul,factor_lj;
  myScalar r,rexp;
...
void PairBuckCoulCut::coeff(int narg, char **arg)
{
  ...
  myScalar c_one = force->numeric(FLERR,arg[4]);

  myScalar cut_lj_one = cut_lj_global;
  myScalar cut_coul_one = cut_coul_global;
...
myScalar PairBuckCoulCut::init_one(int i, int j)
{
  if (setflag[i][j] == 0) error->all(FLERR,"All pair coeffs are not set");

  myScalar cut = MAX(cut_lj[i][j],cut_coul[i][j]);
  ...
  if (offset_flag && (cut_lj[i][j] > 0.0)) {
    myScalar rexp = exp(-cut_lj[i][j]/rho[i][j]);
  ...
  if (tail_flag) {
    ...
    myScalar rho1 = rho[i][j];
    myScalar rho2 = rho1*rho1;
    myScalar rho3 = rho2*rho1;
    myScalar rc = cut_lj[i][j];
    myScalar rc2 = rc*rc;
    myScalar rc3 = rc2*rc;
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        ...
        fwrite(&cut_coul[i][j],sizeof(myScalar),1,fp);
      }
    }
}
    ...
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          ...
          fread(&cut_coul[i][j],sizeof(myScalar),1,fp);
        }
...
void PairBuckCoulCut::write_restart_settings(FILE *fp)
{
  ...
  fwrite(&cut_coul_global,sizeof(myScalar),1,fp);
...
void PairBuckCoulCut::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
    ...
    fread(&cut_coul_global,sizeof(myScalar),1,fp);
...
myScalar PairBuckCoulCut::single(int i, int j, int itype, int jtype,
                               myScalar rsq,
                               myScalar factor_coul, myScalar factor_lj,
                               myScalar &fforce)
{
  myScalar r2inv,r6inv,r,rexp,forcecoul,forcebuck,phicoul,phibuck;
