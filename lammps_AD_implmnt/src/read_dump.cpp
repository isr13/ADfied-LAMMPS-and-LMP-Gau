...
void ReadDump::command(int narg, char **arg)
{
  ...
  if (filereader)
    for (int i = 0; i < nreader; i++)
      tmp += nsnapatoms[i];
  MY_MPI_Allreduce(&tmp,&nsnap_all,1,MPI_LMP_BIGINT,MPI_SUM,world);

  tmp = npurge;
  MY_MPI_Allreduce(&tmp,&npurge_all,1,MPI_LMP_BIGINT,MPI_SUM,world);
  tmp = nreplace;
  MY_MPI_Allreduce(&tmp,&nreplace_all,1,MPI_LMP_BIGINT,MPI_SUM,world);
  tmp = ntrim;
  MY_MPI_Allreduce(&tmp,&ntrim_all,1,MPI_LMP_BIGINT,MPI_SUM,world);
  tmp = nadd;
  MY_MPI_Allreduce(&tmp,&nadd_all,1,MPI_LMP_BIGINT,MPI_SUM,world);
...
void ReadDump::process_atoms()
{
  ...
  for (i = 0; i < nnew; i++) {
    ...
    // NOTE: atom ID in fields is stored as myScalar, not as ubuf
...
void ReadDump::migrate_new_atoms()
{
  ...
  irregular->exchange_data((char *) &fields[0][0],nfield*sizeof(myScalar),
                           (char *) &newfields[0][0]);
...
myScalar ReadDump::xfield(int i, int j)
{
...
myScalar ReadDump::yfield(int i, int j)
{
  if (!scaled) return fields[i][j];
...
myScalar ReadDump::zfield(int i, int j)
{
  if (!scaled) return fields[i][j];
