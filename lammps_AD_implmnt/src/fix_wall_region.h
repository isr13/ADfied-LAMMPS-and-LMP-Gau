...
class FixWallRegion : public Fix {
 ...
 private:
  int style,iregion;
  myScalar epsilon,sigma,cutoff;
  int eflag;
  myScalar ewall[4],ewall_all[4];
  int ilevel_respa;
  char *idregion;

  myScalar coeff1,coeff2,coeff3,coeff4,offset;
  myScalar coeff5,coeff6,coeff7;
  myScalar eng,fwall;

  void lj93(myScalar);
  void lj126(myScalar);
  void lj1043(myScalar);
  void colloid(myScalar, myScalar);
  void harmonic(myScalar);
};
