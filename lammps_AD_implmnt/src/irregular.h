...
class Irregular : protected Pointers {
 ...
 private:
  ...
  int maxsend,maxrecv;              // size of buf send/recv in # of myScalars
  myScalar *buf_send,*buf_recv;       // bufs used in migrate_atoms
  int maxdbuf;                      // size of myScalar buf in bytes
  myScalar *dbuf;                     // myScalar buf for largest single atom send
  ...
  int sendmax_proc;          // # of myScalars/datums in largest send message
  ...
  int *length_send;          // # of myScalars to send to each proc
  int *length_recv;          // # of myScalars to recv from each proc
  ...
  void exchange_atom(myScalar *, int *, myScalar *);
  void destroy_atom();
