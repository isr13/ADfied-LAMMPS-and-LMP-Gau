...
void CommTiled::setup()
{
  ...
  myScalar lo1[3],hi1[3],lo2[3],hi2[3];
  int one,two;
  ...
  for (int idim = 0; idim < dimension; idim++) {
    for (int idir = 0; idir < 2; idir++) {
      ...
      myScalar oboxlo[3],oboxhi[3],sbox[6];
      ...
      for (i = 0; i < noverlap; i++) {
        ...
        memcpy(sendbox[iswap][i],sbox,6*sizeof(myScalar));
      }
...
void CommTiled::borders()
{
  ...
  myScalar xlo,xhi,ylo,yhi,zlo,zhi;
  myScalar *bbox;
  myScalar **x;
  AtomVec *avec = atom->avec;
...
void CommTiled::forward_comm_array(int nsize, myScalar **array)
{
...
int CommTiled::exchange_variable(int n, myScalar * /*inbuf*/, myScalar *& /*outbuf*/)
{
  int nrecv = n;
  return nrecv;
}
...
void CommTiled::box_drop_brick(int idim, myScalar *lo, myScalar *hi, int &indexme)
{
...
void CommTiled::box_drop_tiled(int /*idim*/, myScalar *lo, myScalar *hi, int &indexme)
{
  box_drop_tiled_recurse(lo,hi,0,nprocs-1,indexme);
}

void CommTiled::box_drop_tiled_recurse(myScalar *lo, myScalar *hi,
                                       ...
                                       int &indexme)
{
  ...
  myScalar cut = boxlo[idim] + prd[idim]*rcbinfo[procmid].cutfrac;
...
void CommTiled::box_other_brick(int idim, int idir,
                                int proc, myScalar *lo, myScalar *hi)
{
...
void CommTiled::box_other_tiled(int /*idim*/, int /*idir*/,
                                int proc, myScalar *lo, myScalar *hi)
{
  myScalar (*split)[2] = rcbinfo[proc].mysplit;
...
int CommTiled::point_drop_brick(int idim, myScalar *x)
{
...
int CommTiled::point_drop_tiled(int idim, myScalar *x)
{
  myScalar xnew[3];
...
int CommTiled::point_drop_tiled_recurse(myScalar *x,
                                        int proclower, int procupper)
{
  // end recursion when partition is a single proc
  // return proc
...
int CommTiled::closer_subbox_edge(int idim, myScalar *x)
{
  myScalar deltalo,deltahi;
...
void CommTiled::coord2proc_setup()
{
  ...
  rcbnew = 0;
  RCBinfo rcbone;
  memcpy(&rcbone.mysplit[0][0],&mysplit[0][0],6*sizeof(myScalar));
...
int CommTiled::coord2proc(myScalar *x, int &igx, int &igy, int &igz)
{
