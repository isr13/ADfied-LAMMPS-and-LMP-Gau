...
void ComputeDisplaceAtom::refresh()
{
  ...
  for (int i = 0; i < nlocal; i++)
    if (ad::passive_value(varatom[i])) domain->unmap(x[i],image[i],xoriginal[i]);
}
...
myScalar ComputeDisplaceAtom::memory_usage()
{
  myScalar bytes = nmax*4 * sizeof(myScalar);
  bytes += nvmax * sizeof(myScalar);
  return bytes;
}
