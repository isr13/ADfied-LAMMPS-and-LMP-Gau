...
static myScalar timespec2seconds(char *timespec)
{
  myScalar vals[3];
  char *num;
  int i = 0;
...
static myScalar CPU_Time()
{
  ...
  if (getrusage(RUSAGE_SELF, &ru) == 0) {
    rv = (myScalar) ru.ru_utime.tv_sec;
    rv += (myScalar) ru.ru_utime.tv_usec * 0.000001;
  }
...
{
  myScalar current_cpu=0.0, current_wall=0.0;

  if (_level > NORMAL) current_cpu = CPU_Time();
  current_wall = MPI_Wtime();

  if ((which > TOTAL) && (which < NUM_TIMER)) {
    const myScalar delta_cpu = current_cpu - previous_cpu;
    const myScalar delta_wall = current_wall - previous_wall;
...
myScalar Timer::cpu(enum ttype which)
{
  myScalar current_cpu = CPU_Time();
  return (current_cpu - cpu_array[which]);
}

/* ---------------------------------------------------------------------- */

myScalar Timer::elapsed(enum ttype which)
{
  if (_level == OFF) return 0.0;
  myScalar current_wall = MPI_Wtime();
  return (current_wall - wall_array[which]);
}

/* ---------------------------------------------------------------------- */

void Timer::set_wall(enum ttype which, myScalar newtime)
{
...
void Timer::print_timeout(FILE *fp)
{
  ...
  if (_timeout > 0) {
    ...
    const myScalar d = MPI_Wtime() - timeout_start;
    // remaining timeout in seconds
    myScalar s_tmp = _timeout - d;
    int s = static_cast<int>(s_tmp);
    //int s = _timeout - d;
    // remaining 1/100ths of seconds
    const int hs = 100*(static_cast<int>(_timeout - d) -  s);
...
{
  myScalar walltime = MPI_Wtime() - timeout_start;
...
/* ---------------------------------------------------------------------- */
myScalar Timer::get_timeout_remain()
{
  return (_timeout < 0.0) ? 0.0 : _timeout + ad::passive_value(timeout_start) - MPI_Wtime();
}
...
void Timer::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],timer_style[OFF])           == 0) {
      ...
      ++iarg;
      if (iarg < narg) {
        _timeout = static_cast<int>(timespec2seconds(arg[iarg]));
