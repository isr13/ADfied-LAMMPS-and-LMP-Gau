...
{
  const myScalar * const q = atom->q;
  const int nlocal = atom->nlocal;
  myScalar qsum_local(0.0), qsqsum_local(0.0);
...
#pragma omp parallel for default(none) reduction(+:qsum_local,qsqsum_local)
  ...
  MY_MPI_Allreduce(&qsum_local,&qsum,1,MPI_DOUBLE,MPI_SUM,world);
  MY_MPI_Allreduce(&qsqsum_local,&qsqsum,1,MPI_DOUBLE,MPI_SUM,world);
...
myScalar KSpace::estimate_table_accuracy(myScalar q2_over_sqrt, myScalar spr)
{
  myScalar table_accuracy = 0.0;
  ...
  if (nctb) {
    myScalar empirical_precision[17];
...
void KSpace::x2lamdaT(myScalar *v, myScalar *lamda)
{
  myScalar *h_inv = domain->h_inv;
  myScalar lamda_tmp[3];
...
void KSpace::lamda2xT(myScalar *lamda, myScalar *v)
{
  myScalar h[6];
  h[0] = domain->h[0];
  h[1] = domain->h[1];
...
void KSpace::lamda2xvector(myScalar *lamda, myScalar *v)
{
  myScalar *h = domain->h;
...
void KSpace::kspacebbox(myScalar r, myScalar *b)
{
  myScalar *h = domain->h;
  myScalar lx,ly,lz,xy,xz,yz;
  lx = h[0];
  ly = h[1];
  lz = h[2];
  yz = h[3];
