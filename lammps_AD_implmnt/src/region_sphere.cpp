...
int RegSphere::inside(myScalar x, myScalar y, myScalar z)
{
  myScalar delx = x - xc;
  myScalar dely = y - yc;
  myScalar delz = z - zc;
  myScalar r = sqrt(delx*delx + dely*dely + delz*delz);
...
int RegSphere::surface_interior(myScalar *x, myScalar cutoff)
{
  myScalar delx = x[0] - xc;
  myScalar dely = x[1] - yc;
  myScalar delz = x[2] - zc;
  myScalar r = sqrt(delx*delx + dely*dely + delz*delz);
  if (r > radius || r == 0.0) return 0;

  myScalar delta = radius - r;
  if (delta < cutoff) {
    contact[0].r = delta;
...
int RegSphere::surface_exterior(myScalar *x, myScalar cutoff)
{
  myScalar delx = x[0] - xc;
  myScalar dely = x[1] - yc;
  myScalar delz = x[2] - zc;
...
void RegSphere::velocity_contact_shape(myScalar *vwall, myScalar *xc)
{
  myScalar delx, dely, delz; // Displacement of contact point in x,y,z
