...
void ComputeRDF::init()
{
  ...
  if (cutflag) {
    myScalar skin = neighbor->skin;
    mycutneigh = cutoff_user + skin;

    myScalar cutghost;            // as computed by Neighbor and Comm
    if (force->pair)
...
void ComputeRDF::init_norm()
{
  ...
  MY_MPI_Allreduce(icount,scratch,npairs,MPI_INT,MPI_SUM,world);
  for (i = 0; i < npairs; i++) icount[i] = scratch[i];
  MY_MPI_Allreduce(jcount,scratch,npairs,MPI_INT,MPI_SUM,world);
  for (i = 0; i < npairs; i++) jcount[i] = scratch[i];
  MY_MPI_Allreduce(duplicates,scratch,npairs,MPI_INT,MPI_SUM,world);
...
void ComputeRDF::compute_array()
{
  ...
  myScalar xtmp,ytmp,ztmp,delx,dely,delz,r;
  ...
  MY_MPI_Allreduce(hist[0],histall[0],npairs*nbin,MPI_DOUBLE,MPI_SUM,world);
  ...
  myScalar constant,vfrac,gr,ncoord,rlower,rupper,normfac;

  if (domain->dimension == 3) {
    constant = 4.0*MY_PI / (3.0*domain->xprd*domain->yprd*domain->zprd);

    for (m = 0; m < npairs; m++) {
      normfac = (icount[m] > 0) ? static_cast<myScalar>(jcount[m])
                - static_cast<myScalar>(duplicates[m])/icount[m] : static_cast<myScalar>(0.0);
      ncoord = 0.0;
