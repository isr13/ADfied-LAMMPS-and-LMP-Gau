...
void RCB::compute(int dimension, int n, myScalar **x, myScalar *wt,
                  myScalar *bboxlo, myScalar *bboxhi)
{
  int i,j,k;
  ...
  myScalar wttot,wtlo,wthi,wtsum,wtok,wtupto,wtmax;
  myScalar targetlo,targethi;
  myScalar valuemin,valuemax,valuehalf,valuehalf_select,smaller;
  myScalar tolerance;
  ...
  while (proclower != procupper) {
    ...
    MY_MPI_Allreduce(&wtsum,&wttot,1,MPI_DOUBLE,MPI_SUM,comm);
    if (wt) MY_MPI_Allreduce(&wtmax,&tolerance,1,MPI_DOUBLE,MPI_MAX,comm);
    else tolerance = 1.0;
    ...
    for (dim = 0; dim < dimension; dim++) {
      ...
      while (1) {
        ...
        counters[0]++;
        MY_MPI_Allreduce(&medme,&med,1,med_type,med_op,comm);
...
void RCB::compute_old(int dimension, int n, myScalar **x, myScalar *wt,
                      myScalar *bboxlo, myScalar *bboxhi)
{
  int i,j,k;
  int keep,outgoing,incoming,incoming2;
  ...
  myScalar wttot,wtlo,wthi,wtsum,wtok,wtupto,wtmax;
  myScalar targetlo,targethi;
  myScalar valuemin,valuemax,valuehalf;
  myScalar tolerance;
  MPI_Comm comm,comm_half;
  MPI_Request request,request2;
  Median med,medme;
...
void RCB::check()
{
  ...
  myScalar weight,wtmax,wtmin,wtone,tolerance;

  // check that total # of dots remained the same

  MY_MPI_Allreduce(&ndotorig,&total1,1,MPI_INT,MPI_SUM,world);
  MY_MPI_Allreduce(&ndot,&total2,1,MPI_INT,MPI_SUM,world);
  if (total1 != total2) {
    if (me == 0)
  ...
  MY_MPI_Allreduce(&weight,&wtmin,1,MPI_DOUBLE,MPI_MIN,world);
  MY_MPI_Allreduce(&weight,&wtmax,1,MPI_DOUBLE,MPI_MAX,world);
  MY_MPI_Allreduce(&wtone,&tolerance,1,MPI_DOUBLE,MPI_MAX,world);
...
void RCB::stats(int flag)
{
  ...
  myScalar ave,rsum,rmin,rmax;
  myScalar weight,wttot,wtmin,wtmax;
  ...
  MY_MPI_Allreduce(&weight,&wttot,1,MPI_DOUBLE,MPI_SUM,world);
  MY_MPI_Allreduce(&weight,&wtmin,1,MPI_DOUBLE,MPI_MIN,world);
  MY_MPI_Allreduce(&weight,&wtmax,1,MPI_DOUBLE,MPI_MAX,world);
  ...
  MY_MPI_Allreduce(&counters[0],&sum,1,MPI_INT,MPI_SUM,world);
  MY_MPI_Allreduce(&counters[0],&min,1,MPI_INT,MPI_MIN,world);
  MY_MPI_Allreduce(&counters[0],&max,1,MPI_INT,MPI_MAX,world);
  ave = ((myScalar) sum)/nprocs;
  if (me == 0)
  ...
  MY_MPI_Allreduce(&counters[1],&sum,1,MPI_INT,MPI_SUM,world);
  MY_MPI_Allreduce(&counters[1],&min,1,MPI_INT,MPI_MIN,world);
  MY_MPI_Allreduce(&counters[1],&max,1,MPI_INT,MPI_MAX,world);
  ave = ((myScalar) sum)/nprocs;
  if (me == 0)
  ...
  MY_MPI_Allreduce(&counters[2],&sum,1,MPI_INT,MPI_SUM,world);
  MY_MPI_Allreduce(&counters[2],&min,1,MPI_INT,MPI_MIN,world);
  MY_MPI_Allreduce(&counters[2],&max,1,MPI_INT,MPI_MAX,world);
  ave = ((myScalar) sum)/nprocs;
  if (me == 0)
  ...
  MY_MPI_Allreduce(&counters[3],&sum,1,MPI_INT,MPI_SUM,world);
  MY_MPI_Allreduce(&counters[3],&min,1,MPI_INT,MPI_MIN,world);
  MY_MPI_Allreduce(&counters[3],&max,1,MPI_INT,MPI_MAX,world);
  ave = ((myScalar) sum)/nprocs;
  if (me == 0)
  ...
  MY_MPI_Allreduce(&counters[4],&sum,1,MPI_INT,MPI_SUM,world);
  MY_MPI_Allreduce(&counters[4],&min,1,MPI_INT,MPI_MIN,world);
  MY_MPI_Allreduce(&counters[4],&max,1,MPI_INT,MPI_MAX,world);
  ave = ((myScalar) sum)/nprocs;
  if (me == 0)
  ...
  if (reuse) {
    MY_MPI_Allreduce(&counters[5],&sum,1,MPI_INT,MPI_SUM,world);
    MY_MPI_Allreduce(&counters[5],&min,1,MPI_INT,MPI_MIN,world);
    MY_MPI_Allreduce(&counters[5],&max,1,MPI_INT,MPI_MAX,world);
    ave = ((myScalar) sum)/nprocs;
    if (me == 0)
  ...
  MY_MPI_Allreduce(&counters[6],&sum,1,MPI_INT,MPI_SUM,world);
  MY_MPI_Allreduce(&counters[6],&min,1,MPI_INT,MPI_MIN,world);
  MY_MPI_Allreduce(&counters[6],&max,1,MPI_INT,MPI_MAX,world);
  ave = ((myScalar) sum)/nprocs;
  if (me == 0)
