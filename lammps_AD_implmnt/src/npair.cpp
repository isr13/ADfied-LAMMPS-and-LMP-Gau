...
int NPair::coord2bin(myScalar *x, int &ix, int &iy, int &iz)
{
  if (!std::isfinite(static_cast<long>(x[0])) || !std::isfinite(static_cast<long>(x[1])) || !std::isfinite(static_cast<long>(x[2])))
