...
LAMMPS::~LAMMPS()
{
  ...
  myScalar totalclock = MPI_Wtime() - initclock;
  if ((me == 0) && (screen || logfile)) {
    char outtime[128];
    int seconds = fmod(ad::passive_value(totalclock),60.0);
    totalclock  = (totalclock - seconds) / 60.0;
    int minutes = fmod(ad::passive_value(totalclock),60.0);
    int hours = static_cast<int>((totalclock - minutes) / 60.0);
