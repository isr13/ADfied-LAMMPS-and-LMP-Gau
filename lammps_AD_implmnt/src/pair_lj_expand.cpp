...
void PairLJExpand::coeff(int narg, char **arg)
{
  ...
  myScalar shift_one = force->numeric(FLERR,arg[4]);
...
myScalar PairLJExpand::init_one(int i, int j)
{
  ...
  if (tail_flag) {
    ...
    myScalar rc1 = cut[i][j];
    myScalar rc2  = rc1*rc1;
    myScalar rc3  = rc2*rc1;
    myScalar rc9  = rc3*rc3*rc3;
    myScalar shift1 = shift[i][j];
    myScalar shift2 = shift1*shift1;
    myScalar shift3 = shift2*shift1;
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        ...
        fwrite(&shift[i][j],sizeof(myScalar),1,fp);
    ...
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          ...
          fread(&shift[i][j],sizeof(myScalar),1,fp);
...
myScalar PairLJExpand::single(int /*i*/, int /*j*/, int itype, int jtype, myScalar rsq,
                            myScalar /*factor_coul*/, myScalar factor_lj,
                            myScalar &fforce)
{
  myScalar r,rshift,rshiftsq,r2inv,r6inv,forcelj,philj;
