...
void ComputeReduceChunk::compute_vector()
{
  ...
  if (mode == SUM)
    MY_MPI_Allreduce(vlocal,vglobal,nchunk,MPI_DOUBLE,MPI_SUM,world);
  else if (mode == MINN)
    MY_MPI_Allreduce(vlocal,vglobal,nchunk,MPI_DOUBLE,MPI_MIN,world);
  else if (mode == MAXX)
    MY_MPI_Allreduce(vlocal,vglobal,nchunk,MPI_DOUBLE,MPI_MAX,world);
}
...
void ComputeReduceChunk::compute_array()
{
  ...
  if (mode == SUM)
    MY_MPI_Allreduce(&alocal[0][0],&aglobal[0][0],nchunk*nvalues,
...
void ComputeReduceChunk::compute_one(int m, myScalar *vchunk, int nstride)
{
  ...
  if (which[m] == COMPUTE) {
    ...
    if (argindex[m] == 0) {
      myScalar *vcompute = compute->vector_atom;
      ...
      myScalar **acompute = compute->array_atom;
...
void ComputeReduceChunk::combine(myScalar &one, myScalar two)
{
  if (mode == SUM) one += two;
  else if (mode == MINN) {
    if (two < one) one = two;
...
myScalar ComputeReduceChunk::memory_usage()
{
  myScalar bytes = (bigint) maxatom * sizeof(myScalar);
  if (nvalues == 1) bytes += (bigint) maxchunk * 2 * sizeof(myScalar);
  else bytes += (bigint) maxchunk * nvalues * 2 * sizeof(myScalar);
  return bytes;
}
