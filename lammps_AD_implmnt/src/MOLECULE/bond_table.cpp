...
myScalar BondTable::equilibrium_distance(int i)
{
  return r0[i];
}
...
myScalar BondTable::single(int type, myScalar rsq, int /*i*/, int /*j*/,
                         myScalar &fforce)
{
  myScalar r = sqrt(rsq);
  myScalar u;
...
void BondTable::spline(myScalar *x, myScalar *y, int n,
...
myScalar BondTable::splint(myScalar *xa, myScalar *ya, myScalar *y2a, int n, myScalar x)
{
  int klo,khi,k;
  myScalar h,b,a,y;
...
void BondTable::uf_lookup(int type, myScalar x, myScalar &u, myScalar &f)
{
...
void BondTable::u_lookup(int type, myScalar x, myScalar &u)
{
  if (!std::isfinite(ad::passive_value(x))) {
    error->one(FLERR,"Illegal bond in bond style table");
  }
