...
void PairLJCutTIP4PCut::compute(int eflag, int vflag)
{
  ...
  myScalar qtmp,xtmp,ytmp,ztmp,delx,dely,delz,evdwl,ecoul;
  myScalar rsq,r2inv,r6inv,forcecoul,forcelj,factor_lj,factor_coul;
  ...
  int iH1,iH2,jH1,jH2;
  myScalar cforce;
  myScalar fO[3],fH[3],fd[3],v[6];
  myScalar *x1,*x2,*xH1,*xH2;
...
void PairLJCutTIP4PCut::init_style()
{
  ...
  myScalar theta = force->angle->equilibrium_angle(typeA);
  myScalar blen = force->bond->equilibrium_distance(typeB);
...
myScalar PairLJCutTIP4PCut::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
  ...
  myScalar cut = MAX(cut_lj[i][j],cut_coul+2.0*qdist);
  ...
  if (offset_flag && (cut_lj[i][j] > 0.0)) {
    myScalar ratio = sigma[i][j] / cut_lj[i][j];
  ...
  if (tail_flag) {
    ...
    myScalar sig2 = sigma[i][j]*sigma[i][j];
    myScalar sig6 = sig2*sig2*sig2;
    myScalar rc3 = cut_lj[i][j]*cut_lj[i][j]*cut_lj[i][j];
    myScalar rc6 = rc3*rc3;
    myScalar rc9 = rc3*rc6;
...
void PairLJCutTIP4PCut::write_restart_settings(FILE *fp)
{
  ...
  fwrite(&qdist,sizeof(myScalar),1,fp);
...
void PairLJCutTIP4PCut::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
    ...
    fread(&qdist,sizeof(myScalar),1,fp);
...
void PairLJCutTIP4PCut::compute_newsite(myScalar *xO,  myScalar *xH1,
                                        myScalar *xH2, myScalar *xM)
{
  myScalar delx1 = xH1[0] - xO[0];
  myScalar dely1 = xH1[1] - xO[1];
  myScalar delz1 = xH1[2] - xO[2];

  myScalar delx2 = xH2[0] - xO[0];
  myScalar dely2 = xH2[1] - xO[1];
  myScalar delz2 = xH2[2] - xO[2];
...
myScalar PairLJCutTIP4PCut::memory_usage()
{
  myScalar bytes = maxeatom * sizeof(myScalar);
  bytes += maxvatom*6 * sizeof(myScalar);
  bytes += 2 * nmax * sizeof(myScalar);
  return bytes;
}
