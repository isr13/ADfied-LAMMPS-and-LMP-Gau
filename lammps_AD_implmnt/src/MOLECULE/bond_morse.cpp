...
void BondMorse::coeff(int narg, char **arg)
{
  ...
  myScalar d0_one = force->numeric(FLERR,arg[1]);
  myScalar alpha_one = force->numeric(FLERR,arg[2]);
  myScalar r0_one = force->numeric(FLERR,arg[3]);
...
myScalar BondMorse::equilibrium_distance(int i)
{
  return r0[i];
}
...
{
  fwrite(&d0[1],sizeof(myScalar),atom->nbondtypes,fp);
  fwrite(&alpha[1],sizeof(myScalar),atom->nbondtypes,fp);
...
void BondMorse::read_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    fread(&d0[1],sizeof(myScalar),atom->nbondtypes,fp);
    fread(&alpha[1],sizeof(myScalar),atom->nbondtypes,fp);
...
myScalar BondMorse::single(int type, myScalar rsq, int /*i*/, int /*j*/,
                         myScalar &fforce)
{
  myScalar r = sqrt(rsq);
  myScalar dr = r - r0[type];
  myScalar ralpha = exp(-alpha[type]*dr);
  fforce = 0;
