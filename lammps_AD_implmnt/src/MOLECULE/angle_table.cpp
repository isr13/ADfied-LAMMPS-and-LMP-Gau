...
void AngleTable::compute(int eflag, int vflag)
{
  ...
  myScalar theta,u,mdu; //mdu: minus du, -du/dx=f
...
myScalar AngleTable::equilibrium_angle(int i)
{
  return theta0[i];
}
...
myScalar AngleTable::single(int type, int i1, int i2, int i3)
{
  myScalar **x = atom->x;
...
void AngleTable::spline_table(Table *tb)
{
  ...
  myScalar ep0 = - tb->ffile[0];
  myScalar epn = - tb->ffile[tb->ninput-1];
  ...
  myScalar fp0 = tb->fplo;
  myScalar fpn = tb->fphi;
...
void AngleTable::spline(myScalar *x, myScalar *y, int n,
                       myScalar yp1, myScalar ypn, myScalar *y2)
{
  int i,k;
  myScalar p,qn,sig,un;
  myScalar *u = new myScalar[n];
...
myScalar AngleTable::splint(myScalar *xa, myScalar *ya, myScalar *y2a, int n, myScalar x)
{
  int klo,khi,k;
  myScalar h,b,a,y;
...
void AngleTable::uf_lookup(int type, myScalar x, myScalar &u, myScalar &f)
{
  if (!std::isfinite(ad::passive_value(x))) {
    error->one(FLERR,"Illegal angle in angle style table");
  }

  myScalar fraction,a,b;
...
void AngleTable::u_lookup(int type, myScalar x, myScalar &u)
{
  if (!std::isfinite(ad::passive_value(x))) {
    error->one(FLERR,"Illegal angle in angle style table");
  }
