...
void DihedralMultiHarmonic::compute(int eflag, int vflag)
{
  ...
  myScalar sb1,sb2,sb3,rb1,rb3,c0,b1mag2,b1mag,b2mag2;
  myScalar b2mag,b3mag2,b3mag,ctmp,r12c1,c1mag,r12c2;
  myScalar c2mag,sc1,sc2,s1,s12,c,p,pd,a,a11,a22;
  myScalar a33,a12,a13,a23,sx2,sy2,sz2;
  myScalar s2,sin2;
...
void DihedralMultiHarmonic::coeff(int narg, char **arg)
{
  ...
  myScalar a1_one = force->numeric(FLERR,arg[1]);
  myScalar a2_one = force->numeric(FLERR,arg[2]);
  myScalar a3_one = force->numeric(FLERR,arg[3]);
  myScalar a4_one = force->numeric(FLERR,arg[4]);
  myScalar a5_one = force->numeric(FLERR,arg[5]);
...
{
  fwrite(&a1[1],sizeof(myScalar),atom->ndihedraltypes,fp);
  fwrite(&a2[1],sizeof(myScalar),atom->ndihedraltypes,fp);
  fwrite(&a3[1],sizeof(myScalar),atom->ndihedraltypes,fp);
  fwrite(&a4[1],sizeof(myScalar),atom->ndihedraltypes,fp);
  fwrite(&a5[1],sizeof(myScalar),atom->ndihedraltypes,fp);
}
...
void DihedralMultiHarmonic::read_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    fread(&a1[1],sizeof(myScalar),atom->ndihedraltypes,fp);
    fread(&a2[1],sizeof(myScalar),atom->ndihedraltypes,fp);
    fread(&a3[1],sizeof(myScalar),atom->ndihedraltypes,fp);
    fread(&a4[1],sizeof(myScalar),atom->ndihedraltypes,fp);
    fread(&a5[1],sizeof(myScalar),atom->ndihedraltypes,fp);
  }
