...
void DihedralCharmmfsw::compute(int eflag, int vflag)
{
  ...
  myScalar vb1x,vb1y,vb1z,vb2x,vb2y,vb2z,vb3x,vb3y,vb3z,vb2xm,vb2ym,vb2zm;
  myScalar edihedral,f1[3],f2[3],f3[3],f4[3];
  myScalar ax,ay,az,bx,by,bz,rasq,rbsq,rgsq,rg,rginv,ra2inv,rb2inv,rabinv;
  myScalar df,df1,ddf1,fg,hg,fga,hgb,gaa,gbb;
  myScalar dtfx,dtfy,dtfz,dtgx,dtgy,dtgz,dthx,dthy,dthz;
  myScalar c,s,p,sx2,sy2,sz2;
  int itype,jtype;
  myScalar delx,dely,delz,rsq,r2inv,r6inv,r;
  myScalar forcecoul,forcelj,fpair,ecoul,evdwl;
...
void DihedralCharmmfsw::coeff(int narg, char **arg)
{
  ...
  myScalar weight_one = force->numeric(FLERR,arg[4]);
...
void DihedralCharmmfsw::init_style()
{
  ...
  if (weightflag) {
    ...
    lj14_1 = (myScalar **) force->pair->extract("lj14_1",itmp);
    lj14_2 = (myScalar **) force->pair->extract("lj14_2",itmp);
    lj14_3 = (myScalar **) force->pair->extract("lj14_3",itmp);
    lj14_4 = (myScalar **) force->pair->extract("lj14_4",itmp);
  ...
  myScalar *p_cutljinner = (myScalar *) force->pair->extract("cut_lj_inner",itmp);
  myScalar *p_cutlj = (myScalar *) force->pair->extract("cut_lj",itmp);
  myScalar *p_cutcoul = (myScalar *) force->pair->extract("cut_coul",itmp);
...
{
  fwrite(&k[1],sizeof(myScalar),atom->ndihedraltypes,fp);
  fwrite(&multiplicity[1],sizeof(int),atom->ndihedraltypes,fp);
  fwrite(&shift[1],sizeof(int),atom->ndihedraltypes,fp);
  fwrite(&weight[1],sizeof(myScalar),atom->ndihedraltypes,fp);
...
void DihedralCharmmfsw::read_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    fread(&k[1],sizeof(myScalar),atom->ndihedraltypes,fp);
    fread(&multiplicity[1],sizeof(int),atom->ndihedraltypes,fp);
    fread(&shift[1],sizeof(int),atom->ndihedraltypes,fp);
    fread(&weight[1],sizeof(myScalar),atom->ndihedraltypes,fp);
