...
void ImproperHarmonic::compute(int eflag, int vflag)
{
  ...
  myScalar ss1,ss2,ss3,r1,r2,r3,c0,c1,c2,s1,s2;
  myScalar s12,c,s,domega,a,a11,a22,a33,a12,a13,a23;
  myScalar sx2,sy2,sz2;
...
void ImproperHarmonic::coeff(int narg, char **arg)
{
  ...
  myScalar chi_one = force->numeric(FLERR,arg[2]);
...
{
  fwrite(&k[1],sizeof(myScalar),atom->nimpropertypes,fp);
  fwrite(&chi[1],sizeof(myScalar),atom->nimpropertypes,fp);
}
...
void ImproperHarmonic::read_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    fread(&k[1],sizeof(myScalar),atom->nimpropertypes,fp);
    fread(&chi[1],sizeof(myScalar),atom->nimpropertypes,fp);
  }
