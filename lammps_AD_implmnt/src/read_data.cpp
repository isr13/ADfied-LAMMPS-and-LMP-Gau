...
void ReadData::command(int narg, char **arg)
{
  ...
  if (addflag == APPEND) {
    ...
    MY_MPI_Allreduce(&maxid,&id_offset,1,MPI_LMP_TAGINT,MPI_MAX,world);
    MY_MPI_Allreduce(&maxmol,&mol_offset,1,MPI_LMP_TAGINT,MPI_MAX,world);
  }
...
void ReadData::atoms()
{
  ...
  bigint sum;
  MY_MPI_Allreduce(&n,&sum,1,MPI_LMP_BIGINT,MPI_SUM,world);
...
void ReadData::bonds(int firstpass)
{
  ...
  if (firstpass) {
    ...
    int maxall;
    MY_MPI_Allreduce(&max,&maxall,1,MPI_INT,MPI_MAX,world);
...
/* ----------------------------------------------------------------------
   read all body data
   variable amount of info per body, described by ninteger and nmyScalar
   to find atoms, must build atom map if not a molecular system
...
{
  int m,nchunk,nline,nmax,ninteger,nmyScalar,nword,ncount,onebody,tmp,rv;
  char *eof;
