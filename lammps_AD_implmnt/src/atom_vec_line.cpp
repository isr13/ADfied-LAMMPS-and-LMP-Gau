...
void AtomVecLine::set_length(int i, myScalar value)
{
  if (line[i] < 0) {
    if (value == 0.0) return;
...
int AtomVecLine::pack_comm(int n, int *list, myScalar *buf,
...
int AtomVecLine::pack_comm_vel(int n, int *list, myScalar *buf,
                               int pbc_flag, int *pbc)
{
  int i,j,m;
...
int AtomVecLine::pack_comm_hybrid(int n, int *list, myScalar *buf)
{
  int i,j,m;
...
void AtomVecLine::unpack_comm(int n, int first, myScalar *buf)
{
  int i,m,last;
...
void AtomVecLine::unpack_comm_vel(int n, int first, myScalar *buf)
{
  int i,m,last;
...
int AtomVecLine::unpack_comm_hybrid(int n, int first, myScalar *buf)
{
  int i,m,last;
...
int AtomVecLine::pack_reverse(int n, int first, myScalar *buf)
{
  int i,m,last;
...
int AtomVecLine::pack_reverse_hybrid(int n, int first, myScalar *buf)
{
  int i,m,last;
...
void AtomVecLine::unpack_reverse(int n, int *list, myScalar *buf)
{
  int i,j,m;
...
int AtomVecLine::unpack_reverse_hybrid(int n, int *list, myScalar *buf)
{
  int i,j,m;
...
int AtomVecLine::pack_border(int n, int *list, myScalar *buf,
                             int pbc_flag, int *pbc)
{
  int i,j,m;
  myScalar dx,dy,dz;
...
int AtomVecLine::pack_border_vel(int n, int *list, myScalar *buf,
                                 int pbc_flag, int *pbc)
{
  int i,j,m;
  myScalar dx,dy,dz,dvx,dvy,dvz;
...
int AtomVecLine::pack_border_hybrid(int n, int *list, myScalar *buf)
{
  int i,j,m;
...
void AtomVecLine::unpack_border(int n, int first, myScalar *buf)
{
  int i,j,m,last;
...
void AtomVecLine::unpack_border_vel(int n, int first, myScalar *buf)
{
  int i,j,m,last;
...
int AtomVecLine::unpack_border_hybrid(int n, int first, myScalar *buf)
{
  int i,j,m,last;
...
int AtomVecLine::pack_exchange(int i, myScalar *buf)
{
  int m = 1;
  buf[m++] = x[i][0];
  buf[m++] = x[i][1];
...
int AtomVecLine::unpack_exchange(myScalar *buf)
{
...
int AtomVecLine::pack_restart(int i, myScalar *buf)
{
  int m = 1;
  buf[m++] = x[i][0];
  buf[m++] = x[i][1];
...
int AtomVecLine::unpack_restart(myScalar *buf)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) {
    grow(0);
...
void AtomVecLine::create_atom(int itype, myScalar *coord)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecLine::data_atom(myScalar *coord, imageint imagetmp, char **values)
{
  int nlocal = atom->nlocal;
  if (nlocal == nmax) grow(0);
...
void AtomVecLine::data_atom_bonus(int m, char **values)
{
  ...
  myScalar x1 = utils::numeric(FLERR,values[0],true,lmp);
  myScalar y1 = utils::numeric(FLERR,values[1],true,lmp);
  myScalar x2 = utils::numeric(FLERR,values[2],true,lmp);
  myScalar y2 = utils::numeric(FLERR,values[3],true,lmp);
  myScalar dx = x2 - x1;
  myScalar dy = y2 - y1;
  myScalar length = sqrt(dx*dx + dy*dy);
  ...
  myScalar xc = 0.5*(x1+x2);
  myScalar yc = 0.5*(y1+y2);
  dx = xc - x[m][0];
  dy = yc - x[m][1];
  myScalar delta = sqrt(dx*dx + dy*dy);
...
void AtomVecLine::pack_data(myScalar **buf)
{
  int nlocal = atom->nlocal;
...
int AtomVecLine::pack_data_hybrid(int i, myScalar *buf)
{
...
void AtomVecLine::write_data(FILE *fp, int n, myScalar **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecLine::write_data_hybrid(FILE *fp, myScalar *buf)
{
...
void AtomVecLine::pack_vel(myScalar **buf)
{
  int nlocal = atom->nlocal;
  for (int i = 0; i < nlocal; i++) {
    buf[i][0] = ubuf(tag[i]).d;
...
int AtomVecLine::pack_vel_hybrid(int i, myScalar *buf)
{
  buf[0] = omega[i][0];
  buf[1] = omega[i][1];
  buf[2] = omega[i][2];
...
void AtomVecLine::write_vel(FILE *fp, int n, myScalar **buf)
{
  for (int i = 0; i < n; i++)
...
int AtomVecLine::write_vel_hybrid(FILE *fp, myScalar *buf)
{
