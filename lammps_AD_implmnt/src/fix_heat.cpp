...
{
  int i;
  myScalar heat,ke,massone;
  myScalar vsub[3],vcm[3];
  ...
  myScalar vcmsq = vcm[0]*vcm[0] + vcm[1]*vcm[1] + vcm[2]*vcm[2];
...
myScalar FixHeat::compute_scalar()
{
  myScalar average_scale = scale;
  if (hstyle == ATOM) {
    myScalar scale_sum = 0.0;
    int ncount = 0;
    int *mask = atom->mask;
    myScalar **x = atom->x;
    int nlocal = atom->nlocal;
    ...
    myScalar scale_sum_all = 0.0;
    int ncount_all = 0;
    MY_MPI_Allreduce(&scale_sum,&scale_sum_all,1,MPI_DOUBLE,MPI_SUM,world);
    MY_MPI_Allreduce(&ncount,&ncount_all,1,MPI_INT,MPI_SUM,world);
    if (ncount_all == 0) average_scale = 0.0;
    else average_scale = scale_sum_all/static_cast<myScalar>(ncount_all);
  }
...
myScalar FixHeat::memory_usage()
{
  myScalar bytes = 0.0;
  if (hstyle == ATOM) bytes = atom->nmax*2 * sizeof(myScalar);
  return bytes;
}
