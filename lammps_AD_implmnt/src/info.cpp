...
void Info::command(int narg, char **arg)
{
  ...
  if (flags & MEMORY) {
    ...
    fprintf(out,"Non-shared memory use: %.4g Mbyte\n",
            (myScalar)pmc.PrivateUsage/1048576.0);
    fprintf(out,"Maximum working set size: %.4g Mbyte\n",
            (myScalar)pmc.PeakWorkingSetSize/1048576.0);
#else
#if defined(__linux)
    struct mallinfo mi;
    mi = mallinfo();
    fprintf(out,"Current reserved memory pool size: %.4g Mbyte\n",
            (myScalar)mi.uordblks/1048576.0+(myScalar)mi.hblkhd/1048576.0);
#endif
    struct rusage ru;
    if (getrusage(RUSAGE_SELF, &ru) == 0) {
      fprintf(out,"Maximum resident set size: %.4g Mbyte\n",
              (myScalar)ru.ru_maxrss/1024.0);
    }
#endif
  }
  ...
  if (flags & SYSTEM) {
    ...
    if (atom->molecular > 0) {
      ...
      const myScalar * const special_lj   = force->special_lj;
      const myScalar * const special_coul = force->special_coul;
  ...
  if (flags & TIME) {
    myScalar wallclock = MPI_Wtime() - lmp->initclock;
    myScalar cpuclock = 0.0;
    ...
    struct rusage ru;
    if (getrusage(RUSAGE_SELF, &ru) == 0) {
      cpuclock  = (myScalar) ru.ru_utime.tv_sec;
      cpuclock += (myScalar) ru.ru_utime.tv_usec * 0.000001;
    }
#endif /* ! _WIN32 */

    int cpuh,cpum,cpus,wallh,wallm,walls;
    cpus = fmod(ad::passive_value(cpuclock),60.0);
    cpuclock = (cpuclock - cpus);
    cpuclock = cpuclock / 60.0;
    cpum = fmod(ad::passive_value(cpuclock),60.0);
    cpuh = static_cast<int>((cpuclock - cpum)/60.0);
    walls = fmod(ad::passive_value(wallclock),60.0);
    wallclock = (wallclock - walls);
    wallclock =  wallclock / 60;
    wallm = fmod(ad::passive_value(wallclock),60.0);
    wallh = static_cast<int>((wallclock - wallm) / 60.0);
