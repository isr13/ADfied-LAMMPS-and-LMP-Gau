...
private:
  ...
  myScalar **fields;         // per-atom field values
  int maxnew;              // allocation size of fields array
  myScalar **buf;            // read buffer

  int scaled;              // 0/1 if dump file coords are unscaled/scaled
  int wrapped;             // 0/1 if dump file coords are unwrapped/wrapped

  myScalar box[3][3];         // dump file box parameters
  myScalar xlo,xhi,ylo,yhi,zlo,zhi,xy,xz,yz;  // dump snapshot box params
  myScalar xprd,yprd,zprd;
  ...
  myScalar xfield(int, int);
  myScalar yfield(int, int);
  myScalar zfield(int, int);
};
