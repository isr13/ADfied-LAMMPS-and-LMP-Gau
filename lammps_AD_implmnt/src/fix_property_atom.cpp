...
void FixPropertyAtom::write_data_section_pack(int /*mth*/, myScalar **buf)
{
  int i;
  ...
  for (int m = 0; m < nvalue; m++) {
    ...
    if (style[m] == MOLECULE) {
      ...
      myScalar *dvec = atom->dvector[index[m]];
...
myScalar FixPropertyAtom::memory_usage()
{
  myScalar bytes = 0.0;
  for (int m = 0; m < nvalue; m++) {
    if (style[m] == MOLECULE) bytes = atom->nmax * sizeof(tagint);
    else if (style[m] == CHARGE) bytes = atom->nmax * sizeof(myScalar);
    else if (style[m] == RMASS) bytes = atom->nmax * sizeof(myScalar);
    else if (style[m] == INTEGER) bytes = atom->nmax * sizeof(int);
    else if (style[m] == DOUBLE) bytes = atom->nmax * sizeof(myScalar);
  }
  return bytes;
}
...
void FixPropertyAtom::grow_arrays(int nmax)
{
  for (int m = 0; m < nvalue; m++) {
    if (style[m] == MOLECULE) {
      ...
      size_t nbytes = (nmax-nmax_old) * sizeof(myScalar);
...
int FixPropertyAtom::pack_border(int n, int *list, myScalar *buf)
{
  int i,j,k;
  ...
  for (k = 0; k < nvalue; k++) {
    if (style[k] == MOLECULE) {
      ...
      myScalar *dvector = atom->dvector[index[k]];
      for (i = 0; i < n; i++) {
        j = list[i];
        buf[m++] = dvector[j];
      }
...
int FixPropertyAtom::unpack_border(int n, int first, myScalar *buf)
{
  int i,k,last;
...
int FixPropertyAtom::pack_exchange(int i, myScalar *buf)
{
  for (int m = 0; m < nvalue; m++) {
...
int FixPropertyAtom::unpack_exchange(int nlocal, myScalar *buf)
{
  for (int m = 0; m < nvalue; m++) {
    if (style[m] == MOLECULE)
...
int FixPropertyAtom::pack_restart(int i, myScalar *buf)
{
  buf[0] = nvalue+1;
