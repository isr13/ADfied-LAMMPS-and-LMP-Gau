...
myScalar ComputeTempRamp::compute_scalar()
{
  myScalar fraction,vramp,vthermal[3];
...
void ComputeTempRamp::remove_bias(int i, myScalar *v)
{
  myScalar fraction = (atom->x[i][coord_dim] - coord_lo) / (coord_hi - coord_lo);
  fraction = MAX(fraction,0.0);
  fraction = MIN(fraction,1.0);
...
void ComputeTempRamp::remove_bias_thr(int i, myScalar *v, myScalar *b)
{
  myScalar fraction = (atom->x[i][coord_dim] - coord_lo) / (coord_hi - coord_lo);
  fraction = MAX(fraction,0.0);
  fraction = MIN(fraction,1.0);
...
void ComputeTempRamp::restore_bias(int /*i*/, myScalar *v)
{
  v[v_dim] += vbias[v_dim];
}
...
void ComputeTempRamp::restore_bias_thr(int /*i*/, myScalar *v, myScalar *b)
{
  v[v_dim] += b[v_dim];
}
...
myScalar ComputeTempRamp::memory_usage()
{
