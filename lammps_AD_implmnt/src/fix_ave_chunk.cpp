...
void FixAveChunk::end_of_step()
{
  ...
  for (m = 0; m < nvalues; m++) {
    ...
    // invoke compute if not previously invoked
      ...
      myScalar *vector = compute->vector_atom;
      myScalar **array = compute->array_atom;
      int jm1 = j - 1;
    ...
    // access fix fields, guaranteed to be ready
      ...
      myScalar *vector = modify->fix[n]->vector_atom;
      myScalar **array = modify->fix[n]->array_atom;
      int jm1 = j - 1;
  ...
  if (normflag == ALL) {
    ...
    MY_MPI_Allreduce(count_one,count_many,nchunk,MPI_DOUBLE,MPI_SUM,world);
  ...
  myScalar repeat = nrepeat;

  if (normflag == ALL) {
    MY_MPI_Allreduce(count_many,count_sum,nchunk,MPI_DOUBLE,MPI_SUM,world);
    MY_MPI_Allreduce(&values_many[0][0],&values_sum[0][0],nchunk*nvalues,
...
myScalar FixAveChunk::compute_array(int i, int j)
{
  if (values_total == NULL) return 0.0;
  if (i >= nchunk) return 0.0;
  if (j < colextra) {
    if (cchunk->compress) {
      if (j == 0) return (myScalar) cchunk->chunkID[i];
...
myScalar FixAveChunk::memory_usage()
{
  myScalar bytes = maxvar * sizeof(myScalar);         // varatom
  bytes += 4*maxchunk * sizeof(myScalar);           // count one,many,sum,total
  bytes += nvalues*maxchunk * sizeof(myScalar);     // values one,many,sum,total
  bytes += nwindow*maxchunk * sizeof(myScalar);          // count_list
  bytes += nwindow*maxchunk*nvalues * sizeof(myScalar);  // values_list
  return bytes;
}
