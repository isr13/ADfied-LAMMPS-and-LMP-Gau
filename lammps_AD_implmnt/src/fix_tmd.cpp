...
nfileevery(0), fp(NULL), xf(NULL), xold(NULL)
{
  ...
  myScalar rho_start_total;
  MY_MPI_Allreduce(&rho_start,&rho_start_total,1,MPI_DOUBLE,MPI_SUM,world);
...
{
  myScalar a,b,c,d,e;
  myScalar dx,dy,dz,dxkt,dykt,dzkt;
  myScalar dxold,dyold,dzold,xback,yback,zback;
  myScalar gamma_forward,gamma_back,gamma_max,lambda;
  myScalar kt,fr,kttotal,frtotal,dtfm;
  myScalar unwrap[3];
  ...
  myScalar rho_target = rho_start + delta * (rho_stop - rho_start);
  ...
  myScalar abe[3],abetotal[3];
  abe[0] = a;  abe[1] = b;  abe[2] = e;
  MY_MPI_Allreduce(abe,abetotal,3,MPI_DOUBLE,MPI_SUM,world);
  ...
  myScalar r[2],rtotal[2];
  r[0] = fr;  r[1] = kt;
  MY_MPI_Allreduce(r,rtotal,2,MPI_DOUBLE,MPI_SUM,world);
  frtotal = rtotal[0];
  kttotal = rtotal[1];
...
myScalar FixTMD::memory_usage()
{
  myScalar bytes = 2*atom->nmax*3 * sizeof(myScalar);
  return bytes;
}
...
int FixTMD::pack_exchange(int i, myScalar *buf)
{
  buf[0] = xf[i][0];
  buf[1] = xf[i][1];
  buf[2] = xf[i][2];
...
int FixTMD::unpack_exchange(int nlocal, myScalar *buf)
{
  xf[nlocal][0] = buf[0];
  xf[nlocal][1] = buf[1];
  xf[nlocal][2] = buf[2];
      ...
      *next = '\0';
      ...
      if (imageflag){
        n = 7 - sscanf(bufptr,TAGINT_FORMAT " %lg %lg %lg %d %d %d",
                       &itag,&tmp_x,&tmp_y,&tmp_z,&ix,&iy,&iz);
	x = tmp_x;
	y = tmp_y;
	z = tmp_z;
        }
      else {
        n = 4 - sscanf(bufptr,TAGINT_FORMAT " %lg %lg %lg",&itag,&tmp_x,&tmp_y,&tmp_z);
        x = tmp_x;
	y = tmp_y;
	z = tmp_z;
