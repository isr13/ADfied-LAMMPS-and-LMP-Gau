  ...
  if (sort_flag) {
    ...
    if (sortcol == 0 && atom->tag_consecutive() && !gcmcflag) {
      ...
      if (maxall-minall+1 == isize) {
        reorderflag = 1;
        myScalar range = maxall-minall + EPSILON;
...
void Dump::write()
{
  ...
  bigint bnme = nme;
  MY_MPI_Allreduce(&bnme,&ntotal,1,MPI_LMP_BIGINT,MPI_SUM,world);

  int nmax;
  if (multiproc != nprocs) MY_MPI_Allreduce(&nme,&nmax,1,MPI_INT,MPI_MAX,world);
  else nmax = nme;
  ...
  if (multiproc)
    MY_MPI_Allreduce(&bnme,&nheader,1,MPI_LMP_BIGINT,MPI_SUM,clustercomm);
  ...
  if (pbcflag) {
    ...
    if (nlocal) {
      memcpy(&xpbc[0][0],&atom->x[0][0],3*nlocal*sizeof(myScalar));
      memcpy(&vpbc[0][0],&atom->v[0][0],3*nlocal*sizeof(myScalar));
  ...
  // if buffering, convert myScalars into strings
  ...
  if (buffer_flag && !binary) {
    ...
    int nsmin,nsmax;
    MY_MPI_Allreduce(&nsme,&nsmin,1,MPI_INT,MPI_MIN,world);
    if (nsmin < 0) error->all(FLERR,"Too much buffered per-proc info for dump");
    if (multiproc != nprocs)
      MY_MPI_Allreduce(&nsme,&nsmax,1,MPI_INT,MPI_MAX,world);
    else nsmax = nsme;
    if (nsmax > maxsbuf) {
      maxsbuf = nsmax;
  ...
  // comm and output buf of myScalars
  ...
  // comm and output sbuf = one big string of formatted values per proc
    ...
    if (filewriter) {
      for (int iproc = 0; iproc < nclusterprocs; iproc++) {
        ...
        write_data(nchars,(myScalar *) sbuf);
      }
      if (flush_flag && fp) fflush(fp);
...
void Dump::sort()
{
  ...
  // if multiple procs, exchange datums between procs via irregular
    ...
    if (sortcol == 0) {
      ...
      myScalar range = maxall-minall + 0.5;
      for (i = 0; i < nme; i++) {
      ...
      myScalar minall,maxall;
      MY_MPI_Allreduce(&min,&minall,1,MPI_DOUBLE,MPI_MIN,world);
      MY_MPI_Allreduce(&max,&maxall,1,MPI_DOUBLE,MPI_MAX,world);
      myScalar range = maxall-minall + EPSILON*(maxall-minall);
    ...
    irregular->exchange_data((char *) buf,size_one*sizeof(myScalar),
                             (char *) bufsort);
    if (sortcol == 0)
  ...
  int nmax;
  MY_MPI_Allreduce(&nme,&nmax,1,MPI_INT,MPI_MAX,world);
  ...
  int nbytes = size_one*sizeof(myScalar);
  for (i = 0; i < nme; i++)
...
{
  myScalar *bufsort = dumpptr->bufsort;
...
bigint Dump::memory_usage()
{
  ...
  if (pbcflag) {
    bytes += 6*maxpbc * sizeof(myScalar);
