...
myScalar FixReaxC::memory_usage()
{
  int nmax = atom->nmax;
  myScalar bytes = nmax * 2 * sizeof(int);
  return bytes;
}
...
int FixReaxC::pack_exchange(int i, myScalar *buf)
{
  buf[0] = num_bonds[i];
  buf[1] = num_hbonds[i];
  return 2;
...
int FixReaxC::unpack_exchange(int nlocal, myScalar *buf)
{
...
int FixReaxC::pack_forward_comm(int n, int *list, myScalar *buf,
...
void FixReaxC::unpack_forward_comm(int n, int first, myScalar *buf)
{
  int i,m,last;
