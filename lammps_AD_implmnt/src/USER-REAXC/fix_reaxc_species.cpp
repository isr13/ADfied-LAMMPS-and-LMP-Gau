...
void FixReaxCSpecies::FindMolecule ()
{
  ...
  int *mask = atom->mask;
  int *ilist;
  myScalar bo_tmp,bo_cut;
  myScalar **spec_atom = f_SPECBOND->array_atom;
  ...
  while (1) {
    ...
    MY_MPI_Allreduce(&change,&anychange,1,MPI_INT,MPI_MAX,world);
    if (!anychange) break;

    MY_MPI_Allreduce(&loop,&looptot,1,MPI_INT,MPI_SUM,world);
...
void FixReaxCSpecies::SortMolecule(int &Nmole)
{
  ...
  MY_MPI_Allreduce(&lo,&idlo,1,MPI_INT,MPI_MIN,world);
  MY_MPI_Allreduce(&hi,&idhi,1,MPI_INT,MPI_MAX,world);
  if (idlo == ntotal)
    if (me == 0)
  ...
  MY_MPI_Allreduce(molmap,molmapall,nlen,MPI_INT,MPI_MAX,world);
...
void FixReaxCSpecies::FindSpecies(int Nmole, int &Nspec)
{
  ...
  for (m = 1, Nspec = 0; m <= Nmole; m ++) {
    ...
    MY_MPI_Allreduce(&flag_mol,&flag_tmp,1,MPI_INT,MPI_MAX,world);
    flag_mol = flag_tmp;

    MY_MPI_Allreduce(Name,Nameall,ntypes,MPI_INT,MPI_SUM,world);
...
void FixReaxCSpecies::WritePos(int Nmole, int Nspec)
{
  ...
  int *Nameall;
  int *mask =atom->mask;
  myScalar avq, avq_tmp, avx[3], avx_tmp, box[3], halfbox[3];
  myScalar **spec_atom = f_SPECBOND->array_atom;
  ...
  for (m = 1; m <= Nmole; m ++) {
    ...
    avq_tmp = 0.0;
    MY_MPI_Allreduce(&avq,&avq_tmp,1,MPI_DOUBLE,MPI_SUM,world);
    avq = avq_tmp;
...
myScalar FixReaxCSpecies::compute_vector(int n)
{
  if (n == 0)
    return vector_nmole;
  if (n == 1)
...
int FixReaxCSpecies::nint(const myScalar &r)
{
  int i = 0;
...
int FixReaxCSpecies::pack_forward_comm(int n, int *list, myScalar *buf,
...
void FixReaxCSpecies::unpack_forward_comm(int n, int first, myScalar *buf)
{
  int i,m,last;
...
myScalar FixReaxCSpecies::memory_usage()
{
  myScalar bytes;

  bytes = 4*nmax*sizeof(myScalar);  // clusterID + x0
