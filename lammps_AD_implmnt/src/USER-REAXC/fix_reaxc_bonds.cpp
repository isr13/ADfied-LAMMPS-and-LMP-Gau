...

{
  ...
  MY_MPI_Allreduce(&numbonds,&numbonds_max,1,MPI_INT,MPI_MAX,world);
  MY_MPI_Allreduce(&nlocal,&nlocal_max,1,MPI_INT,MPI_MAX,world);
...
void FixReaxCBonds::PassBuffer(myScalar *buf, int &nbuf_local)
{
  int i, j, k, numbonds;
  int nlocal = atom->nlocal;
...
void FixReaxCBonds::RecvBuffer(myScalar *buf, int nbuf, int nbuf_local,
                               int natoms, int maxnum)
{
  ...
  myScalar sbotmp, nlptmp, avqtmp, abotmp;

  myScalar cutof3 = reaxc->control->bg_cut;
...
int FixReaxCBonds::nint(const myScalar &r)
{
  int i = 0;
...
myScalar FixReaxCBonds::memory_usage()
{
  myScalar bytes;

  bytes = 3.0*nmax*sizeof(myScalar);
  bytes += nmax*sizeof(int);
  bytes += 1.0*nmax*MAXREAXBOND*sizeof(myScalar);
