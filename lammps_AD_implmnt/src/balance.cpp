...
void Balance::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"x") == 0) {
      ...
      if (strcmp(arg[iarg+1],"uniform") == 0) {
        ...
        xflag = USER;
        delete [] user_xsplit;
        user_xsplit = new myScalar[procgrid[0]+1];
        user_xsplit[0] = 0.0;
        iarg++;
      ...
      if (strcmp(arg[iarg+1],"uniform") == 0) {
        ...
        yflag = USER;
        delete [] user_ysplit;
        user_ysplit = new myScalar[procgrid[1]+1];
        user_ysplit[0] = 0.0;
        iarg++;
      ...
      if (strcmp(arg[iarg+1],"uniform") == 0) {
        ...
        zflag = USER;
        delete [] user_zsplit;
        user_zsplit = new myScalar[procgrid[2]+1];
        user_zsplit[0] = 0.0;
        iarg++;
  ...
  init_imbalance(0);
  set_weights();
  myScalar imbinit = imbalance_factor(maxinit);
  ...
  MY_MPI_Allreduce(&nblocal,&natoms,1,MPI_LMP_BIGINT,MPI_SUM,world);
  ...
  myScalar maxfinal;
  myScalar imbfinal = imbalance_factor(maxfinal);
  if (wtflag) fixstore->disable = 1;

  // stats output

  myScalar stop_time = MPI_Wtime();
...
myScalar Balance::imbalance_factor(myScalar &maxcost)
{
  myScalar mycost,totalcost;
  ...
  MY_MPI_Allreduce(&mycost,&maxcost,1,MPI_DOUBLE,MPI_MAX,world);
  MY_MPI_Allreduce(&mycost,&totalcost,1,MPI_DOUBLE,MPI_SUM,world);

  myScalar imbalance = 1.0;
...
int *Balance::bisection(int sortflag)
{
  ...
  myScalar *boxlo = domain->boxlo;
  myScalar *boxhi = domain->boxhi;
  myScalar *prd = domain->prd;
  ...
  MY_MPI_Allreduce(shrink,shrinkall,6,MPI_DOUBLE,MPI_MIN,world);
  shrinkall[3] = -shrinkall[3];
  shrinkall[4] = -shrinkall[4];
  shrinkall[5] = -shrinkall[5];

  myScalar *shrinklo = &shrinkall[0];
  myScalar *shrinkhi = &shrinkall[3];
...
void Balance::shift_setup_static(char *str)
{
  ...
  onecost = new myScalar[max];
  allcost = new myScalar[max];
  sum = new myScalar[max+1];
  target = new myScalar[max+1];
  lo = new myScalar[max+1];
  hi = new myScalar[max+1];
  losum = new myScalar[max+1];
  hisum = new myScalar[max+1];
...
void Balance::shift_setup(char *str, int nitermax_in, myScalar thresh_in)
{
...
int Balance::shift()
{
  ...
  myScalar delta = pow(stopthresh,1.0/ndim) - 1.0;
  int *procgrid = comm->procgrid;
  ...
  for (int idim = 0; idim < ndim; idim++) {
    ...
    */
    ...
    myScalar imbfactor = imbalance_splits();
...
void Balance::tally(int dim, int n, myScalar *split)
{
  ...
  MY_MPI_Allreduce(onecost,allcost,n,MPI_DOUBLE,MPI_SUM,world);
...
int Balance::adjust(int n, myScalar *split)
{
  int i;
  myScalar fraction;
...
myScalar Balance::imbalance_splits()
{
  myScalar *xsplit = comm->xsplit;
  myScalar *ysplit = comm->ysplit;
  myScalar *zsplit = comm->zsplit;
  ...
  MY_MPI_Allreduce(proccost,allproccost,nprocs,MPI_DOUBLE,MPI_SUM,world);

  myScalar maxcost = 0.0;
  myScalar totalcost = 0.0;
...
int Balance::binary(myScalar value, int n, myScalar *vec)
{
  int lo = 0;
  int hi = n-1;
...
#ifdef BALANCE_DEBUG
void Balance::debug_shift_output(int idim, int m, int np, myScalar *split)
{
  int i;
  const char *dim = NULL;
