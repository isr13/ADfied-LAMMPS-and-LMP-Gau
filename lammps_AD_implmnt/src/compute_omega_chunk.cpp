...
void ComputeOmegaChunk::compute_array()
{
  ...
  MY_MPI_Allreduce(&com[0][0],&comall[0][0],3*nchunk,MPI_DOUBLE,MPI_SUM,world);
  ...
  MY_MPI_Allreduce(&inertia[0][0],&inertiaall[0][0],6*nchunk,
  ...
  MY_MPI_Allreduce(&angmom[0][0],&angmomall[0][0],3*nchunk,
                MPI_DOUBLE,MPI_SUM,world);

  // compute omega for each chunk

  myScalar determinant,invdeterminant;
  myScalar idiag[3],ex[3],ey[3],ez[3],cross[3];
  myScalar ione[3][3],inverse[3][3],evectors[3][3];
  myScalar *iall,*mall;
...
myScalar ComputeOmegaChunk::memory_usage()
{
  myScalar bytes = (bigint) maxchunk * 2 * sizeof(myScalar);
  bytes += (bigint) maxchunk * 2*3 * sizeof(myScalar);
  bytes += (bigint) maxchunk * 2*6 * sizeof(myScalar);
  bytes += (bigint) maxchunk * 2*3 * sizeof(myScalar);
  bytes += (bigint) maxchunk * 3 * sizeof(myScalar);
  return bytes;
}
