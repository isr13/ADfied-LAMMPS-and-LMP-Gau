...
static const myScalar  MIN_ETOL_MAG = 1.0e-8;
...
void MinHFTN::setup_style()
{
  ...
  if (nextra_global) {
    for (int  i = 1; i < NUM_HFTN_ATOM_BASED_VECTORS; i++) {
      _daExtraGlobal[i] = new myScalar[nextra_global];
  ...
  if (nextra_atom) {
    for (int  i = 0; i < NUM_HFTN_ATOM_BASED_VECTORS; i++)
      _daExtraAtom[i] = new myScalar*[nextra_atom];
...
int MinHFTN::iterate(int)
{
  ...
  myScalar  dFinalEnergy = 0.0;
  myScalar  dFinalFnorm2 = 0.0;
                           ...
                           myScalar &  dFinalForce2)
{
  ...
  if (nextra_atom) {
    for (int  m = 0; m < nextra_atom; m++) {
      myScalar *  xatom  = xextra_atom[m];
      myScalar *  xkAtom = _daExtraAtom[VEC_XK][m];
      int  n = extra_nlen[m];
      for (int  i = 0; i < n; i++)
        xkAtom[i] = xatom[i];
    }
  ...
  myScalar  dMaxTrustRadius = dmax * sqrt((myScalar) _nNumUnknowns);

  dTrustRadius = MIN (dTrustRadius, dMaxTrustRadius);
  myScalar  dLastNewtonStep2 = dMaxTrustRadius;
  ...
  myScalar  dCurrentEnergy    = dInitialEnergy;
  myScalar  dCurrentForce2    = dInitialForce2;
  for (niter = 0; niter < update->nsteps; niter++) {
    ...
    myScalar  dCgForce2StopTol = MIN(ad::passive_value((dCurrentForce2 / 2.0)), (0.1 / (niter+1)));
    ...
    myScalar  dGradDotP = calc_grad_dot_v_using_mpi_ (VEC_CG_P);
    ...
    if (nextra_atom) {
      for (int  m = 0; m < nextra_atom; m++) {
        myScalar *  xatom  = xextra_atom[m];
        myScalar *  xkAtom = _daExtraAtom[VEC_XK][m];
        myScalar *  pAtom  = _daExtraAtom[VEC_CG_P][m];
        int  n = extra_nlen[m];
        for (int  i = 0; i < n; i++)
    ...
    myScalar  dAred = dCurrentEnergy - dNewEnergy;
    ...
    if (nStepType != CGSTEP_TO_DMAX) {
      myScalar  dMag = 0.5 * (fabs (dCurrentEnergy) + fabs (dNewEnergy));
    ...
    myScalar  dPHP = calc_dot_prod_using_mpi_ (VEC_CG_P, VEC_CG_HD);
    myScalar  dPred = - dGradDotP - (0.5 * dPHP);
...
bool MinHFTN::compute_inner_cg_step_(const myScalar    dTrustRadius,
                                     ...
                                     myScalar &  dStepLengthInf)
{
  ...
  myScalar  dEnergyAtX = dEnergyAtXin;
  myScalar  dForce2AtX = dForce2AtXin;
  ...
  if (nextra_atom) {
    for (int  m = 0; m < nextra_atom; m++) {
      myScalar *  fatom = fextra_atom[m];
      myScalar *  rAtom = _daExtraAtom[VEC_CG_R][m];
      myScalar *  dAtom = _daExtraAtom[VEC_CG_D][m];
      int  n = extra_nlen[m];
      for (int  i = 0; i < n; i++) {
        rAtom[i] = fatom[i];
        dAtom[i] = fatom[i];
  ...
  myScalar  dRR = dForce2AtX * dForce2AtX;
  myScalar  dR0norm2 = sqrt (dRR);
  ...
  if (dForce2AtX < (sqrt (MACHINE_EPS) * MAX (1.0, fabs (ad::passive_value(dEnergyAtX)))) )
  ...
  for (int  nInnerIter = 0; nInnerIter < nMaxInnerIters; nInnerIter++) {
    ...
    if (dDHD <= (MACHINE_EPS * dDD)) {
      ...
      myScalar  dPdotD  = calc_dot_prod_using_mpi_ (VEC_CG_P, VEC_CG_D);
      myScalar  dPdotHD = calc_dot_prod_using_mpi_ (VEC_CG_P, VEC_CG_HD);
      ...
      myScalar  dGradDotD = calc_grad_dot_v_using_mpi_ (VEC_CG_D);

      myScalar  tau = compute_to_tr_ (dPP, dPdotD, dDD, dTrustRadius,
                                    true, dDHD, dPdotHD, dGradDotD);

      //---- MOVE THE POINT.
      if (nextra_global) {
        myScalar *  pGlobal = _daExtraGlobal[VEC_CG_P];
        myScalar *  dGlobal = _daExtraGlobal[VEC_CG_D];
        for (int  i = 0; i < nextra_global; i++) {
    ...
    if (nextra_global) {
      myScalar *  pGlobal  = _daExtraGlobal[VEC_CG_P];
      myScalar *  dGlobal  = _daExtraGlobal[VEC_CG_D];
      myScalar *  d1Global = _daExtraGlobal[VEC_DIF1];
      for (int  i = 0; i < nextra_global; i++) {
    ...
    if (nextra_atom) {
      for (int  m = 0; m < nextra_atom; m++) {
        myScalar *  pAtom  = _daExtraAtom[VEC_CG_P][m];
        myScalar *  dAtom  = _daExtraAtom[VEC_CG_D][m];
        myScalar *  d1Atom = _daExtraAtom[VEC_DIF1][m];
        int  n = extra_nlen[m];
        for (int  i = 0; i < n; i++) {
          d1Atom[i] = pAtom[i];
    ...
    if (nextra_global) {
      myScalar *  rGlobal  = _daExtraGlobal[VEC_CG_R];
      myScalar *  hdGlobal = _daExtraGlobal[VEC_CG_HD];
      for (int  i = 0; i < nextra_global; i++)
    ...
    if (nextra_atom) {
      for (int  m = 0; m < nextra_atom; m++) {
        myScalar *  rAtom  = _daExtraAtom[VEC_CG_R][m];
        myScalar *  hdAtom = _daExtraAtom[VEC_CG_HD][m];
        int  n = extra_nlen[m];
        for (int  i = 0; i < n; i++)
          rAtom[i] -= dAlpha * hdAtom[i];
      }
    }
    myScalar  dRnewDotRnew = calc_dot_prod_using_mpi_ (VEC_CG_R, VEC_CG_R);
    ...
    myScalar  dBeta = dRnewDotRnew / dRR;
    if (nextra_global) {
      myScalar *  rGlobal = _daExtraGlobal[VEC_CG_R];
      myScalar *  dGlobal = _daExtraGlobal[VEC_CG_D];
      for (int  i = 0; i < nextra_global; i++)
...
myScalar MinHFTN::calc_xinf_using_mpi_(void) const
{
  myScalar dXInfLocal = 0.0;
  for (int  i = 0; i < nvec; i++)
    dXInfLocal = MAX(ad::passive_value(dXInfLocal),fabs(ad::passive_value(xvec[i])));

  myScalar  dXInf;
  MY_MPI_Allreduce (&dXInfLocal, &dXInf, 1, MPI_DOUBLE, MPI_MAX, world);

  if (nextra_atom) {
    for (int  m = 0; m < nextra_atom; m++) {
      myScalar *  xatom = xextra_atom[m];
      int  n = extra_nlen[m];
      myScalar  dXInfLocalExtra = 0.0;
      for (int  i = 0; i < n; i++)
        dXInfLocalExtra = MAX (ad::passive_value(dXInfLocalExtra), fabs (ad::passive_value(xatom[i])));
      myScalar  dXInfExtra;
      MY_MPI_Allreduce (&dXInfLocalExtra, &dXInfExtra,
...
myScalar MinHFTN::calc_dot_prod_using_mpi_(const int  nIx1,
                                          const int  nIx2) const
{
  myScalar dDotLocal = 0.0;
  for (int  i = 0; i < nvec; i++)
    dDotLocal += _daAVectors[nIx1][i] * _daAVectors[nIx2][i];
  if (nextra_atom) {
    for (int  m = 0; m < nextra_atom; m++) {
      myScalar *  i1Atom = _daExtraAtom[nIx1][m];
      myScalar *  i2Atom = _daExtraAtom[nIx2][m];
      int  n = extra_nlen[m];
      for (int  i = 0; i < n; i++)
  ...
  myScalar  dDot;
  MY_MPI_Allreduce (&dDotLocal, &dDot, 1, MPI_DOUBLE, MPI_SUM, world);

  if (nextra_global) {
    for (int  i = 0; i < nextra_global; i++) {
      myScalar *  i1Global = _daExtraGlobal[nIx1];
      myScalar *  i2Global = _daExtraGlobal[nIx2];
...
myScalar MinHFTN::calc_grad_dot_v_using_mpi_(const int  nIx) const
{
  //---- ASSUME THAT FORCES HAVE BEEN EVALUATED AT DESIRED ATOM POSITIONS.
  //---- REMEMBER THAT FORCES = -GRADIENT.

  myScalar dGradDotVLocal = 0.0;
  for (int i = 0; i < nvec; i++)
    dGradDotVLocal += - _daAVectors[nIx][i] * fvec[i];
  if (nextra_atom) {
    for (int m = 0; m < nextra_atom; m++) {
      myScalar *  fatom = fextra_atom[m];
      myScalar *  iAtom = _daExtraAtom[nIx][m];
      int  n = extra_nlen[m];
      for (int i = 0; i < n; i++)
  ...
  myScalar  dGradDotV;
  MY_MPI_Allreduce (&dGradDotVLocal, &dGradDotV, 1, MPI_DOUBLE, MPI_SUM, world);

  if (nextra_global) {
    for (int  i = 0; i < nextra_global; i++) {
      myScalar *  iGlobal = _daExtraGlobal[nIx];
...
void MinHFTN::calc_dhd_dd_using_mpi_(myScalar &  dDHD,
                                     myScalar &  dDD) const
{
  myScalar dDHDLocal = 0.0;
  myScalar dDDLocal  = 0.0;
  for (int  i = 0; i < nvec; i++) {
  ...
  myScalar  daDotsLocal[2];
  daDotsLocal[0] = dDHDLocal;
  daDotsLocal[1] = dDDLocal;
  myScalar  daDots[2];
  MY_MPI_Allreduce (daDotsLocal, daDots, 2, MPI_DOUBLE, MPI_SUM, world);
...
void MinHFTN::calc_ppnew_pdold_using_mpi_(myScalar &  dPnewDotPnew,
                                          myScalar &  dPoldDotD) const
{
  myScalar dPnewDotPnewLocal = 0.0;
  myScalar dPoldDotDLocal    = 0.0;
  for (int  i = 0; i < nvec; i++) {
    dPnewDotPnewLocal
...
void MinHFTN::calc_plengths_using_mpi_(myScalar &  dStepLength2,
                                       myScalar &  dStepLengthInf) const
{
  myScalar dPPLocal   = 0.0;
  myScalar dPInfLocal = 0.0;
  for (int  i = 0; i < nvec; i++) {
    dPPLocal += _daAVectors[VEC_CG_P][i] * _daAVectors[VEC_CG_P][i];
    dPInfLocal = MAX (ad::passive_value(dPInfLocal), fabs (ad::passive_value(_daAVectors[VEC_CG_P][i])));
  }
  if (nextra_atom) {
    for (int  m = 0; m < nextra_atom; m++) {
      myScalar *  pAtom = _daExtraAtom[VEC_CG_P][m];
      ...
      for (int  i = 0; i < n; i++) {
        ...
        dPInfLocal = MAX (ad::passive_value(dPInfLocal), fabs (ad::passive_value(pAtom[i])));
      }
    }
  }

  myScalar  dPP;
  MY_MPI_Allreduce (&dPPLocal, &dPP, 1, MPI_DOUBLE, MPI_SUM, world);

  myScalar  dPInf;
  MY_MPI_Allreduce (&dPInfLocal, &dPInf, 1, MPI_DOUBLE, MPI_MAX, world);

  if (nextra_global) {
    for (int  i = 0; i < nextra_global; i++) {
      myScalar *  pGlobal = _daExtraGlobal[VEC_CG_P];
      dPP += pGlobal[i] * pGlobal[i];
      dPInf = MAX (ad::passive_value(dPInf), fabs (ad::passive_value(pGlobal[i])));
    }
  }
...
bool MinHFTN::step_exceeds_TR_(const myScalar    dTrustRadius,
                               const myScalar    dPP,
                               const myScalar    dPD,
                               const myScalar    dDD,
                               myScalar &  dTau) const
...
{
  myScalar  dAlpha = dmax * sqrt((myScalar) _nNumUnknowns);
  ...
  if (nextra_global) {
    ...
    myScalar  dAlphaExtra = modify->max_alpha (_daExtraGlobal[VEC_CG_P]);
...
void MinHFTN::adjust_step_to_tau_(const myScalar tau)
{
  if (nextra_global) {
    myScalar *  pGlobal  = _daExtraGlobal[VEC_CG_P];
    myScalar *  dGlobal  = _daExtraGlobal[VEC_CG_D];
...
myScalar MinHFTN::compute_to_tr_(const myScalar  dPP,
                               const myScalar  dPD,
                               const myScalar  dDD,
                               ...
                               const myScalar  dGradDotD) const
{
  ...
  myScalar  dTRsqrd = dTrustRadius * dTrustRadius;
  myScalar  dDiscr = (dPD * dPD) - (dDD * (dPP - dTRsqrd));
  dDiscr = MAX (0.0, dDiscr);    //-- SHOULD NEVER BE NEGATIVE
  dDiscr = sqrt (dDiscr);

  myScalar  dRootPos = (-dPD + dDiscr) / dDD;
  myScalar  dRootNeg = (-dPD - dDiscr) / dDD;

  if (bConsiderBothRoots == false)
    return( dRootPos );

  //---- EVALUATE THE CG OBJECTIVE FUNCTION FOR EACH ROOT.
  myScalar  dTmpTerm = dGradDotD + dPdotHD;
  myScalar  dCgRedPos = (dRootPos * dTmpTerm) + (0.5 * dRootPos*dRootPos * dDHD);
  myScalar  dCgRedNeg = (dRootNeg * dTmpTerm) + (0.5 * dRootNeg*dRootNeg * dDHD);
                                ...
                                myScalar &  dNewEnergy)
{
  ...
  myScalar dDirNorm2SqrdLocal = 0.0;
  for (int  i = 0; i < nvec; i++)
    dDirNorm2SqrdLocal
      += _daAVectors[nIxDir][i] * _daAVectors[nIxDir][i];
  if (nextra_atom) {
    for (int  m = 0; m < nextra_atom; m++) {
      myScalar *  iAtom = _daExtraAtom[nIxDir][m];
      int  n = extra_nlen[m];
      for (int  i = 0; i < n; i++)
        dDirNorm2SqrdLocal += iAtom[i] * iAtom[i];
    }
  }
  myScalar  dDirNorm2Sqrd = 0.0;
  MY_MPI_Allreduce (&dDirNorm2SqrdLocal, &dDirNorm2Sqrd,
                 1, MPI_DOUBLE, MPI_SUM, world);
  if (nextra_global) {
    for (int  i = 0; i < nextra_global; i++) {
      myScalar *  iGlobal = _daExtraGlobal[nIxDir];
      dDirNorm2Sqrd += iGlobal[i] * iGlobal[i];
    }
  }
  myScalar  dDirNorm2 = sqrt (dDirNorm2Sqrd);
  ...
  if (bUseForwardDiffs) {
    ...
    myScalar  dEps = 2.0 * sqrt (1000.0 * MACHINE_EPS) / dDirNorm2;
    ...
    if (nextra_atom) {
      for (int  m = 0; m < nextra_atom; m++) {
        myScalar *  fatom  = fextra_atom[m];
        myScalar *  d2Atom = _daExtraAtom[VEC_DIF2][m];
        int  n = extra_nlen[m];
        for (int  i = 0; i < n; i++)
          d2Atom[i] = fatom[i];
      }
    ...
    if (nextra_atom) {
      for (int  m = 0; m < nextra_atom; m++) {
        myScalar *  iAtom  = _daExtraAtom[nIxResult][m];
        myScalar *  d2Atom = _daExtraAtom[VEC_DIF2][m];
        int  n = extra_nlen[m];
        for (int  i = 0; i < n; i++)
    ...
    //---- EQUATION IS FROM THE OLD LAMMPS VERSION, SAND98-8201.
    myScalar  dEps = pow (3000.0 * MACHINE_EPS, 0.33333333) / dDirNorm2;
    ...
    //---- REMEMBER THAT FORCES = -GRADIENT.
    if (nextra_global) {
      myScalar *  iGlobal  = _daExtraGlobal[nIxResult];
      myScalar *  d2Global = _daExtraGlobal[VEC_DIF2];
      for (int  i = 0; i < nextra_global; i++)
