...
myScalar ComputeReduceRegion::compute_one(int m, int flag)
{
  int i;
  ...
  // invoke compute if not previously invoked
    ...
    if (flavor[m] == PERATOM) {
      ...
      if (j == 0) {
        myScalar *compute_vector = compute->vector_local;
        ...
        myScalar **compute_array = compute->array_local;
        int n = compute->size_local_rows;
        int jm1 = j - 1;
        if (flag < 0)
          for (i = 0; i < n; i++)
...
bigint ComputeReduceRegion::count(int m)
{
  ...
  if (which[m] == X || which[m] == V || which[m] == F)
    ...
    if (flavor[m] == PERATOM) {
      ...
      bigint ncountall;
      MY_MPI_Allreduce(&ncount,&ncountall,1,MPI_DOUBLE,MPI_SUM,world);
      return ncountall;
    }
  } else if (which[m] == FIX) {
    Fix *fix = modify->fix[n];
