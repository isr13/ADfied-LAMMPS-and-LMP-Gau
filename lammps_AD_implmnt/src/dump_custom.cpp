...
void DumpCustom::header_binary(bigint ndump)
{
  ...
  fwrite(&boxxlo,sizeof(myScalar),1,fp);
  fwrite(&boxxhi,sizeof(myScalar),1,fp);
  fwrite(&boxylo,sizeof(myScalar),1,fp);
  fwrite(&boxyhi,sizeof(myScalar),1,fp);
  fwrite(&boxzlo,sizeof(myScalar),1,fp);
  fwrite(&boxzhi,sizeof(myScalar),1,fp);
...
void DumpCustom::header_binary_triclinic(bigint ndump)
{
  ...
  fwrite(&boxzlo,sizeof(myScalar),1,fp);
  fwrite(&boxzhi,sizeof(myScalar),1,fp);
  fwrite(&boxxy,sizeof(myScalar),1,fp);
  fwrite(&boxxz,sizeof(myScalar),1,fp);
  fwrite(&boxyz,sizeof(myScalar),1,fp);
  fwrite(&size_one,sizeof(int),1,fp);
  if (multiproc) fwrite(&nclusterprocs,sizeof(int),1,fp);
  else fwrite(&nprocs,sizeof(int),1,fp);
}
  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      if (thresh_array[ithresh] == ID) {
        ...
        myScalar **x = atom->x;
        myScalar boxxlo = domain->boxlo[0];
        myScalar invxprd = 1.0/domain->xprd;
        for (i = 0; i < nlocal; i++)
        ...
        myScalar **x = atom->x;
        myScalar boxylo = domain->boxlo[1];
        myScalar invyprd = 1.0/domain->yprd;
        for (i = 0; i < nlocal; i++)
        ...
        myScalar **x = atom->x;
        myScalar boxzlo = domain->boxlo[2];
        myScalar invzprd = 1.0/domain->zprd;
        for (i = 0; i < nlocal; i++)
...
/* ----------------------------------------------------------------------
   convert mybuf of myScalars to one big formatted string in sbuf
   return -1 if strlen exceeds an int, since used as arg in MPI calls in Dump
------------------------------------------------------------------------- */

int DumpCustom::convert_string(int n, myScalar *mybuf)
{
  int i,j;
...
void DumpCustom::write_data(int n, myScalar *mybuf)
{
...
void DumpCustom::write_binary(int n, myScalar *mybuf)
{
  n *= size_one;
  fwrite(&n,sizeof(int),1,fp);
  fwrite(mybuf,sizeof(myScalar),n,fp);
}

/* ---------------------------------------------------------------------- */

void DumpCustom::write_string(int n, myScalar *mybuf)
{
...
void DumpCustom::write_lines(int n, myScalar *mybuf)
{
  int i,j;
...
{
  myScalar *vector = compute[field2index[n]]->vector_atom;
  myScalar **array = compute[field2index[n]]->array_atom;
...
{
  myScalar *vector = fix[field2index[n]]->vector_atom;
  myScalar **array = fix[field2index[n]]->array_atom;
  int index = argindex[n];
...
{
  myScalar *vector = vbuf[field2index[n]];
...
void DumpCustom::pack_custom(int n)
{
  ...
  } else if (flag_custom[index] == 1) { // myScalar
    int iwhich,tmp;
    iwhich = atom->find_custom(id_custom[index],tmp);

    myScalar *dvector = atom->dvector[iwhich];
    for (int i = 0; i < nchoose; i++) {
